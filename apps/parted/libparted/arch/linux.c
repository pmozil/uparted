...
static void _device_set_sector_size(PedDevice *dev) {
    ...
    if (dev->phys_sector_size == 0) {
        ped_exception_throw(
            PED_EXCEPTION_WARNING, PED_EXCEPTION_OK,
...
static int init_ide(PedDevice *dev) {
    ...
    if (r && errno == EINVAL) {
        ...
        ex_status = ped_exception_throw(
            ...
            _("Could not get identity of device %s - %s"), dev->path,
            strerror(errno));
        switch (ex_status) {
...
static int init_scsi(PedDevice *dev) {
    ...
    if (ioctl(arch_specific->fd, SCSI_IOCTL_GET_IDLUN, &idlun) < 0) {
        ...
        if (ped_exception_throw(
                ...
                _("Error initialising SCSI device %s - %s"), dev->path,
                strerror(errno)) != PED_EXCEPTION_IGNORE)
            goto error_close_dev;
        if (!_device_probe_geometry(dev))
            goto error_close_dev;
        _device_close(dev);
...
static int init_dasd(PedDevice *dev, const char *model_name) {
    ...
    if (!ioctl(arch_specific->fd, BIODASDINFO, &dasd_info)) {
        ...
        arch_specific->devno =
            arch_specific->major * 256 + arch_specific->minor;
    }
...
static int _probe_proc_partitions() {
    ...
    while (fgets(buf, 512, proc_part_file) &&
           sscanf(buf, "%d %d %d %255s", &major, &minor, &size, part_name) ==
               4) {
    ...
    *length /= part->disk->dev->sector_size;
    ...
    if (!ok)
        ped_exception_throw(
            ...
            _("Unable to determine the start and length of %s."), dev_name);
    free(dev_name);
    return ok;
}
...
static int _dm_resize_partition(PedDisk *disk, const PedPartition *part) {
    ...
    dm_task_add_target(task, 0,
                       part->geom.length *
                           (disk->dev->sector_size / PED_SECTOR_SIZE_DEFAULT),
                       "linear", params);
    /* NOTE: DM_DEVICE_RELOAD doesn't generate udev events, so no cookie is
     * needed (it will freeze). DM_DEVICE_RESUME does, so get a cookie and
...
static int _disk_sync_part_table(PedDisk *disk) {
    ...
    for (i = 1; i <= lpn; i++) {
        ...
        unsigned int n_sleep =
            (max_sleep_seconds * 1000000 / sleep_microseconds);
        do {
...
static int _ped_device_like_dasd(const PedDevice *dev) {
    return (dev->type == PED_DEVICE_DASD) ||
           (dev->hw_geom.heads == 15 && dev->hw_geom.sectors == 12 &&
            (dev->hw_geom.cylinders * dev->hw_geom.heads *
