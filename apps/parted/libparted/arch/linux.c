...
#define SCSI_BLK_MAJOR(M)                                                      \
  ((M) == SCSI_DISK0_MAJOR || (M) == SCSI_CDROM_MAJOR ||                       \
...
static int _read_fd(int fd, char **buf) {
  char *p;
...
static int _major_type_in_devices(int major, const char *type) {
  int fd;
  char *buf = NULL;
  char *line;
  char *end;
...
static int _is_ide_major(int major) {
  switch (major) {
  case IDE0_MAJOR:
  case IDE1_MAJOR:
  case IDE2_MAJOR:
...
static int _is_dm_major(int major) {
  return _major_type_in_devices(major, "device-mapper");
}

static int _dm_maptype(PedDevice *dev) {
  ...
  if (!dm_task_set_major_minor(dmt, arch_specific->major, arch_specific->minor,
                               0))
    goto bad;
...
/* Return nonzero if device-mapper device, DEVPATH, is part of a dmraid
   ...
   start of its UUID.  */
static int _is_dmraid_device(const char *devpath) {
  int rc = 0;
  ...
  while (1) {
    if (!stat(dev->path, dev_stat)) {
      ...
      if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_CANCEL,
                              _("Could not stat device %s - %s."), dev->path,
                              strerror(errno)) != PED_EXCEPTION_RETRY)
        return 0;
    }
  }
}

static int _device_probe_type(PedDevice *dev) {
  ...
  } else if (_is_blkext_major(dev_major) && dev->path &&
             strstr(dev->path, "nvme")) {
    dev->type = PED_DEVICE_NVME;
  } else if (dev_major == RAM_MAJOR) {
    dev->type = PED_DEVICE_RAM;
  } else if (_is_blkext_major(dev_major) && dev->path &&
             strstr(dev->path, "pmem")) {
...
#if USE_BLKID
static void get_blkid_topology(LinuxSpecific *arch_specific) {
  arch_specific->probe = blkid_new_probe();
  if (!arch_specific->probe)
    return;

  if (blkid_probe_set_device(arch_specific->probe, arch_specific->fd, 0, 0))
    return;

  arch_specific->topology = blkid_probe_get_topology(arch_specific->probe);
}
#endif
...
static void _device_set_sector_size(PedDevice *dev) {
  ...
  if (!arch_specific->topology) {
    ...
    dev->phys_sector_size =
        blkid_topology_get_physical_sector_size(arch_specific->topology);
  }
...
static int _kernel_has_blkgetsize64(void) {
  ...
  if (version >= KERNEL_VERSION(2, 5, 4))
    return 1;
  if (version < KERNEL_VERSION(2, 5, 0) && version >= KERNEL_VERSION(2, 4, 18))
    return 1;
  return 0;
}
...
static PedSector _device_get_length(PedDevice *dev) {
  ...
  if (test_str && xstrtoll(test_str, NULL, 10, &test_size, NULL) == LONGINT_OK)
    return test_size;
...
static int _device_probe_geometry(PedDevice *dev) {
  ...
  dev->bios_geom.cylinders =
      dev->length / (dev->bios_geom.heads * dev->bios_geom.sectors);
...
static int init_ide(PedDevice *dev) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
  struct stat dev_stat;
  ...
  if (r && errno == EINVAL) {
    ...
    ex_status =
        ped_exception_throw(PED_EXCEPTION_WARNING, PED_EXCEPTION_IGNORE_CANCEL,
...
static char *read_device_sysfs_file(PedDevice *dev, const char *file) {
  FILE *f;
  char name_buf[128];
  char buf[256];

  snprintf(name_buf, 127, "/sys/block/%s/device/%s", last_component(dev->path),
           file);
 ...
 * issue this query.
 */
static int scsi_query_product_info(PedDevice *dev, char **vendor,
                                   char **product) {
 ...
 */
static int scsi_get_product_info(PedDevice *dev, char **vendor,
                                 char **product) {
...
static int init_scsi(PedDevice *dev) {
  struct scsi_idlun {
    uint32_t dev_id;
  ...
  if (ioctl(arch_specific->fd, SCSI_IOCTL_GET_IDLUN, &idlun) < 0) {
    ...
    if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_IGNORE_CANCEL,
                            ...
                            dev->path, strerror(errno)) != PED_EXCEPTION_IGNORE)
      goto error_close_dev;
    if (!_device_probe_geometry(dev))
      goto error_close_dev;
    _device_close(dev);
...
static int init_file(PedDevice *dev) {
  struct stat dev_stat;
...
#if defined __s390__ || defined __s390x__
static int init_dasd(PedDevice *dev, const char *model_name) {
  struct stat dev_stat;
  ...
  if (!ioctl(arch_specific->fd, BIODASDINFO, &dasd_info)) {
    ...
    arch_specific->devno = arch_specific->major * 256 + arch_specific->minor;
  }
...
static int init_generic(PedDevice *dev, const char *model_name) {
  struct stat dev_stat;
  PedExceptionOption ex_status;
...
static int sdmmc_get_product_info(PedDevice *dev, char **type, char **name) {
...
static int init_sdmmc(PedDevice *dev) {
  char id[128];
  char *type = NULL;
  char *name = NULL;
  ...
  if (sdmmc_get_product_info(dev, &type, &name)) {
    ...
    snprintf(id, sizeof(id) - 1, "%s", _("Generic SD/MMC Storage Card"));
  }
  free(type);
  free(name);
...
static int init_nvme(PedDevice *dev) {
  int ret;
...
static PedDevice *linux_new(const char *path) {
  PedDevice *dev;
  ...
  case PED_DEVICE_DM: {
    char *type;
    if (arch_specific->dmtype == NULL ||
        asprintf(&type, _("Linux device-mapper (%s)"), arch_specific->dmtype) ==
            -1)
      goto error_free_arch_specific;
...
static void linux_destroy(PedDevice *dev) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
...
static int linux_is_busy(PedDevice *dev) {
  int i;
  char *part_name;
      ...
      retry:
        if (fsync(fd) < 0 || close(fd) < 0)
          if (ped_exception_throw(PED_EXCEPTION_WARNING,
                                  ...
                                  _("Error fsyncing/closing %s: %s"), name,
                                  strerror(errno)) == PED_EXCEPTION_RETRY)
            goto retry;
      }
    }
...
static int linux_open(PedDevice *dev) { return _device_open(dev, RW_MODE); }
...
static int linux_refresh_open(PedDevice *dev) { return 1; }
...
retry:
  if (fsync(arch_specific->fd) < 0 || close(arch_specific->fd) < 0)
    if (ped_exception_throw(PED_EXCEPTION_WARNING,
                            PED_EXCEPTION_RETRY + PED_EXCEPTION_IGNORE,
                            _("Error fsyncing/closing %s: %s"), dev->path,
                            strerror(errno)) == PED_EXCEPTION_RETRY)
      goto retry;
  return 1;
}

static int linux_refresh_close(PedDevice *dev) {
  if (dev->dirty)
    _flush_cache(dev);
  return 1;
}
...
static _syscall5(int, _llseek, unsigned int, fd, unsigned long, offset_high,
  ...
  retval = _llseek(fd, ((unsigned long long)offset) >> 32,
                   ((unsigned long long)offset) & 0xffffffff, &result, whence);
  return (retval == -1 ? (loff_t)retval : result);
}

#endif /* SIZEOF_OFF_T < 8 */

static int _device_seek(const PedDevice *dev, PedSector sector) {
  LinuxSpecific *arch_specific;
...
static int _read_lastoddsector(const PedDevice *dev, void *buffer) {
  LinuxSpecific *arch_specific;
...
retry:
  ...
  if (ioctl(arch_specific->fd, BLKGETLASTSECT, &ioctl_param) == -1) {
    ...
    opt = ped_exception_throw(
        ...
        _("%s during read on %s"), strerror(errno), dev->path);
...
static int linux_read(const PedDevice *dev, void *buffer, PedSector start,
                      PedSector count) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
  PedExceptionOption ex_status;
  void *diobuf = NULL;
    ...
    */
    if (dev->type != PED_DEVICE_FILE && (dev->length & 1) &&
      ...
      return ped_device_read(dev, buffer, start, count - 1) &&
             _read_lastoddsector(dev, (char *)buffer + (count - 1) * 512);
  }
  while (1) {
    ...
    ex_status = ped_exception_throw(
        PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_IGNORE_CANCEL,
        _("%s during seek for read on %s"), strerror(errno), dev->path);
  ...
  while (1) {
    ...
    ex_status = ped_exception_throw(
        PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_IGNORE_CANCEL,
        (status == 0 ? _("%0.0send of file while reading %s")
...
static int _write_lastoddsector(PedDevice *dev, const void *buffer) {
  LinuxSpecific *arch_specific;
  struct blkdev_ioctl_param ioctl_param;
...
retry:
  ...
  if (ioctl(arch_specific->fd, BLKSETLASTSECT, &ioctl_param) == -1) {
    ...
    opt = ped_exception_throw(
        PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_IGNORE_CANCEL,
        _("%s during write on %s"), strerror(errno), dev->path);
...
static int linux_write(PedDevice *dev, const void *buffer, PedSector start,
                       PedSector count) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
  PedExceptionOption ex_status;
  void *diobuf;
    ...
    if (dev->type != PED_DEVICE_FILE && (dev->length & 1) &&
      ...
      return ped_device_write(dev, buffer, start, count - 1) &&
             _write_lastoddsector(
                 dev, ((char *)buffer + (count - 1) * dev->sector_size));
  }
  while (1) {
    if (_device_seek(dev, start))
      break;
    ...
    ex_status = ped_exception_throw(
        PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_IGNORE_CANCEL,
        _("%s during seek for write on %s"), strerror(errno), dev->path);
 ...
 */
static PedSector linux_check(PedDevice *dev, void *buffer, PedSector start,
                             PedSector count) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
  PedSector done = 0;
  int status;
...
static int _do_fsync(PedDevice *dev) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
  int status;
  PedExceptionOption ex_status;
...
static int linux_sync(PedDevice *dev) {
  PED_ASSERT(dev != NULL);
  PED_ASSERT(!dev->external_mode);
...
static int linux_sync_fast(PedDevice *dev) {
  PED_ASSERT(dev != NULL);
  PED_ASSERT(!dev->external_mode);
 ...
 */
static int _GL_ATTRIBUTE_PURE _match_rd_device(const char *name) {
  const char *pos;
  int state;
...
static int _probe_proc_partitions() {
  FILE *proc_part_file;
  ...
  while (fgets(buf, 512, proc_part_file) &&
         sscanf(buf, "%d %d %d %255s", &major, &minor, &size, part_name) == 4) {
...
static int _GL_ATTRIBUTE_PURE _skip_entry(const char *name) {
  struct _entry *i;
...
static int _probe_standard_devices() {
...
#ifdef ENABLE_DEVICE_MAPPER
static char *dm_canonical_path(PedDevice const *dev) {
  ...
  if (!task)
    goto err;
  if (!dm_task_set_major_minor(task, arch_specific->major, arch_specific->minor,
                               0))
    goto err;
  if (!dm_task_run(task))
    goto err;
...
static char *_device_get_part_path(PedDevice const *dev, int num) {
  ...
  if (5 < path_len && !strcmp(devpath + path_len - 5, "/disc")) {
    ...
    char const *p =
        (dev->type == PED_DEVICE_DAC960 || dev->type == PED_DEVICE_CPQARRAY ||
                 dev->type == PED_DEVICE_ATARAID ||
...
static char *linux_partition_get_path(const PedPartition *part) {
...
static int linux_partition_is_busy(const PedPartition *part) {
  PedPartition *walk;
...
static int _blkpg_part_command(PedDevice *dev, struct blkpg_partition *part,
                               int op) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(dev);
...
static int _blkpg_add_partition(PedDisk *disk, const PedPartition *part) {
  ...
  if (!_blkpg_part_command(disk->dev, &linux_part, BLKPG_ADD_PARTITION)) {
    return 0;
  }
...
static int _blkpg_remove_partition(PedDisk *disk, int n) {
  struct blkpg_partition linux_part;

  memset(&linux_part, 0, sizeof(linux_part));
  linux_part.pno = n;
  return _blkpg_part_command(disk->dev, &linux_part, BLKPG_DEL_PARTITION);
}
...
static int _blkpg_resize_partition(PedDisk *disk, const PedPartition *part) {
  ...
  if (!_blkpg_part_command(disk->dev, &linux_part, BLKPG_RESIZE_PARTITION)) {
    return ped_exception_throw(
               ...
               _("Error informing the kernel about modifications to "
                 ...
                 "before rebooting."),
               linux_part.devname, strerror(errno),
               linux_part.devname) == PED_EXCEPTION_IGNORE;
  }
...
static bool _sysfs_int_entry_from_dev(PedDevice const *dev, const char *entry,
                                      int *val) {
  char path[128];
...
/* Read the unsigned long long from /sys/block/DEV_BASE/PART_BASE/ENTRY
   ...
   Upon success, return true. Otherwise, return false. */
static bool _sysfs_ull_entry_from_part(PedPartition const *part,
                                       const char *entry,
...
/* Get the starting sector and length of a partition PART within a block device
   ...
   return false. */
static bool _kernel_get_partition_start_and_length(PedPartition const *part,
...
#ifdef ENABLE_DEVICE_MAPPER
static int _dm_remove_partition(PedDisk *disk, int partno) {
  int rc = 0;
  uint32_t cookie = 0;
...
static bool _dm_get_partition_start_and_length(PedPartition const *part,
                                               unsigned long long *start,
                                               unsigned long long *length) {
  ...
  char *params;
  char *target_type;
  dm_get_next_target(task, NULL, (uint64_t *)start, (uint64_t *)length,
                     &target_type, &params);
  if (sscanf(params, "%d:%d %Lu", &major, &minor, start) != 3)
    goto err;
  rc = 1;

  /* device-mapper uses 512b units, make sure we return length and start in
   * terms of the device's sector size.
   */
...
static int _dm_add_partition(PedDisk *disk, const PedPartition *part) {
  ...
  vol_name = zasprintf("%s%s%d", dev_name,
                       isdigit(dev_name[name_len - 1]) ? "p" : "", part->num);
  if (vol_name == NULL)
    goto err;
  ...
  if (!(params =
            zasprintf("%d:%d %lld", arch_specific->major, arch_specific->minor,
                      part->geom.start *
                          (disk->dev->sector_size / PED_SECTOR_SIZE_DEFAULT))))
    goto err;
...
static int _dm_resize_partition(PedDisk *disk, const PedPartition *part) {
  LinuxSpecific *arch_specific = LINUX_SPECIFIC(disk->dev);
  char *params = NULL;
  char *vol_name = NULL;
  const char *dev_name = NULL;
  ...
  dm_task_add_target(task, 0,
                     part->geom.length *
                         (disk->dev->sector_size / PED_SECTOR_SIZE_DEFAULT),
                     "linear", params);
  /* NOTE: DM_DEVICE_RELOAD doesn't generate udev events, so no cookie is needed
   * (it will freeze). DM_DEVICE_RESUME does, so get a cookie and synchronize
   * with udev.
   */
  if (dm_task_run(task)) {
    dm_task_destroy(task);
...
static int _disk_sync_part_table(PedDisk *disk) {
  ...
  for (i = 1; i <= lpn; i++) {
    ...
    if (part) {
      ...
      if (get_partition_start_and_length(part, &start, &length) &&
    ...
    unsigned int n_sleep = (max_sleep_seconds * 1000000 / sleep_microseconds);
    do {
  ...
  for (i = 1; i <= lpn; i++) {
    ...
    if (get_partition_start_and_length(part, &start, &length) &&
      ...
      if (resize_partition && start == part->geom.start) {
        /* try to resize */
...
#if USE_BLKID
static PedAlignment *linux_get_minimum_alignment(const PedDevice *dev) {
  blkid_topology tp = LINUX_SPECIFIC(dev)->topology;
  if (!tp)
    return NULL;

  if (blkid_topology_get_minimum_io_size(tp) == 0)
    return ped_alignment_new(blkid_topology_get_alignment_offset(tp) /
                                 dev->sector_size,
                             dev->phys_sector_size / dev->sector_size);
...
static PedAlignment *linux_get_optimum_alignment(const PedDevice *dev) {
  blkid_topology tp = LINUX_SPECIFIC(dev)->topology;
  if (!tp)
    return NULL;
  ...
  if ((!optimal_io && !minimum_io) ||
      (optimal_io && PED_DEFAULT_ALIGNMENT % optimal_io == 0 && minimum_io &&
       PED_DEFAULT_ALIGNMENT % minimum_io == 0) ||
      (!minimum_io && optimal_io && PED_DEFAULT_ALIGNMENT % optimal_io == 0) ||
      (!optimal_io && minimum_io && PED_DEFAULT_ALIGNMENT % minimum_io == 0))
    return ped_alignment_new(blkid_topology_get_alignment_offset(tp) /
                                 dev->sector_size,
 ...
 *         and 0 otherwise
 */
static int _ped_device_like_dasd(const PedDevice *dev) {
  return (dev->type == PED_DEVICE_DASD) ||
         (dev->hw_geom.heads == 15 && dev->hw_geom.sectors == 12 &&
          (dev->hw_geom.cylinders * dev->hw_geom.heads * dev->hw_geom.sectors *
               dev->phys_sector_size ==
           dev->length * dev->sector_size));
}

static PedAlignment *s390_get_minimum_alignment(const PedDevice *dev) {
#if USE_BLKID
  return linux_get_minimum_alignment(dev);
#else
  return ped_alignment_new(0, dev->phys_sector_size / dev->sector_size);
#endif
}

static PedAlignment *s390_get_optimum_alignment(const PedDevice *dev) {
