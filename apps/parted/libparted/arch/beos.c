...
static void _scan_for_disks(const char *path) {
...
#if B_ZETA_VERSION >= B_ZETA_VERSION_1_0_0
static int _device_init_ata(PedDevice *dev) {
  ...
  if (ide_info._48_bit_addresses_supported)
    ...
    dev->length = ide_info.cylinders * ide_info.heads * ide_info.sectors;
...
static int _device_init_generic_blkdev(PedDevice *dev) {
  ...
  dev->sector_size = dev->phys_sector_size = os.bytes_per_sector;
...
static int _device_init_blkdev(PedDevice *dev) {
  int type;
...
static int _device_init_file(PedDevice *dev, struct stat *dev_statp) {
...
static PedDevice *beos_new(const char *path) {
  ...
  dev->arch_specific = (BEOSSpecific *)ped_malloc(sizeof(BEOSSpecific));
...
static void beos_destroy(PedDevice *dev) {
  free(dev->arch_specific);
  free(dev->path);
  free(dev->model);
  free(dev);
...
static int beos_is_busy(PedDevice *dev) { return 1; }

static int beos_open(PedDevice *dev) {
...
static int beos_refresh_open(PedDevice *dev) { return 1; }

static int beos_close(PedDevice *dev) {
  BEOSSpecific *arch_specific = BEOS_SPECIFIC(dev);
...
static int beos_refresh_close(PedDevice *dev) {
  if (dev->dirty)
    _flush_cache(dev);
...
static int beos_read(const PedDevice *dev, void *buffer, PedSector start,
                     PedSector count) {
  BEOSSpecific *arch_specific = BEOS_SPECIFIC(dev);
  int status;
...
static int beos_write(PedDevice *dev, const void *buffer, PedSector start,
                      PedSector count) {
  BEOSSpecific *arch_specific = BEOS_SPECIFIC(dev);
  int status;
  PedExceptionOption ex_status;
...
static PedSector beos_check(PedDevice *dev, void *buffer, PedSector start,
                            PedSector count) {
  BEOSSpecific *arch_specific = BEOS_SPECIFIC(dev);
  PedSector done = 0;
  int status;
...
static int beos_sync(PedDevice *dev) { return 1; }

static int beos_sync_fast(PedDevice *dev) { return 1; }

/* Probe for all available disks */
static void beos_probe_all() {
  /* For BeOS/ZETA/Haiku, all disks are published under /dev/disk */
  _scan_for_disks("/dev/disk");
}

static char *beos_partition_get_path(const PedPartition *part) { return NULL; }

static int beos_partition_is_busy(const PedPartition *part) { return 0; }

static int beos_disk_commit(PedDisk *disk) { return 0; }
