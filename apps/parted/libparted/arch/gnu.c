...
retry_open:
  if (!ped_device_open(dev)) {
    ex_status =
        ped_exception_throw(PED_EXCEPTION_WARNING, PED_EXCEPTION_RETRY_CANCEL,
                            _("Unable to open %s."), dev->path);
    switch (ex_status) {
...
static PedDevice *_init_device(const char *path) {
  ...
  dev->arch_specific = (GNUSpecific *)ped_malloc(sizeof(GNUSpecific));
...
static int _reread_part_table(PedDevice *dev) {
  ...
  while (1) {
    ...
    err = file_set_translator(node, 0, FS_TRANS_SET, 0, 0, 0, MACH_PORT_NULL,
...
static void gnu_destroy(PedDevice *dev) {
  GNUSpecific *arch_specific = GNU_SPECIFIC(dev);
...
static PedDevice *gnu_new(const char *path) {
  PedDevice *dev;
  GNUSpecific *arch_specific;
...
retry_open:
  ...
  if (rw_err) {
    if (ispath) {
      ro_err =
          store_open(dev->path, STORE_READONLY, NULL, &arch_specific->store);
    } else {
    ...
    if (ro_err) {
      if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_CANCEL,
                              ...
                              strerror(ro_err)) != PED_EXCEPTION_RETRY) {
        return NULL;
      } else
        goto retry_open;
    } else {
...
static int gnu_is_busy(PedDevice *dev) { return 0; }

static int gnu_open(PedDevice *dev) { return 1; }

static int gnu_refresh_open(PedDevice *dev) { return 1; }

static int gnu_close(PedDevice *dev) {
  GNUSpecific *arch_specific = GNU_SPECIFIC(dev);
...
static int gnu_refresh_close(PedDevice *dev) {
  _flush_cache(dev);
  return 1;
}

static int gnu_read(const PedDevice *dev, void *user_buffer,
  ...
  if (PED_SECTOR_SIZE_DEFAULT != arch_specific->store->block_size) {
    ...
    store_read_length =
        (device_read_length + arch_specific->store->block_size - 1) /
  ...
  read_offset = start - store_start_block * arch_specific->store->block_size;

  if (store_read_length % arch_specific->store->block_size != 0)
    store_read_length = store_read_length + arch_specific->store->block_size -
  ...
  while (1) {
    ...
    if (err) {
      ex_status = ped_exception_throw(
          ...
          _("%s during read on %s"), strerror(err), dev->path);
    ...
    memcpy(user_buffer + have_read - read_offset,
           store_read_buffer +
               (have_read >= read_offset ? 0 : read_offset - have_read),
    ...
    if (store_read_buffer != local_buffer)
      vm_deallocate(mach_task_self(), (long)store_read_buffer, did_read);

    have_read += did_read;
    store_start_block += did_read / arch_specific->store->block_size;
...
static int gnu_write(PedDevice *dev, const void *buffer, PedSector start,
                     PedSector count) {
  GNUSpecific *arch_specific = GNU_SPECIFIC(dev);
  error_t err;
  PedExceptionOption ex_status;
...
#define PED_TO_STORE(store, sector)                                            \
  (((sector) * PED_SECTOR_SIZE_DEFAULT) / (store)->block_size)
  ...
  doggy_first_block_read:
    ...
    err = store_read(arch_specific->store,
                     ...
                     arch_specific->store->block_size, &temp, &did_read);
    ...
    write_offset =
        (start * PED_SECTOR_SIZE_DEFAULT) % arch_specific->store->block_size;
  ...
  doggy_first_block_write:
    err = store_write(arch_specific->store,
                      PED_TO_STORE(arch_specific->store, start), temp,
                      arch_specific->store->block_size, &did_write);
    ...
    if (err) {
      ex_status = ped_exception_throw(
          PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_IGNORE_CANCEL,
          _("%s during write on %s"), strerror(err), dev->path);
  ...
  while (count > 0 &&
    ...
    err = store_write(arch_specific->store,
                      PED_TO_STORE(arch_specific->store, start), buffer,
                      count * PED_SECTOR_SIZE_DEFAULT, &did_write);
 ...
 */
static PedSector gnu_check(PedDevice *dev, void *buffer, PedSector start,
                           PedSector count) {
  int status;
  int done = 0;
...
static int gnu_sync(PedDevice *dev) {
  GNUSpecific *arch_specific;
  error_t err;
  PedExceptionOption ex_status;
  ...
  while (1) {
    ...
    ex_status = ped_exception_throw(
        PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_IGNORE_CANCEL,
        _("%s trying to sync %s to disk"), strerror(errno), dev->path);
...
static void gnu_probe_all() { probe_standard_devices(); }

static char *gnu_partition_get_path(const PedPartition *part) {
...
static int gnu_partition_is_busy(const PedPartition *part) { return 0; }

static int gnu_disk_commit(PedDisk *disk) {
