...
static PedSector _device_get_length(PedDevice *dev) {
    ...
    return ((long long)store_blocks * store_block_size) /
...
retry_open:
    if (!ped_device_open(dev)) {
        ex_status = ped_exception_throw(PED_EXCEPTION_WARNING,
...
static int _reread_part_table(PedDevice *dev) {
    ...
    while (1) {
        ...
        err = file_set_translator(node, 0, FS_TRANS_SET, 0, 0, 0,
...
#define PED_TO_STORE(store, sector)                                            \
    ...
    while (count > 0 && count >= arch_specific->store->block_size /
                                     PED_SECTOR_SIZE_DEFAULT) {
        err = store_write(arch_specific->store,
