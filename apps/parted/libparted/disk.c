...
PedDisk *ped_disk_new(PedDevice *dev) {
  ...
  if (!type) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                        _("%s: unrecognised disk label"), dev->path);
...
static int _add_duplicate_part(PedDisk *disk, PedPartition *old_part) {
...
static PedDiskType const *_GL_ATTRIBUTE_PURE find_disk_type(char const *name) {
  PedDiskType const *t;
...
int ped_disk_clobber(PedDevice *dev) {
  ...
  /* Clear the last few.  */
  PedSector t = (dev->length - (n_sectors < dev->length ? n_sectors : 1));
...
int ped_disk_set_flag(PedDisk *disk, PedDiskFlag flag, int state) {
  ...
  if (!ped_disk_is_flag_available(disk, flag)) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                        ...
                        ped_disk_flag_get_name(flag), disk->type->name);
...
PedDiskFlag ped_disk_flag_get_by_name(const char *name) {
  ...
  for (flag = ped_disk_flag_next(0); flag; flag = ped_disk_flag_next(flag)) {
    ...
    if (strcasecmp(name, flag_name) == 0 || strcasecmp(name, _(flag_name)) == 0)
      return flag;
  }
...
static int _assert_disk_uuid_feature(const PedDiskType *disk_type) {
  if (!ped_disk_type_check_feature(disk_type, PED_DISK_TYPE_DISK_UUID)) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
 ...
 * function.
 */
static int _partition_align(PedPartition *part,
...
static int _partition_enumerate(PedPartition *part) {
  const PedDiskType *disk_type;
 ...
 */
static int ped_disk_enumerate_partitions(PedDisk *disk) {
  PedPartition *walk;
  int i;
  int end;
...
static int _disk_remove_metadata(PedDisk *disk) {
...
static int _disk_alloc_metadata(PedDisk *disk) {
  PED_ASSERT(disk != NULL);
...
static int _disk_remove_freespace(PedDisk *disk) {
  PedPartition *walk;
  PedPartition *next;
...
static int _alloc_extended_freespace(PedDisk *disk) {
  PedSector last_end;
  PedPartition *walk;
  PedPartition *last;
  ...
  for (walk = extended_part->part_list; walk; walk = walk->next) {
    if (walk->geom.start > last_end + 1) {
      free_space = ped_partition_new(
          disk, PED_PARTITION_FREESPACE | PED_PARTITION_LOGICAL, NULL,
  ...
  if (last_end < extended_part->geom.end) {
    free_space =
        ped_partition_new(disk, PED_PARTITION_FREESPACE | PED_PARTITION_LOGICAL,
                          NULL, last_end + 1, extended_part->geom.end);
...
static int _disk_alloc_freespace(PedDisk *disk) {
  PedSector last_end;
  PedPartition *walk;
  PedPartition *last;
  PedPartition *free_space;
  ...
  for (walk = disk->part_list; walk; walk = walk->next) {
    if (walk->geom.start > last_end + 1) {
      free_space = ped_partition_new(disk, PED_PARTITION_FREESPACE, NULL,
                                     last_end + 1, walk->geom.start - 1);
      _disk_raw_insert_before(disk, walk, free_space);
    }
...
PedPartition *_ped_partition_alloc(const PedDisk *disk, PedPartitionType type,
...
void _ped_partition_free(PedPartition *part) { free(part); }
 ...
 */
PedPartition *ped_partition_new(const PedDisk *disk, PedPartitionType type,
                                const PedFileSystemType *fs_type,
                                PedSector start, PedSector end) {
  ...
  if (!supports_extended &&
      (type == PED_PARTITION_EXTENDED || type == PED_PARTITION_LOGICAL)) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
...
static int _assert_partition_name_feature(const PedDiskType *disk_type) {
  if (!ped_disk_type_check_feature(disk_type, PED_DISK_TYPE_PARTITION_NAME)) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
...
static int _assert_partition_type_id_feature(const PedDiskType *disk_type) {
  if (!ped_disk_type_check_feature(disk_type,
...
static int _assert_partition_type_uuid_feature(const PedDiskType *disk_type) {
  if (!ped_disk_type_check_feature(disk_type,
...
static int _assert_partition_uuid_feature(const PedDiskType *disk_type) {
  if (!ped_disk_type_check_feature(disk_type, PED_DISK_TYPE_PARTITION_UUID)) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
...
/**
 * Set the type-id of the partition \p part. This will only work if the disk
 * label supports it.
 */
...
/**
 * Get the type-id of the partition \p part. This will only work if the disk
 * label supports it.
 */
...
/**
 * Set the type-uuid of the partition \p part. This will only work if the disk
 * label supports it.
 */
...
/**
 * Get the type-uuid of the partition \p part. This will only work if the disk
 * label supports it.
 */
...
#ifdef DEBUG
static int _GL_ATTRIBUTE_PURE _disk_check_sanity(PedDisk *disk) {
  PedPartition *walk;
...
static int _disk_raw_remove(PedDisk *disk, PedPartition *part) {
  ...
  if (part->prev) {
    ...
    if (part->type & PED_PARTITION_LOGICAL) {
      ped_disk_extended_partition(disk)->part_list = part->next;
    } else {
...
static int _disk_raw_add(PedDisk *disk, PedPartition *part) {
  ...
  last = NULL;
  walk = (part->type & PED_PARTITION_LOGICAL) ? ext_part->part_list
                                              : disk->part_list;
...
static PedConstraint *_partition_get_overlap_constraint(PedPartition *part,
                                                        PedGeometry *geom) {
  PedSector min_start;
  PedSector max_end;
  PedPartition *walk;
  ...
  while (walk != NULL &&
         (walk->geom.start < geom->start || min_start >= walk->geom.start)) {
    if (walk != part)
  ...
  ped_geometry_init(&free_space, part->disk->dev, min_start,
                    max_end - min_start + 1);
  return ped_constraint_new_from_max(&free_space);
}

static int _partition_check_basic_sanity(PedDisk *disk, PedPartition *part) {
  ...
  if (ped_partition_is_active(part) && !(part->type & PED_PARTITION_LOGICAL)) {
...
static int _check_extended_partition(PedDisk *disk, PedPartition *part) {
  PedPartition *walk;
  PedPartition *ext_part;
...
static int _check_partition(PedDisk *disk, PedPartition *part) {
  PedPartition *ext_part = ped_disk_extended_partition(disk);
  ...
  if (!(part->type & PED_PARTITION_LOGICAL) && ext_part && ext_part != part &&
      ped_geometry_test_inside(&ext_part->geom, &part->geom)) {
...
int ped_disk_add_partition(PedDisk *disk, PedPartition *part,
  ...
  if (ped_partition_is_active(part)) {
    overlap_constraint = _partition_get_overlap_constraint(part, &part->geom);
...
static int ped_disk_delete_all_logical(PedDisk *disk);
...
int ped_disk_set_partition_geom(PedDisk *disk, PedPartition *part,
  ...
  overlap_constraint = _partition_get_overlap_constraint(part, &new_geom);
  constraints = ped_constraint_intersect(overlap_constraint, constraint);
  if (!constraints && constraint) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                        _("Can't have overlapping partitions."));
...
int ped_disk_maximize_partition(PedDisk *disk, PedPartition *part,
  ...
  if (!ped_disk_set_partition_geom(disk, part, constraint, new_start, new_end))
    goto error;
...
error:
  ...
  ped_disk_set_partition_geom(disk, part, constraint_any, old_geom.start,
                              old_geom.end);
...
ped_disk_get_max_partition_geometry(PedDisk *disk, PedPartition *part,
  ...
  ped_disk_set_partition_geom(disk, part, constraint_exact, old_geom.start,
                              old_geom.end);
...
const char *ped_partition_flag_get_name(PedPartitionFlag flag) {
  ...
  default:
    ped_exception_throw(PED_EXCEPTION_BUG, PED_EXCEPTION_CANCEL,
                        _("Unknown partition flag, %d."), flag);
    return NULL;
  }
}
...
static void ped_partition_print(const PedPartition *part) {
  PED_ASSERT(part != NULL);

  printf("  %-10s %02d  (%d->%d)\n", ped_partition_type_get_name(part->type),
         part->num, (int)part->geom.start, (int)part->geom.end);
}
...
void ped_disk_print(const PedDisk *disk) {
  ...
  for (part = disk->part_list; part; part = ped_disk_next_partition(disk, part))
