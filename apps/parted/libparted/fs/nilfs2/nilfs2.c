...
static int is_valid_nilfs_sb(struct nilfs2_super_block *sb) {
    ...
    crc =
        __efi_crc32((unsigned char *)sb + sumoff + 4, bytes - sumoff - 4, crc);
...
PedGeometry *nilfs2_probe(PedGeometry *geom) {
    ...
    PedSector sb2off =
        NILFS_SB2_OFFSET(length) / (geom->dev->sector_size / 512);
    if (sb2off <= 2)
        return NULL;
    const int sectors =
    ...
    const int sectors2 =
        (1024 + geom->dev->sector_size - 1) / geom->dev->sector_size;
    ...
    if ((!sb || !is_valid_nilfs_sb(sb)) && (!sb2 || !is_valid_nilfs_sb(sb2)))
        return NULL;

    /* reserve 4k bytes for secondary superblock */
    length =
        sb2off + ((4096 + geom->dev->sector_size - 1) / geom->dev->sector_size);
