 ...
 */
static int fat_construct_directory(FatOpContext *ctx,
  ...
  while ((dir_entry = fat_traverse_next_dir_entry(trav_info))) {
    ...
    old_first_cluster = fat_dir_entry_get_first_cluster(dir_entry, ctx->old_fs);
    fat_dir_entry_set_first_cluster(
        dir_entry, ctx->new_fs,
        fat_op_context_map_cluster(ctx, old_first_cluster));

    if (fat_dir_entry_is_directory(dir_entry) && dir_entry->name[0] != '.') {
      sub_dir_info = fat_traverse_directory(trav_info, dir_entry);
      if (!sub_dir_info)
        return 0;
...
static int duplicate_legacy_root_dir(FatOpContext *ctx) {
...
*/
static int fat_construct_legacy_root(FatOpContext *ctx) {
  FatTraverseInfo *trav_info;
...
*/
static int fat_construct_root(FatOpContext *ctx) {
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  FatTraverseInfo *trav_info;
 ...
 * so we save room)
 */
static int fat_convert_directory(FatOpContext *ctx, FatTraverseInfo *old_trav,
  ...
  while ((old_dir_entry = fat_traverse_next_dir_entry(old_trav))) {
    ...
    *new_dir_entry = *old_dir_entry;
    ...
    if (!fat_dir_entry_has_first_cluster(old_dir_entry, ctx->old_fs))
      continue;

    old_first_cluster =
        fat_dir_entry_get_first_cluster(old_dir_entry, ctx->old_fs);
    fat_dir_entry_set_first_cluster(
        new_dir_entry, ctx->new_fs,
        fat_op_context_map_cluster(ctx, old_first_cluster));

    if (fat_dir_entry_is_directory(old_dir_entry) &&
        old_dir_entry->name[0] != '.') {
      sub_old_dir_trav = fat_traverse_directory(old_trav, old_dir_entry);
      if (!sub_old_dir_trav)
        return 0;
      sub_new_dir_trav = fat_traverse_directory(new_trav, new_dir_entry);
      if (!sub_new_dir_trav) {
...
static void clear_cluster(PedFileSystem *fs, FatCluster cluster) {
 ...
 */
static int alloc_root_dir(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  FatCluster i;
  FatCluster cluster;
  ...
  cluster_count =
      ped_div_round_up(PED_MAX(16, old_fs_info->root_dir_sector_count),
                       new_fs_info->cluster_sectors);

  for (i = 0; i < cluster_count; i++) {
    cluster = fat_table_alloc_check_cluster(new_fs_info->fat, ctx->new_fs);
    if (!cluster)
      return 0;
 ...
 */
static int free_root_dir(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  ...
  for (old_cluster = old_fs_info->root_cluster;
    ...
    for (i = 0; i < new_fs_info->cluster_frags; i++) {
      ...
      FatCluster new_clst;
      new_frag = fat_op_context_map_fragment(ctx, old_frag + i);
...
static int fat_clear_root_dir(PedFileSystem *fs) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  int i;
...
static int fat_construct_converted_tree(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  ...
  if (new_fs_info->fat_type == FAT_TYPE_FAT32) {
    new_trav_info =
        fat_traverse_begin(ctx->new_fs, new_fs_info->root_cluster, "\\");
    if (!new_trav_info)
      return 0;
    ...
    if (!new_trav_info)
      return 0;
    old_trav_info =
        fat_traverse_begin(ctx->old_fs, old_fs_info->root_cluster, "\\");
  }
  if (!old_trav_info) {
...
*/
static int fat_construct_dir_tree(FatOpContext *ctx) {
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
...
static FatFragment _get_next_old_frag(FatOpContext *ctx, FatFragment frag) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatCluster cluster;
...
*/
static int fat_construct_new_fat(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  FatFragment old_frag;
  ...
  for (old_frag = 0; old_frag < old_fs_info->frag_count; old_frag++) {
    ...
    if (flag == FAT_FLAG_BAD) {
      new_frag = fat_op_context_map_static_fragment(ctx, old_frag);
      if (new_frag == -1)
        continue;
    ...
    new_next_frag = fat_op_context_map_fragment(ctx, old_next_frag);
    PED_ASSERT(new_next_frag != -1);

    new_next_cluster = fat_frag_to_cluster(ctx->new_fs, new_next_frag);
  ...
  if (old_fs_info->fat_type == FAT_TYPE_FAT32 &&
    ...
    new_fs_info->root_cluster =
        fat_op_context_map_cluster(ctx, old_fs_info->root_cluster);
  }

  if (old_fs_info->fat_type == FAT_TYPE_FAT16 &&
      new_fs_info->fat_type == FAT_TYPE_FAT32) {
    for (i = 0; ctx->new_root_dir[i + 1]; i++) {
      fat_table_set(new_fs_info->fat, ctx->new_root_dir[i],
...
static int ask_type(PedFileSystem *fs, int fat16_ok, int fat32_ok,
  ...
  if (fat16_ok && fat32_ok) {
    status = ped_exception_throw(
        PED_EXCEPTION_INFORMATION, PED_EXCEPTION_YES_NO_CANCEL, _("%s  %s  %s"),
        _("Would you like to use FAT32?"), fat16_msg, fat32_msg);
  ...
  if (fat16_ok) {
    if (fs_info->fat_type != FAT_TYPE_FAT16) {
      status = ped_exception_throw(
          PED_EXCEPTION_WARNING, PED_EXCEPTION_OK_CANCEL, _("%s  %s"),
 ...
 */
static int get_fat_type(PedFileSystem *fs, const PedGeometry *new_geom,
                        FatType *out_fat_type) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  ...
  fat16_ok = fat_calc_resize_sizes(
      new_geom, fs_info->cluster_sectors, FAT_TYPE_FAT16,
      fs_info->root_dir_sector_count, fs_info->cluster_sectors,
      &fat16_cluster_sectors, &dummy_cluster_count, &dummy_fat_sectors);

  fat32_ok = fat_calc_resize_sizes(
      new_geom, fs_info->cluster_sectors, FAT_TYPE_FAT32,
      fs_info->root_dir_sector_count, fs_info->cluster_sectors,
      &fat32_cluster_sectors, &dummy_cluster_count, &dummy_fat_sectors);
 ...
 */
static FatOpContext *create_resize_context(PedFileSystem *fs,
                                           const PedGeometry *new_geom) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  ...
  if (fs_info->root_dir_sector_count)
    ...
    root_dir_sector_count =
        FAT_ROOT_DIR_ENTRY_COUNT * sizeof(FatDirEntry) / 512;
  ...
  fat_calc_resize_sizes(new_geom, fs_info->cluster_sectors, new_fat_type,
                        ...
                        &new_cluster_sectors, &new_cluster_count,
                        &new_fat_sectors);
  ...
  if (new_fs_info->fat_type == FAT_TYPE_FAT32) {
     ...
     */
    new_fs_info->fat_offset = fat_min_reserved_sector_count(FAT_TYPE_FAT32) +
                              fat_calc_align_sectors(new_fs, fs);

    new_fs_info->cluster_offset =
        new_fs_info->fat_offset + 2 * new_fs_info->fat_sectors;
  } else {
    ...
    new_fs_info->fat_offset = fat_min_reserved_sector_count(FAT_TYPE_FAT16) +
                              fat_calc_align_sectors(new_fs, fs);
    ...
    new_fs_info->cluster_offset =
        new_fs_info->root_dir_offset + new_fs_info->root_dir_sector_count;
  }
...
static int resize_context_assimilate(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  ...
  ctx->old_fs->type =
      (new_fs_info->fat_type == FAT_TYPE_FAT16) ? &fat16_type : &fat32_type;
...
static int resize_context_abort(FatOpContext *ctx) {
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
 ...
 */
int _copy_hidden_sectors(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  PedSector first = 1;
  PedSector last;
  ...
  if (!ped_geometry_read(ctx->old_fs->geom, old_fs_info->buffer, first, count))
    return 0;
  if (!ped_geometry_write(ctx->new_fs->geom, old_fs_info->buffer, first, count))
    return 0;
  return 1;
}
