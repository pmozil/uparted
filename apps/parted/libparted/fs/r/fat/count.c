...
static int flag_traverse_fat(PedFileSystem *fs, const char *chain_name,
    ...
    if (size &&
        ...
        if (ped_exception_throw(PED_EXCEPTION_ERROR,
                                ...
                                (int)chain_length * fs_info->cluster_sectors /
                                    2) != PED_EXCEPTION_IGNORE)
            return 0;
    }

    last_cluster_usage = ped_div_round_up(
        64 * remainder_round_up(size, fs_info->cluster_sectors),
...
static int flag_traverse_dir(FatTraverseInfo *trav_info) {
    ...
    while ((this_entry = fat_traverse_next_dir_entry(trav_info))) {
        ...
        if (fat_dir_entry_is_directory(this_entry)) {
            ...
            if (!flag_traverse_fat(fs, file_name, first_cluster, FAT_FLAG_FILE,
                                   size))
                return 0;
        }
    }
