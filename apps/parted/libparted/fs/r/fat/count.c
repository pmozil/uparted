...
*/
#ifdef PED_VERBOSE
static void print_chain(PedFileSystem *fs, FatCluster start) {
...
static PedSector remainder_round_up(PedSector a, PedSector b) {
  PedSector result;
...
/*
    ...
    to "flag".
*/
static int flag_traverse_fat(PedFileSystem *fs, const char *chain_name,
                             FatCluster start, FatClusterFlag flag,
                             PedSector size) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  FatCluster clst;
  ...
  if (fat_table_is_eof(fs_info->fat, start)) {
    if (ped_exception_throw(
            ...
            _("Bad directory entry for %s: first cluster is the "
              "end of file marker."),
            chain_name) != PED_EXCEPTION_IGNORE)
      return 0;
  }
  ...
  last_cluster_usage =
      ped_div_round_up(64 * remainder_round_up(size, fs_info->cluster_sectors),
...
*/
static int flag_traverse_dir(FatTraverseInfo *trav_info) {
  ...
  while ((this_entry = fat_traverse_next_dir_entry(trav_info))) {
    ...
    size = ped_div_round_up(fat_dir_entry_get_length(this_entry), 512);
    ...
    if (fat_dir_entry_is_directory(this_entry)) {
      if (!flag_traverse_fat(fs, file_name, first_cluster, FAT_FLAG_DIRECTORY,
                             size))
        return 0;

      subdir_trav_info = fat_traverse_directory(trav_info, this_entry);
      if (!subdir_trav_info)
        return 0;
      if (!flag_traverse_dir(subdir_trav_info))
        return 0;
    } else if (fat_dir_entry_is_file(this_entry)) {
      if (!flag_traverse_fat(fs, file_name, first_cluster, FAT_FLAG_FILE, size))
        return 0;
    }
  }
...
static void _mark_bad_clusters(PedFileSystem *fs) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  FatCluster cluster;
...
int fat_collect_cluster_info(PedFileSystem *fs) {
  ...
  if (fs_info->fat_type == FAT_TYPE_FAT32) {
    ...
    if (!flag_traverse_dir(trav_info))
      return 0;
    if (!flag_traverse_fat(fs, "\\", fs_info->root_cluster, FAT_FLAG_DIRECTORY,
                           0))
      return 0;
  } else {
...
FatClusterFlag fat_get_fragment_flag(PedFileSystem *fs, FatFragment frag) {
  ...
  if (flag != FAT_FLAG_FILE && flag != FAT_FLAG_DIRECTORY)
    return flag;
  last_frag_used =
      (fat_get_cluster_usage(fs, cluster) - 1) / fs_info->frag_sectors;
