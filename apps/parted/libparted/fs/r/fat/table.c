...
int fat_table_read(FatTable *ft, const PedFileSystem *fs, int table_num) {
  ...
  if (!ped_geometry_read(fs->geom, (void *)ft->table,
                         fs_info->fat_offset + table_num * fs_info->fat_sectors,
                         fs_info->fat_sectors))
    return 0;
  ...
  if (*((unsigned char *)ft->table) != fs_info->boot_sector->media) {
    if (ped_exception_throw(
            ...
            (int)table_num + 1, (int)*((unsigned char *)ft->table),
            (int)fs_info->boot_sector->media) != PED_EXCEPTION_IGNORE)
      return 0;
  }
...
static int _test_code_available(const FatTable *ft, FatCluster code) {
  return code == 0;
}

static int _test_code_bad(const FatTable *ft, FatCluster code) {
  switch (ft->fat_type) {
  case FAT_TYPE_FAT12:
    if (code == 0xff7)
      return 1;
...
static int _test_code_eof(const FatTable *ft, FatCluster code) {
  switch (ft->fat_type) {
  case FAT_TYPE_FAT12:
    if (code >= 0xff7)
      return 1;
...
void _update_stats(FatTable *ft, FatCluster cluster, FatCluster value) {
  if (_test_code_available(ft, value) && !fat_table_is_available(ft, cluster)) {
    ft->free_cluster_count++;
...
int fat_table_set(FatTable *ft, FatCluster cluster, FatCluster value) {
  ...
  case FAT_TYPE_FAT16:
    ((unsigned short *)ft->table)[cluster] = PED_CPU_TO_LE16(value);
    break;

  case FAT_TYPE_FAT32:
    ((unsigned int *)ft->table)[cluster] = PED_CPU_TO_LE32(value);
    break;
  }
  return 1;
}
...
FatCluster fat_table_get(const FatTable *ft, FatCluster cluster) {
  ...
  case FAT_TYPE_FAT16:
    return PED_LE16_TO_CPU(((unsigned short *)ft->table)[cluster]);

  case FAT_TYPE_FAT32:
    return PED_LE32_TO_CPU(((unsigned int *)ft->table)[cluster]);
  }
...
*/
int _GL_ATTRIBUTE_PURE fat_table_is_eof(const FatTable *ft,
                                        FatCluster cluster) {
...
int fat_table_is_empty(const FatTable *ft, FatCluster cluster) {
  return fat_table_is_available(ft, cluster) || fat_table_is_bad(ft, cluster);
}
...
int fat_table_is_active(const FatTable *ft, FatCluster cluster) {
  return !fat_table_is_bad(ft, cluster) && !fat_table_is_available(ft, cluster);
}
