...
static int needs_duplicating(const FatOpContext *ctx, FatFragment frag) {
...
static int search_next_fragment(FatOpContext *ctx) {
...
static int read_marked_fragments(FatOpContext *ctx, FatFragment length) {
  FatSpecific *fs_info = FAT_SPECIFIC(ctx->old_fs);
  int status;
  FatFragment i;

  ped_exception_fetch_all();
  status = fat_read_fragments(ctx->old_fs, fs_info->buffer, ctx->buffer_offset,
                              length);
...
static int fetch_fragments(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  ...
  for (frag = 0; frag < ctx->buffer_frags &&
...
/* finds the last fragment that is not going to get overwritten (that needs to
   get read in) */
static FatFragment _GL_ATTRIBUTE_PURE get_last_underlay(const FatOpContext *ctx,
                                                        int first, int last) {
  int old;
  FatFragment new;
 ...
 */
static int quick_group_write_read_underlay(FatOpContext *ctx, int first,
                                           int last) {
  ...
  if (!fat_read_fragments(ctx->new_fs,
                          ...
                          first_underlay, underlay_length))
    return 0;
  return 1;
}
 ...
 * further on.
 */
static int quick_group_write(FatOpContext *ctx, int first, int last) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  int active_length;
  int i;
 ...
 * is found.
 */
static int slow_group_write(FatOpContext *ctx, int first, int last) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  int i;
  ...
  for (i = first; i <= last; i++) {
    ...
    while (!fat_write_sync_fragment(
        ctx->new_fs, old_fs_info->buffer + i * old_fs_info->frag_size,
        ctx->buffer_map[i])) {
      fat_table_set_bad(new_fs_info->fat, ctx->buffer_map[i]);
      ctx->buffer_map[i] = fat_table_alloc_cluster(new_fs_info->fat);
...
static int update_remap(FatOpContext *ctx, int first, int last) {
  int i;
...
static int group_write(FatOpContext *ctx, int first, int last) {
  PED_ASSERT(first <= last);
...
static int write_fragments(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatSpecific *new_fs_info = FAT_SPECIFIC(ctx->new_fs);
  int group_start;
  ...
  for (i = 0; i < ctx->buffer_frags; i++) {
    ...
    ctx->buffer_map[i] = fat_cluster_to_frag(ctx->new_fs, new_cluster);

    if (group_start == -1)
      group_start = group_end = i;

    PED_ASSERT(ctx->buffer_map[i] >= ctx->buffer_map[group_start]);

    mapped_length = ctx->buffer_map[i] - ctx->buffer_map[group_start] + 1;
 ...
 */
static void init_remap(FatOpContext *ctx) {
  FatSpecific *old_fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatFragment i;
...
static FatFragment count_frags_to_dup(FatOpContext *ctx) {
  FatSpecific *fs_info = FAT_SPECIFIC(ctx->old_fs);
  FatFragment i;
  FatFragment total;
...
int fat_duplicate_clusters(FatOpContext *ctx, PedTimer *timer) {
  ...
  while (search_next_fragment(ctx)) {
    ped_timer_update(timer, 1.0 * ctx->frags_duped / total_frags_to_dup);
