...
int fat_read_fragments(PedFileSystem *fs, char *buf, FatFragment frag,
                       FatFragment count) {
...
int fat_write_fragments(PedFileSystem *fs, char *buf, FatFragment frag,
                        FatFragment count) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  PedSector sector = fat_frag_to_sector(fs, frag);
  PedSector sector_count = count * fs_info->frag_sectors;
...
int fat_write_sync_fragments(PedFileSystem *fs, char *buf, FatFragment frag,
                             FatFragment count) {
...
int fat_read_clusters(PedFileSystem *fs, char *buf, FatCluster cluster,
                      FatCluster count) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  PedSector sector = fat_cluster_to_sector(fs, cluster);
  PedSector sector_count = count * fs_info->cluster_sectors;

  PED_ASSERT(cluster >= 2 && cluster + count - 1 < fs_info->cluster_count + 2);
...
int fat_write_clusters(PedFileSystem *fs, char *buf, FatCluster cluster,
                       FatCluster count) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  PedSector sector = fat_cluster_to_sector(fs, cluster);
  PedSector sector_count = count * fs_info->cluster_sectors;
...
int fat_write_sync_clusters(PedFileSystem *fs, char *buf, FatCluster cluster,
                            FatCluster count) {
