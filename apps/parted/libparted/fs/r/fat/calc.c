...
static PedSector _GL_ATTRIBUTE_CONST _smallest_power2_over(PedSector ceiling) {
  PedSector result = 1;
...
PedSector _GL_ATTRIBUTE_CONST fat_recommend_min_cluster_size(FatType fat_type,
  ...
  case FAT_TYPE_FAT12:
    return 1;
  case FAT_TYPE_FAT16:
    return fat_min_cluster_size(fat_type);
  case FAT_TYPE_FAT32:
    return PED_MAX(_smallest_power2_over(size / MAX_FAT32_CLUSTERS),
...
int fat_check_resize_geometry(const PedFileSystem *fs, const PedGeometry *geom,
  ...
  free_space = fs_info->fat->free_cluster_count * fs_info->cluster_sectors;
  ...
  PED_ASSERT(new_cluster_count <= fat_max_cluster_count(FAT_TYPE_FAT32));
...
static int entries_per_sector(FatType fat_type) {
  switch (fat_type) {
  case FAT_TYPE_FAT12:
    return 512 * 3 / 2;
  case FAT_TYPE_FAT16:
...
static int calc_sizes(PedSector size, PedSector align, FatType fat_type,
  ...
  for (i = 0; i < 2; i++) {
    ...
    fat_space =
        ped_div_round_up(cluster_count + 2, entries_per_sector(fat_type));
  }
...
int fat_calc_sizes(PedSector size, PedSector align, FatType fat_type,
                   PedSector root_dir_sectors, PedSector *out_cluster_sectors,
                   FatCluster *out_cluster_count, PedSector *out_fat_size) {
  ...
  for (cluster_sectors = fat_recommend_min_cluster_size(fat_type, size);
       ...
       cluster_sectors *= 2) {
    if (calc_sizes(size, align, fat_type, root_dir_sectors, cluster_sectors,
 ...
 * cluster size.
 */
int fat_calc_resize_sizes(const PedGeometry *geom, PedSector align,
                          FatType fat_type, PedSector root_dir_sectors,
                          PedSector cluster_sectors,
 ...
 */
PedSector _GL_ATTRIBUTE_PURE fat_calc_align_sectors(
    const PedFileSystem *new_fs, const PedFileSystem *old_fs) {
  ...
  new_meta_data_size = fat_min_reserved_sector_count(new_fs_info->fat_type) +
                       new_fs_info->fat_sectors * 2;

  if (new_fs_info->fat_type == FAT_TYPE_FAT16)
    new_meta_data_size += new_fs_info->root_dir_sector_count;

  raw_old_meta_data_end = old_fs->geom->start + old_fs_info->cluster_offset;
  ...
  new_clusters_size = new_fs_info->cluster_count * new_fs_info->cluster_sectors;
...
PedSector _GL_ATTRIBUTE_PURE fat_cluster_to_sector(const PedFileSystem *fs,
  ...
  return (cluster - 2) * fs_info->cluster_sectors + fs_info->cluster_offset;
}
...
FatCluster _GL_ATTRIBUTE_PURE fat_sector_to_cluster(const PedFileSystem *fs,
  ...
  return (sector - fs_info->cluster_offset) / fs_info->cluster_sectors + 2;
}
#endif /* !DISCOVER_ONLY */
