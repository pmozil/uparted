 ...
 * works ;-)
 */
static int calc_deltas(FatOpContext *ctx) {
  ...
  if (sector_delta % new_fs_info->cluster_sectors) {
    ped_exception_throw(PED_EXCEPTION_BUG, PED_EXCEPTION_CANCEL,
                        ...
                        (int)sector_delta, (int)new_fs_info->cluster_sectors);
    return 0;
  }
  ...
  printf("Start move delta is: %d %s.\n", (int)ctx->start_move_delta,
         (ctx->start_move_dir == FAT_DIR_FORWARD) ? "forwards" : "backwards");
#endif
...
FatOpContext *fat_op_context_new(PedFileSystem *new_fs, PedFileSystem *old_fs) {
  ...
  ctx->frag_sectors =
      PED_MIN(old_fs_info->cluster_sectors, new_fs_info->cluster_sectors);
  ...
  ctx->buffer_map =
      (FatFragment *)ped_malloc(sizeof(FatFragment) * ctx->buffer_frags);
  if (!ctx->buffer_map)
    goto error_free_ctx;

  ctx->remap =
      (FatFragment *)ped_malloc(sizeof(FatFragment) * old_fs_info->frag_count);
  if (!ctx->remap)
...
FatCluster fat_op_context_map_static_cluster(const FatOpContext *ctx,
  ...
  mapped_frag = fat_op_context_map_static_fragment(
      ctx, fat_cluster_to_frag(ctx->old_fs, clst));
  if (mapped_frag != -1)
...
FatCluster fat_op_context_map_cluster(const FatOpContext *ctx,
  ...
  mapped_frag =
      fat_op_context_map_fragment(ctx, fat_cluster_to_frag(ctx->old_fs, clst));
  if (mapped_frag != -1)
    return fat_frag_to_cluster(ctx->new_fs, mapped_frag);
  else
    return 0;
...
int fat_op_context_create_initial_fat(FatOpContext *ctx) {
  ...
  new_fs_info->fat = fat_table_new(
      new_fs_info->fat_type, new_fs_info->fat_sectors * 512 /
  ...
  for (sect = 0; sect < old_fs_info->cluster_offset; sect++) {
    new_sect = ped_geometry_map(ctx->new_fs->geom, ctx->old_fs->geom, sect);
    if (new_sect == -1 || !fat_is_sector_in_clusters(ctx->new_fs, new_sect))
      continue;
