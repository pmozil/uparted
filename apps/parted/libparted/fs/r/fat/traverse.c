 ...
 */
static int is_last_buffer(FatTraverseInfo *trav_info) {
...
static int write_root_dir(FatTraverseInfo *trav_info) {
  FatSpecific *fs_info = FAT_SPECIFIC(trav_info->fs);
...
static int write_dir_cluster(FatTraverseInfo *trav_info) {
  if (!fat_write_sync_cluster(trav_info->fs, (void *)trav_info->dir_entries,
...
static int write_dir_buffer(FatTraverseInfo *trav_info) {
  if (trav_info->is_legacy_root_dir)
...
static int read_next_dir_buffer(FatTraverseInfo *trav_info) {
  FatSpecific *fs_info = FAT_SPECIFIC(trav_info->fs);
  ...
  if (trav_info->this_buffer < 2 ||
    ...
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                        ...
                        (long)trav_info->this_buffer, trav_info->dir_name);
    return 0;
  }

  trav_info->next_buffer = fat_table_get(fs_info->fat, trav_info->this_buffer);
...
FatTraverseInfo *fat_traverse_begin(PedFileSystem *fs, FatCluster start_cluster,
                                    const char *dir_name) {
  ...
  trav_info->dir_entries = (FatDirEntry *)ped_malloc(trav_info->buffer_size);
...
FatTraverseInfo *fat_traverse_directory(FatTraverseInfo *trav_info,
  ...
  fat_dir_entry_get_name(parent, tmp_buffer + strlen(trav_info->dir_name));
  strcat(tmp_buffer, "\\");

  return fat_traverse_begin(
      trav_info->fs, fat_dir_entry_get_first_cluster(parent, trav_info->fs),
      tmp_buffer);
}
...
FatDirEntry *fat_traverse_next_dir_entry(FatTraverseInfo *trav_info) {
  ...
  if (trav_info->current_entry >= fat_traverse_entries_per_buffer(trav_info)) {
    if (trav_info->dirty) {
      if (!write_dir_buffer(trav_info))
        return NULL;
    }
...
fat_dir_entry_get_first_cluster(FatDirEntry *dir_entry, PedFileSystem *fs) {
  ...
  case FAT_TYPE_FAT32:
    return PED_LE16_TO_CPU(dir_entry->first_cluster_high) * 65536L +
...
void fat_dir_entry_set_first_cluster(FatDirEntry *dir_entry, PedFileSystem *fs,
                                     FatCluster cluster) {
  FatSpecific *fs_info = FAT_SPECIFIC(fs);
  ...
  case FAT_TYPE_FAT32:
    dir_entry->first_cluster = PED_CPU_TO_LE16(cluster & 0xffff);
    dir_entry->first_cluster_high = PED_CPU_TO_LE16(cluster / 0x10000);
    break;
  }
}
...
int _GL_ATTRIBUTE_PURE fat_dir_entry_is_active(FatDirEntry *dir_entry) {
  if ((unsigned char)dir_entry->name[0] == DELETED_FLAG)
    return 0;
  if ((unsigned char)dir_entry->name[0] == 0)
    return 0;
  if ((unsigned char)dir_entry->name[0] == 0xF6)
    return 0;
  return 1;
}

int _GL_ATTRIBUTE_PURE fat_dir_entry_is_file(FatDirEntry *dir_entry) {
  if (dir_entry->attributes == VFAT_ATTR)
    return 0;
  if (dir_entry->attributes & VOLUME_LABEL_ATTR)
    return 0;
  if (!fat_dir_entry_is_active(dir_entry))
    return 0;
  if ((dir_entry->attributes & DIRECTORY_ATTR) == DIRECTORY_ATTR)
    return 0;
  return 1;
}
...
int fat_dir_entry_has_first_cluster(FatDirEntry *dir_entry, PedFileSystem *fs) {
  ...
  if (first_cluster == 0 || fat_table_is_eof(fs_info->fat, first_cluster))
    return 0;
