...
static HfsCPrivateCacheTable *hfsc_new_cachetable(unsigned int size) {
...
HfsCPrivateCache *hfsc_new_cache(unsigned int block_number,
  ...
  ret->linked_ref_size =
      block_number > block_number + ((1 << CR_SHIFT) - 1)
          ? (block_number >> CR_SHIFT) + 1
          : (block_number + ((1 << CR_SHIFT) - 1)) >> CR_SHIFT;

  ret->linked_ref = (HfsCPrivateExtent **)ped_malloc(sizeof(*ret->linked_ref) *
                                                     ret->linked_ref_size);
  if (!ret->linked_ref) {
    free(ret);
    return NULL;
  ...
  if (cachetable_size < file_number)
    cachetable_size = (unsigned)-1;
...
static void hfsc_delete_cachetable(HfsCPrivateCacheTable *list) {
...
HfsCPrivateExtent *hfsc_cache_add_extent(HfsCPrivateCache *cache,
  ...
  for (ext = cache->linked_ref[idx]; ext && start != ext->ext_start;
       ext = ext->next)
    ;
  ...
  if (cache->last_table->table_first_free == cache->last_table->table_size) {
    cache->last_table->next_cache = hfsc_new_cachetable(
        (cache->first_cachetable_size / CR_NEW_ALLOC_DIV) + CR_ADD_CST);
  ...
  cache->needed_alloc_size =
      cache->needed_alloc_size > (unsigned)PED_SECTOR_SIZE_DEFAULT * sbb
          ? cache->needed_alloc_size
...
hfsc_cache_search_extent(HfsCPrivateCache *cache, uint32_t start) {
  ...
  for (ret = cache->linked_ref[idx]; ret && start != ret->ext_start;
       ret = ret->next)
    ;
...
HfsCPrivateExtent *hfsc_cache_move_extent(HfsCPrivateCache *cache,
  ...
  for (pext = cache->linked_ref[idx2]; pext && new_start != pext->ext_start;
       pext = pext->next)
    ;
  ...
  for (ppext = &(cache->linked_ref[idx1]);
       (*ppext) && old_start != (*ppext)->ext_start; ppext = &((*ppext)->next))
    ;
