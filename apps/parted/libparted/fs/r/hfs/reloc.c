...
static int hfs_effect_move_extent(PedFileSystem *fs, unsigned int *ptr_fblock,
  ...
  */
  ...
  if (ok) {
    /* enough room */
    unsigned int j;
    unsigned int start_block = PED_BE16_TO_CPU(priv_data->mdb->start_block);
    unsigned int block_sz =
        (PED_BE32_TO_CPU(priv_data->mdb->block_size) / PED_SECTOR_SIZE_DEFAULT);
    ...
    for (i = 0; i < size; /*i+=j*/) {
      ...
      j = size - i;
      j = (j < hfs_block_count) ? j : hfs_block_count;

      abs_sector = start_block + (PedSector)(*ptr_fblock + i) * block_sz;
      if (!ped_geometry_read(fs->geom, hfs_block, abs_sector, block_sz * j))
        return -1;

      abs_sector = start_block + (PedSector)(start + i) * block_sz;
      if (!ped_geometry_write(fs->geom, hfs_block, abs_sector, block_sz * j))
        return -1;

      for (ai = i + j; i < ai; i++) {
        /* free source block */
        CLR_BLOC_OCCUPATION(priv_data->alloc_map, *ptr_fblock + i);

        /* set dest block */
        SET_BLOC_OCCUPATION(priv_data->alloc_map, start + i);
      }
    }
...
static int hfs_do_move(PedFileSystem *fs, unsigned int *ptr_src,
  ...
  new_start = hfs_effect_move_extent(fs, ptr_src, ptr_dest, ref->ext_length);
  if (new_start == -1)
    return -1;
  ...
  if (ref->ext_start != (unsigned)new_start) {
    ...
    case CR_PRIM_CAT:
      ...
      goto CR_PRIM;
    case CR_PRIM_EXT:
      priv_data->extent_file->first[ref->ref_index].start_block =
          PED_CPU_TO_BE16(new_start);
    CR_PRIM:
      extent =
          (HfsExtDescriptor *)((uint8_t *)priv_data->mdb + ref->ref_offset);
      extent[ref->ref_index].start_block = PED_CPU_TO_BE16(new_start);
...
/* 0 error, 1 ok */
static int hfs_save_allocation(PedFileSystem *fs) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
  ...
  return (ped_geometry_write(
      fs->geom, priv_data->alloc_map,
      PED_BE16_TO_CPU(priv_data->mdb->volume_bitmap_block), map_sectors));
}
...
static int hfs_move_extent_starting_at(PedFileSystem *fs,
...
static int hfs_cache_from_mdb(HfsCPrivateCache *cache, PedFileSystem *fs,
                              PedTimer *timer) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
  HfsExtDescriptor *extent;
  unsigned int j;
  ...
  for (j = 0; j < HFS_EXT_NB; ++j) {
    ...
    if (!hfsc_cache_add_extent(
            cache, PED_BE16_TO_CPU(extent[j].start_block),
            PED_BE16_TO_CPU(extent[j].block_count), 0, /* unused for mdb */
  ...
  for (j = 0; j < HFS_EXT_NB; ++j) {
    if (!extent[j].block_count)
      break;
    if (!hfsc_cache_add_extent(cache, PED_BE16_TO_CPU(extent[j].start_block),
                               PED_BE16_TO_CPU(extent[j].block_count), 0,
                               ((uint8_t *)extent) -
                                   ((uint8_t *)priv_data->mdb),
                               1, CR_PRIM_CAT, j))
      return 0;
  }
...
static int hfs_cache_from_catalog(HfsCPrivateCache *cache, PedFileSystem *fs,
                                  PedTimer *timer) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
  uint8_t node[PED_SECTOR_SIZE_DEFAULT];
  ...
  for (leaf_node = PED_BE32_TO_CPU(header->first_leaf_node); leaf_node;
       leaf_node = PED_BE32_TO_CPU(desc->next)) {
    if (!hfs_file_read_sector(priv_data->catalog_file, node, leaf_node))
      return 0;
    record_number = PED_BE16_TO_CPU(desc->rec_nb);
    for (i = 1; i <= record_number; ++i) {
      /* undocumented alignement */
      uint16_t value;
      memcpy(&value, node + (PED_SECTOR_SIZE_DEFAULT - (2 * i)),
             sizeof(uint16_t));
      ...
      if ((catalog_pos < HFS_FIRST_REC) ||
          ((uint8_t *)catalog_data - node >=
           PED_SECTOR_SIZE_DEFAULT - 2 * (signed)(record_number + 1))) {
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                            _("The file system contains errors."));
        return 0;
      }

      if (catalog_data->type != HFS_CAT_FILE)
        continue;
      ...
      for (j = 0; j < HFS_EXT_NB; ++j) {
        ...
        if (!hfsc_cache_add_extent(
                cache, PED_BE16_TO_CPU(extent[j].start_block),
                PED_BE16_TO_CPU(extent[j].block_count), leaf_node,
                (uint8_t *)extent - node, 1, /* hfs => btree block = 512 b */
                CR_BTREE_CAT, j))
          return 0;
      }
...
static int hfs_cache_from_extent(HfsCPrivateCache *cache, PedFileSystem *fs,
                                 PedTimer *timer) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
  uint8_t node[PED_SECTOR_SIZE_DEFAULT];
  HfsHeaderRecord *header;
  ...
  for (leaf_node = PED_BE32_TO_CPU(header->first_leaf_node); leaf_node;
       leaf_node = PED_BE32_TO_CPU(desc->next)) {
    if (!hfs_file_read_sector(priv_data->extent_file, node, leaf_node))
      return 0;
    record_number = PED_BE16_TO_CPU(desc->rec_nb);
    for (i = 1; i <= record_number; i++) {
      uint8_t where;
      ...
      case PED_CPU_TO_BE32(HFS_XTENT_ID):
        if (ped_exception_throw(
                ...
                _("The extents overflow file should not"
                  ...
                  "should check the file system.")) != PED_EXCEPTION_IGNORE)
          return 0;
...
static HfsCPrivateCache *hfs_cache_extents(PedFileSystem *fs, PedTimer *timer) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
...
int hfs_pack_free_space_from_block(PedFileSystem *fs, unsigned int fblock,
  ...
  unsigned int divisor =
      PED_BE16_TO_CPU(mdb->total_blocks) + 1 - start - to_free;
  int ret;
  ...
  bytes_buff =
      PED_BE32_TO_CPU(priv_data->mdb->block_size) * (PedSector)BLOCK_MAX_BUFF;
  if (bytes_buff > BYTES_MAX_BUFF) {
    hfs_block_count =
        BYTES_MAX_BUFF / PED_BE32_TO_CPU(priv_data->mdb->block_size);
    if (!hfs_block_count)
      hfs_block_count = 1;
  ...
  while (fblock < PED_BE16_TO_CPU(mdb->total_blocks)) {
    if (TST_BLOC_OCCUPATION(priv_data->alloc_map, fblock) &&
      ...
      if (!(ret = hfs_move_extent_starting_at(fs, &fblock, &to_fblock, cache)))
        to_fblock = ++fblock;
      else if (ret == -1) {
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
