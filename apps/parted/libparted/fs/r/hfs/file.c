...
/* Close an HFS file */
void hfs_file_close(HfsPrivateFile *file) { free(file); }

/* warning : only works on data forks */
static int hfs_get_extent_containing(HfsPrivateFile *file, unsigned int block,
                                     HfsExtDataRec cache,
                                     uint16_t *ptr_start_cache) {
  ...
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)file->fs->type_specific;
  ...
  if (!hfs_btree_search(priv_data->extent_file, (HfsPrivateGenericKey *)&search,
...
/* return 0 on error */
static PedSector hfs_file_find_sector(HfsPrivateFile *file, PedSector sector) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)file->fs->type_specific;
  ...
  for (s = 0, i = 0; i < HFS_EXT_NB; i++) {
    if ((block >= s) &&
        (block < s + PED_BE16_TO_CPU(file->first[i].block_count))) {
      vol_block = (block - s) + PED_BE16_TO_CPU(file->first[i].start_block);
      goto sector_found;
    }
  ...
  if (file->start_cache && block >= file->start_cache)
    for (s = file->start_cache, i = 0; i < HFS_EXT_NB; i++) {
      if ((block >= s) &&
          (block < s + PED_BE16_TO_CPU(file->cache[i].block_count))) {
        vol_block = (block - s) + PED_BE16_TO_CPU(file->cache[i].start_block);
        goto sector_found;
      }
...
sector_found:
  return (PedSector)PED_BE16_TO_CPU(priv_data->mdb->start_block) +
         (PedSector)vol_block * sect_by_block + offset;
}
