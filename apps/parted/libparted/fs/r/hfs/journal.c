...
static uint32_t hfsj_calc_checksum(uint8_t *ptr, int len) {
  int i;
  uint32_t cksum = 0;
...
int hfsj_update_jl(PedFileSystem *fs, uint32_t block) {
  ...
  binsect =
      HFS_32_TO_CPU(priv_data->vh->block_size, is_le) / PED_SECTOR_SIZE_DEFAULT;
...
/* or 0 on error */
static PedSector hfsj_journal_read(PedGeometry *geom, HfsJJournalHeader *jh,
...
static int hfsj_replay_transaction(PedFileSystem *fs, HfsJJournalHeader *jh,
  ...
  blhdr =
      (HfsJBlockListHeader *)ped_malloc(blhdr_nbsect * PED_SECTOR_SIZE_DEFAULT);
  if (!blhdr)
    return 0;

  start = HFS_64_TO_CPU(jh->start, is_le) / PED_SECTOR_SIZE_DEFAULT;
  do {
    start = hfsj_journal_read(priv_data->plus_geom, jh, jsector, jlength, start,
                              blhdr_nbsect, blhdr);
    if (!start)
      goto err_replay;
    ...
    for (i = 1; i < HFS_16_TO_CPU(blhdr->num_blocks, is_le); ++i) {
      ...
      if (!block)
        goto err_replay;
      start = hfsj_journal_read(priv_data->plus_geom, jh, jsector, jlength,
                                start, size / PED_SECTOR_SIZE_DEFAULT, block);
      if (!start) {
        free(block);
        goto err_replay;
      }
      ...
      /* check if wrapper mdb or vh with no wrapper has
         changed */
      if ((sector != ~0LL) && (2 >= sector) &&
          (2 < sector + size / PED_SECTOR_SIZE_DEFAULT))
        hfsj_vh_replayed = 1;
      /* check if vh of embedded hfs+ has changed */
      if ((sector != ~0LL) && (priv_data->plus_geom != fs->geom) &&
          (sector + fs->geom->start - priv_data->plus_geom->start <= 2) &&
          (sector + size / PED_SECTOR_SIZE_DEFAULT + fs->geom->start -
...
int hfsj_replay_journal(PedFileSystem *fs) {
  ...
  if ((jib->flags & PED_CPU_TO_BE32(1 << HFSJ_JOURN_IN_FS)) &&
    ...
    priv_data->jl_start_block =
        HFS_64_TO_CPU(jib->offset, is_le) / (PED_SECTOR_SIZE_DEFAULT * binsect);
  }
  ...
  if (ped_exception_throw(
          ...
          _("The journal is not empty.  Parted must replay the "
            ...
            "modify the file system.")) != PED_EXCEPTION_FIX)
    return 0;
