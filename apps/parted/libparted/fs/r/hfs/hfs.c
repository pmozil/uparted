...
PedFileSystem *hfs_open(PedGeometry *geom) {
  ...
  if (!fs)
    goto ho;
  mdb = (HfsMasterDirectoryBlock *)ped_malloc(sizeof(HfsMasterDirectoryBlock));
  if (!mdb)
    goto ho_fs;
  priv_data = (HfsPrivateFSData *)ped_malloc(sizeof(HfsPrivateFSData));
  if (!priv_data)
    goto ho_mdb;
  ...
  priv_data->extent_file = hfs_file_open(
      fs, PED_CPU_TO_BE32(HFS_XTENT_ID), mdb->extents_file_rec,
      PED_CPU_TO_BE32(mdb->extents_file_size) / PED_SECTOR_SIZE_DEFAULT);
  if (!priv_data->extent_file)
    goto ho_pd;
  priv_data->catalog_file = hfs_file_open(
      fs, PED_CPU_TO_BE32(HFS_CATALOG_ID), mdb->catalog_file_rec,
      PED_CPU_TO_BE32(mdb->catalog_file_size) / PED_SECTOR_SIZE_DEFAULT);
  ...
  fs->checked = (PED_BE16_TO_CPU(mdb->volume_attributes) >> HFS_UNMOUNTED) & 1;
...
PedConstraint *hfs_get_resize_constraint(const PedFileSystem *fs) {
  ...
  return ped_constraint_new(&start_align, ped_alignment_any, &start_sector,
                            &full_dev, min_size, fs->geom->length);
}
...
int hfs_resize(PedFileSystem *fs, PedGeometry *geom, PedTimer *timer) {
  ...
  mdb = priv_data->mdb;
  hfs_sect_block = PED_BE32_TO_CPU(mdb->block_size) / PED_SECTOR_SIZE_DEFAULT;

  if (fs->geom->start != geom->start || geom->length > fs->geom->length ||
      geom->length < hgee + 2) {
  ...
  if (!ped_geometry_write(fs->geom, buf, 2, 1) || !ped_geometry_sync(fs->geom))
    return 0;
  ...
  /* relocate data */
  to_free =
      (fs->geom->length - geom->length + hfs_sect_block - 1) / hfs_sect_block;
  ...
  nblock =
      (geom->length - (PED_BE16_TO_CPU(mdb->start_block) + 2)) / hfs_sect_block;
  ...
  for (block = nblock; block < PED_BE16_TO_CPU(mdb->total_blocks); block++) {
...
int hfsplus_close(PedFileSystem *fs) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
...
PedFileSystem *hfsplus_open(PedGeometry *geom) {
  ...
  if ((wrapper_geom = hfs_and_wrapper_probe(geom))) {
    ...
    if (!priv_data->wrapper)
      goto hpo_gm;
    hfs_priv_data = (HfsPrivateFSData *)priv_data->wrapper->type_specific;
    bs = PED_BE32_TO_CPU(hfs_priv_data->mdb->block_size) /
         PED_SECTOR_SIZE_DEFAULT;
    abs_sect = (PedSector)geom->start +
               (PedSector)PED_BE16_TO_CPU(hfs_priv_data->mdb->start_block) +
               (PedSector)PED_BE16_TO_CPU(
                   hfs_priv_data->mdb->old_new.embedded.location.start_block) *
                   bs;
    length = (PedSector)PED_BE16_TO_CPU(
                 hfs_priv_data->mdb->old_new.embedded.location.block_count) *
             bs;
    priv_data->plus_geom = ped_geometry_new(geom->dev, abs_sect, length);
    if (!priv_data->plus_geom)
      goto hpo_wr;
  ...
  if (vh->signature == PED_CPU_TO_BE16(HFSP_SIGNATURE) &&
    ...
    if (ped_exception_throw(
            PED_EXCEPTION_NO_FEATURE, PED_EXCEPTION_IGNORE_CANCEL,
            _("Version %d of HFS+ isn't supported."),
            PED_BE16_TO_CPU(vh->version)) != PED_EXCEPTION_IGNORE)
      goto hpo_pg;
  }
  ...
  priv_data->bad_blocks_xtent_nb = 0;
  priv_data->bad_blocks_xtent_list = NULL;
  priv_data->extents_file = hfsplus_file_open(
      fs, PED_CPU_TO_BE32(HFS_XTENT_ID), vh->extents_file.extents,
      PED_BE64_TO_CPU(vh->extents_file.logical_size) / PED_SECTOR_SIZE_DEFAULT);
  if (!priv_data->extents_file)
    goto hpo_pg;
  priv_data->catalog_file = hfsplus_file_open(
      fs, PED_CPU_TO_BE32(HFS_CATALOG_ID), vh->catalog_file.extents,
      PED_BE64_TO_CPU(vh->catalog_file.logical_size) / PED_SECTOR_SIZE_DEFAULT);
  if (!priv_data->catalog_file)
    goto hpo_ce;
  priv_data->attributes_file = hfsplus_file_open(
      fs, PED_CPU_TO_BE32(HFSP_ATTRIB_ID), vh->attributes_file.extents,
      PED_BE64_TO_CPU(vh->attributes_file.logical_size) /
          PED_SECTOR_SIZE_DEFAULT);
  if (!priv_data->attributes_file)
    goto hpo_cc;

  map_sectors =
      (PED_BE32_TO_CPU(vh->total_blocks) + PED_SECTOR_SIZE_DEFAULT * 8 - 1) /
      (PED_SECTOR_SIZE_DEFAULT * 8);
  priv_data->dirty_alloc_map = (uint8_t *)ped_malloc((map_sectors + 7) / 8);
  if (!priv_data->dirty_alloc_map)
    goto hpo_cl;
  memset(priv_data->dirty_alloc_map, 0, (map_sectors + 7) / 8);
  priv_data->alloc_map =
      (uint8_t *)ped_malloc(map_sectors * PED_SECTOR_SIZE_DEFAULT);
  if (!priv_data->alloc_map)
    goto hpo_dm;

  priv_data->allocation_file = hfsplus_file_open(
      fs, PED_CPU_TO_BE32(HFSP_ALLOC_ID), vh->allocation_file.extents,
      PED_BE64_TO_CPU(vh->allocation_file.logical_size) /
          PED_SECTOR_SIZE_DEFAULT);
  if (!priv_data->allocation_file)
    goto hpo_am;
  if (!hfsplus_file_read(priv_data->allocation_file, priv_data->alloc_map, 0,
                         map_sectors)) {
    hfsplus_close(fs);
    return NULL;
  }
...
static int hfsplus_volume_resize(PedFileSystem *fs, PedGeometry *geom,
                                 PedTimer *timer) {
  uint8_t buf[PED_SECTOR_SIZE_DEFAULT];
  ...
  /* relocate data */
  to_free =
      (priv_data->plus_geom->length - geom->length + hfsp_sect_block - 1) /
      hfsp_sect_block;
  ...
  /* Check that all block after future end are really free */
  mblock = (priv_data->plus_geom->length - 2) / hfsp_sect_block;
  if (mblock > old_blocks - 1)
    mblock = old_blocks - 1;
  for (block = nblock; block < mblock; block++) {
    if (TST_BLOC_OCCUPATION(priv_data->alloc_map, block)) {
      resize = 0;
      ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
  ...
  if (resize) {
    ...
    priv_data->plus_geom->end = priv_data->plus_geom->start + geom->length - 1;
  }
...
write_VH:
  ...
  map_sectors = (old_blocks + PED_SECTOR_SIZE_DEFAULT * 8 - 1) /
                (PED_SECTOR_SIZE_DEFAULT * 8);
  if (!hfsplus_file_write(priv_data->allocation_file, priv_data->alloc_map, 0,
                          map_sectors)) {
    resize = 0;
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
    ...
    for (block = map_sectors; block < priv_data->allocation_file->sect_nb;
         ++block) {
      if (!hfsplus_file_write_sector(priv_data->allocation_file, buf, block)) {
...
static int hfsplus_wrapper_update(PedFileSystem *fs) {
  ...
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  HfsPrivateFSData *hfs_priv_data =
      (HfsPrivateFSData *)priv_data->wrapper->type_specific;
  unsigned int hfs_sect_block =
      PED_BE32_TO_CPU(hfs_priv_data->mdb->block_size) / PED_SECTOR_SIZE_DEFAULT;
  PedSector hfsplus_sect =
      (PedSector)PED_BE32_TO_CPU(priv_data->vh->total_blocks) *
      (PED_BE32_TO_CPU(priv_data->vh->block_size) / PED_SECTOR_SIZE_DEFAULT);
  unsigned int hfs_blocks_embedded =
      (hfsplus_sect + hfs_sect_block - 1) / hfs_sect_block;
  ...
  hfs_priv_data->mdb->free_blocks =
      PED_CPU_TO_BE16(PED_BE16_TO_CPU(hfs_priv_data->mdb->free_blocks) +
                      hfs_blocks_embedded_old - hfs_blocks_embedded);
  ...
  if (!ped_geometry_write(
          fs->geom, hfs_priv_data->alloc_map,
          PED_BE16_TO_CPU(hfs_priv_data->mdb->volume_bitmap_block),
          (PED_BE16_TO_CPU(hfs_priv_data->mdb->total_blocks) +
           PED_SECTOR_SIZE_DEFAULT * 8 - 1) /
              (PED_SECTOR_SIZE_DEFAULT * 8)))
    return 0;
  if (!ped_geometry_sync(fs->geom))
  ...
  if (!hfs_file_read_sector(hfs_priv_data->extent_file, node, ref.node_number))
    return 0;
  ret_key = (HfsExtentKey *)(node + ref.record_pos);
  ret_data = (HfsExtDescriptor *)(node + ref.record_pos + sizeof(HfsExtentKey));
  ...
  while (ret_key->type == key.type && ret_key->file_ID == key.file_ID) {
    for (i = 0; i < HFS_EXT_NB; i++) {
      if (ret_data[i].start_block ==
        ...
        ret_data[i].block_count =
            hfs_priv_data->mdb->old_new.embedded.location.block_count;
        /* found ! : update */
        if (!hfs_file_write_sector(hfs_priv_data->extent_file, node,
                                   ref.node_number) ||
    ...
    if (ref.record_number < PED_BE16_TO_CPU(node_desc->rec_nb)) {
      ...
      if (!ref.node_number || !hfs_file_read_sector(hfs_priv_data->extent_file,
    ...
    uint16_t value;
    memcpy(&value, node + PED_SECTOR_SIZE_DEFAULT - (2 * ref.record_number),
           sizeof(uint16_t));
    ref.record_pos = PED_BE16_TO_CPU(value);
    ret_key = (HfsExtentKey *)(node + ref.record_pos);
    ret_data =
        (HfsExtDescriptor *)(node + ref.record_pos + sizeof(HfsExtentKey));
  }
...
int hfsplus_resize(PedFileSystem *fs, PedGeometry *geom, PedTimer *timer) {
  ...
  if (priv_data->wrapper) {
    ...
    if (red > priv_data->plus_geom->length - hgee) {
      ...
      hgee = ((hgee + hfs_sect_block - 1) / hfs_sect_block) * hfs_sect_block;
      red = priv_data->plus_geom->length - hgee;
    }
    embedded_geom = ped_geometry_new(geom->dev, priv_data->plus_geom->start,
                                     priv_data->plus_geom->length - red);
    ...
    ped_timer_reset(timer);
    ped_timer_set_state_name(timer, _("shrinking embedded HFS+ volume"));
    ped_timer_update(timer, 0.0);
...
static int hfs_extract_file(const char *filename, HfsPrivateFile *hfs_file) {
  FILE *fout;
  PedSector sect;
...
static int hfs_extract_bitmap(const char *filename, PedFileSystem *fs) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
  ...
  for (sect = PED_BE16_TO_CPU(mdb->volume_bitmap_block);
       sect < PED_BE16_TO_CPU(mdb->start_block); sect += count) {
    uint16_t st_block = PED_BE16_TO_CPU(mdb->start_block);
    count =
        (st_block - sect) < BLOCK_MAX_BUFF ? (st_block - sect) : BLOCK_MAX_BUFF;
    if (!ped_geometry_read(fs->geom, extract_buffer, sect, count))
      goto err_close;
    if (!fwrite(extract_buffer, count * PED_SECTOR_SIZE_DEFAULT, 1, fout))
      goto err_close;
  }
...
static int hfs_extract_mdb(const char *filename, PedFileSystem *fs) {
  FILE *fout;
...
static int hfs_extract(PedFileSystem *fs, PedTimer *timer) {
  HfsPrivateFSData *priv_data = (HfsPrivateFSData *)fs->type_specific;
...
static int hfsplus_extract_file(const char *filename,
                                HfsPPrivateFile *hfsp_file) {
  FILE *fout;
  ...
  for (rem_sect = hfsp_file->sect_nb; rem_sect; rem_sect -= cp_sect) {
    ...
    if (!hfsplus_file_read(hfsp_file, extract_buffer,
      ...
      goto err_close;
    if (!fwrite(extract_buffer, cp_sect * PED_SECTOR_SIZE_DEFAULT, 1, fout))
      goto err_close;
  }
...
static int hfsplus_extract_vh(const char *filename, PedFileSystem *fs) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  FILE *fout;
...
static int hfsplus_extract(PedFileSystem *fs, PedTimer *timer) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  HfsPVolumeHeader *vh = priv_data->vh;
  ...
  startup_file = hfsplus_file_open(
      fs, PED_CPU_TO_BE32(HFSP_STARTUP_ID), vh->startup_file.extents,
      PED_BE64_TO_CPU(vh->startup_file.logical_size) / PED_SECTOR_SIZE_DEFAULT);
  if (startup_file) {
    hfsplus_extract_file(HFSP_STARTUP_FILENAME, startup_file);
    hfsplus_file_close(startup_file);
    startup_file = NULL;
  }
