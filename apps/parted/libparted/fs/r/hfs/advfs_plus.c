...
static int hfsplus_extent_key_cmp(HfsPPrivateGenericKey *a,
                                  HfsPPrivateGenericKey *b) {
  HfsPExtentKey *key1 = (HfsPExtentKey *)a;
  HfsPExtentKey *key2 = (HfsPExtentKey *)b;

  if (key1->file_ID != key2->file_ID)
    return PED_BE32_TO_CPU(key1->file_ID) < PED_BE32_TO_CPU(key2->file_ID) ? -1
                                                                           : +1;
  ...
  if (key1->start == key2->start)
    return 0;
  return PED_BE32_TO_CPU(key1->start) < PED_BE32_TO_CPU(key2->start) ? -1 : +1;
}
...
int hfsplus_btree_search(HfsPPrivateFile *b_tree_file,
  ...
  /* Read the root node */
  if (!hfsplus_file_read(b_tree_file, node, (PedSector)node_number * size,
                         size))
    return 0;
  ...
  while (1) {
    ...
    for (i = record_number; i; i--) {
      ...
      if ((record_pos < HFS_FIRST_REC) ||
          (record_pos >= (signed)bsize - 2 * (signed)(record_number + 1))) {
    ...
    if (desc->type == HFS_IDX_NODE) {
      ...
      skip = (2 + PED_BE16_TO_CPU(record_key->key_length) + 1) & ~1;
      uint32_t value;
...
int hfsplus_read_bad_blocks(const PedFileSystem *fs) {
  ...
  {
    uint8_t record[sizeof(HfsPExtentKey) + sizeof(HfsPExtDataRec)];
    HfsPExtentKey search;
    HfsPExtentKey *ret_key = (HfsPExtentKey *)record;
    HfsPExtDescriptor *ret_cache =
        (HfsPExtDescriptor *)(record + sizeof(HfsPExtentKey));
    ...
    while (1) {
      ...
      if (!hfsplus_btree_search(priv_data->extents_file,
                                (HfsPPrivateGenericKey *)&search, record,
                                sizeof(record), NULL) ||
          ret_key->file_ID != search.file_ID || ret_key->type != search.type) {
        if (first_pass)
          break;
        else
          goto errbbp;
      ...
      for (i = 0; i < HFSP_EXT_NB; i++) {
        if (ret_cache[i].block_count) {
          HfsPPrivateLinkExtent *new_xt = (HfsPPrivateLinkExtent *)ped_malloc(
              sizeof(HfsPPrivateLinkExtent));
          if (!new_xt)
            goto errbbp;
          new_xt->next = priv_data->bad_blocks_xtent_list;
          memcpy(&(new_xt->extent), ret_cache + i, sizeof(HfsPExtDescriptor));
          priv_data->bad_blocks_xtent_list = new_xt;
          priv_data->bad_blocks_xtent_nb++;
          block += PED_BE32_TO_CPU(ret_cache[i].block_count);
        }
      }
      first_pass = 0;
    }
...
int _GL_ATTRIBUTE_PURE hfsplus_is_bad_block(const PedFileSystem *fs,
  ...
  for (walk = priv_data->bad_blocks_xtent_list; walk; walk = walk->next) {
    ...
    if ((fblock >= PED_BE32_TO_CPU(walk->extent.start_block)) &&
        (fblock < (unsigned int)(PED_BE32_TO_CPU(walk->extent.start_block) +
...
PedSector hfsplus_get_empty_end(const PedFileSystem *fs) {
  ...
  end_free_blocks = 0;
  for (block = last_bad; block < PED_BE32_TO_CPU(vh->total_blocks); block++) {
...
PedSector hfsplus_get_min_size(const PedFileSystem *fs) {
  ...
  if (priv_data->wrapper) {
    ...
    PedSector hgee;
    hfs_sect_block = PED_BE32_TO_CPU(hfs_priv_data->mdb->block_size) /
    ...
    if (!hgee)
      return 0;
    min_size =
        ((min_size + hfs_sect_block - 1) / hfs_sect_block) * hfs_sect_block +
        hgee + 2 -
