...
static int hfsplus_effect_move_extent(PedFileSystem *fs,
                                      unsigned int *ptr_fblock,
  ...
  */
  ...
  if (*ptr_to_fblock != *ptr_fblock) {
    start = stop = *ptr_fblock < *ptr_to_fblock + size ? *ptr_fblock
                                                       : *ptr_to_fblock + size;
  ...
  if (ok) {
    ...
    for (i = 0; i < size; /*i++*/) {
      j = size - i;
      j = (j < hfsp_block_count) ? j : hfsp_block_count;

      abs_sector = (PedSector)(*ptr_fblock + i) * block_sz;
      if (!ped_geometry_read(priv_data->plus_geom, hfsp_block, abs_sector,
                             block_sz * j))
        return -1;

      abs_sector = (PedSector)(start + i) * block_sz;
      if (!ped_geometry_write(priv_data->plus_geom, hfsp_block, abs_sector,
                              block_sz * j))
        return -1;
...
/* Returns 0 on error */
/*         1 on succes */
int hfsplus_update_vh(PedFileSystem *fs) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
...
static int hfsplus_do_move(PedFileSystem *fs, unsigned int *ptr_src,
  ...
  new_start =
      hfsplus_effect_move_extent(fs, ptr_src, ptr_dest, ref->ext_length);
  ...
  if (ref->ext_start != (unsigned)new_start) {
    switch (ref->where) {
      /************ VH ************/
    case CR_PRIM_CAT:
      priv_data->catalog_file->first[ref->ref_index].start_block =
          PED_CPU_TO_BE32(new_start);
      goto CR_PRIM;
    case CR_PRIM_EXT:
      priv_data->extents_file->first[ref->ref_index].start_block =
          PED_CPU_TO_BE32(new_start);
      goto CR_PRIM;
    case CR_PRIM_ATTR:
      priv_data->attributes_file->first[ref->ref_index].start_block =
          PED_CPU_TO_BE32(new_start);
      goto CR_PRIM;
    case CR_PRIM_ALLOC:
      priv_data->allocation_file->first[ref->ref_index].start_block =
          PED_CPU_TO_BE32(new_start);
      goto CR_PRIM;
    case CR_PRIM_START:
    ...
    CR_PRIM:
      extent =
          (HfsPExtDescriptor *)((uint8_t *)priv_data->vh + ref->ref_offset);
      extent[ref->ref_index].start_block = PED_CPU_TO_BE32(new_start);
    ...
    case CR_BTREE_EXT_ATTR:
      if (priv_data->attributes_file->cache[ref->ref_index].start_block ==
          PED_CPU_TO_BE32(ref->ext_start))
        priv_data->attributes_file->cache[ref->ref_index].start_block =
            PED_CPU_TO_BE32(new_start);
      goto CR_BTREE_EXT;
    case CR_BTREE_EXT_CAT:
      if (priv_data->catalog_file->cache[ref->ref_index].start_block ==
          PED_CPU_TO_BE32(ref->ext_start))
        priv_data->catalog_file->cache[ref->ref_index].start_block =
            PED_CPU_TO_BE32(new_start);
      goto CR_BTREE_EXT;
    case CR_BTREE_EXT_ALLOC:
      if (priv_data->allocation_file->cache[ref->ref_index].start_block ==
          PED_CPU_TO_BE32(ref->ext_start))
        priv_data->allocation_file->cache[ref->ref_index].start_block =
            PED_CPU_TO_BE32(new_start);
      goto CR_BTREE_EXT;
    ...
    CR_BTREE:
      ...
      if (!hfsplus_file_read(file, hfsp_block,
                             ...
                             ref->sect_by_block))
        return -1;
      extent = (HfsPExtDescriptor *)(hfsp_block + ref->ref_offset);
      extent[ref->ref_index].start_block = PED_CPU_TO_BE32(new_start);
...
static int hfsplus_save_allocation(PedFileSystem *fs) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  ...
  for (i = 0; i < map_sectors;) {
    for (j = i; (TST_BLOC_OCCUPATION(priv_data->dirty_alloc_map, j)); ++j)
      CLR_BLOC_OCCUPATION(priv_data->dirty_alloc_map, j);
    if (j - i) {
      ret = hfsplus_file_write(
                priv_data->allocation_file,
                priv_data->alloc_map + i * PED_SECTOR_SIZE_DEFAULT, i, j - i) &&
            ret;
      i = j;
    } else
      ++i;
...
static int hfsplus_move_extent_starting_at(PedFileSystem *fs,
                                           unsigned int *ptr_fblock,
                                           unsigned int *ptr_to_fblock,
  ...
  if (new_start == (unsigned)-1)
    return -1;
  if (new_start > old_start) {
    new_start = hfsplus_do_move(fs, &new_start, ptr_to_fblock, cache, ref);
...
static int hfsplus_cache_from_vh(HfsCPrivateCache *cache, PedFileSystem *fs,
                                 PedTimer *timer) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  HfsPExtDescriptor *extent;
  unsigned int j;
  ...
  for (j = 0; j < HFSP_EXT_NB; ++j) {
    if (!extent[j].block_count)
      break;
    if (!hfsc_cache_add_extent(cache, PED_BE32_TO_CPU(extent[j].start_block),
...
static int hfsplus_cache_from_catalog(HfsCPrivateCache *cache,
                                      PedFileSystem *fs, PedTimer *timer) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  ...
  uint32_t jib = priv_data->jib_start_block, jl = priv_data->jl_start_block;
  ...
  for (; leaf_node; leaf_node = PED_BE32_TO_CPU(desc->next)) {
    ...
    for (i = 1; i <= record_number; i++) {
      ...
      skip = (2 + PED_BE16_TO_CPU(catalog_key->key_length) + 1) & ~1;
      catalog_data = (HfsPCatalog *)(((uint8_t *)catalog_key) + skip);
      /* check for obvious error in FS */
      if ((catalog_pos < HFS_FIRST_REC) ||
          ((uint8_t *)catalog_data - node >=
           (signed)bsize - 2 * (signed)(record_number + 1))) {
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
      ...
      for (j = 0; j < HFSP_EXT_NB; ++j) {
        if (!extent[j].block_count)
          break;
        where = CR_BTREE_CAT;
        if (PED_BE32_TO_CPU(extent[j].start_block) == jib) {
          jib = 0;
          where = CR_BTREE_CAT_JIB;
        } else if (PED_BE32_TO_CPU(extent[j].start_block) == jl) {
          jl = 0;
          where = CR_BTREE_CAT_JL;
        }
        if (!hfsc_cache_add_extent(
                cache, PED_BE32_TO_CPU(extent[j].start_block),
                PED_BE32_TO_CPU(extent[j].block_count), leaf_node,
                (uint8_t *)extent - node, size, where, j)) {
          free(node);
          return 0;
        }
      }
      ...
      for (j = 0; j < HFSP_EXT_NB; ++j) {
        ...
        if (!hfsc_cache_add_extent(
                cache, PED_BE32_TO_CPU(extent[j].start_block),
                PED_BE32_TO_CPU(extent[j].block_count), leaf_node,
                (uint8_t *)extent - node, size, CR_BTREE_CAT, j)) {
          free(node);
          return 0;
        }
      }
...
static int hfsplus_cache_from_extent(HfsCPrivateCache *cache, PedFileSystem *fs,
                                     PedTimer *timer) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  uint8_t node_1[PED_SECTOR_SIZE_DEFAULT];
  uint8_t *node;
  ...
  for (; leaf_node; leaf_node = PED_BE32_TO_CPU(desc->next)) {
    ...
    for (i = 1; i <= record_number; i++) {
      ...
      extent = (HfsPExtDescriptor *)(((uint8_t *)extent_key) +
                                     sizeof(HfsPExtentKey));
      /* check for obvious error in FS */
      ...
      case PED_CPU_TO_BE32(HFS_XTENT_ID):
        if (ped_exception_throw(
                ...
                _("The extents overflow file should not"
                  ...
                  "check the file system.")) != PED_EXCEPTION_IGNORE)
          return 0;
...
static int hfsplus_cache_from_attributes(HfsCPrivateCache *cache,
                                         PedFileSystem *fs, PedTimer *timer) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
  uint8_t node_1[PED_SECTOR_SIZE_DEFAULT];
  uint8_t *node;
  ...
  for (; leaf_node; leaf_node = PED_BE32_TO_CPU(desc->next)) {
    ...
    for (i = 1; i <= record_number; i++) {
      ...
      skip = (2 + PED_BE16_TO_CPU(generic_key->key_length) + 1) & ~1;
      ...
      if (fork_ext_data->record_type == PED_CPU_TO_BE32(HFSP_ATTR_FORK)) {
        ...
        for (j = 0; j < HFSP_EXT_NB; ++j) {
          ...
          if (!hfsc_cache_add_extent(
                  cache, PED_BE32_TO_CPU(extent[j].start_block),
                  PED_BE32_TO_CPU(extent[j].block_count), leaf_node,
                  (uint8_t *)extent - node, size, CR_BTREE_ATTR, j)) {
            free(node);
            return 0;
          }
        }
...
static HfsCPrivateCache *hfsplus_cache_extents(PedFileSystem *fs,
                                               PedTimer *timer) {
  HfsPPrivateFSData *priv_data = (HfsPPrivateFSData *)fs->type_specific;
...
/* return 0 on error */
int hfsplus_pack_free_space_from_block(PedFileSystem *fs, unsigned int fblock,
  ...
  unsigned int divisor =
      PED_BE32_TO_CPU(vh->total_blocks) + 1 - start - to_free;
  int ret;
  ...
  bytes_buff =
      PED_BE32_TO_CPU(priv_data->vh->block_size) * (PedSector)BLOCK_MAX_BUFF;
  if (bytes_buff > BYTES_MAX_BUFF) {
    hfsp_block_count =
        BYTES_MAX_BUFF / PED_BE32_TO_CPU(priv_data->vh->block_size);
    if (!hfsp_block_count)
      hfsp_block_count = 1;
  ...
  while (fblock <
    ...
    if (TST_BLOC_OCCUPATION(priv_data->alloc_map, fblock) &&
      ...
      if (!(ret = hfsplus_move_extent_starting_at(fs, &fblock, &to_fblock,
                                                  cache)))
        to_fblock = ++fblock;
      else if (ret == -1) {
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
