...
static int hfsplus_effect_move_extent(PedFileSystem *fs,
    ...
    */
    ...
    if (ok) {
        ...
        unsigned int block_sz = (PED_BE32_TO_CPU(priv_data->vh->block_size) /
        ...
        for (i = 0; i < size; /*i++*/) {
            ...
            if (!ped_geometry_write(priv_data->plus_geom, hfsp_block,
                                    abs_sector, block_sz * j))
                return -1;
...
static int hfsplus_do_move(PedFileSystem *fs, unsigned int *ptr_src,
    ...
    if (ref->ext_start != (unsigned)new_start) {
        ...
        CR_PRIM:
            extent = (HfsPExtDescriptor *)((uint8_t *)priv_data->vh +
                                           ref->ref_offset);
...
static int hfsplus_cache_from_vh(HfsCPrivateCache *cache, PedFileSystem *fs,
    ...
    for (j = 0; j < HFSP_EXT_NB; ++j) {
        ...
        if (!hfsc_cache_add_extent(
                ...
                PED_BE32_TO_CPU(extent[j].block_count), 0, /* unused for vh */
