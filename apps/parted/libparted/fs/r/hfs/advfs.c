...
/* in this function */
static int hfs_extent_key_cmp(HfsPrivateGenericKey *a,
                              HfsPrivateGenericKey *b) {
  ...
  /* anyway */
  return (int)(PED_BE16_TO_CPU(key1->start) - PED_BE16_TO_CPU(key2->start));
}
...
int hfs_btree_search(HfsPrivateFile *b_tree_file, HfsPrivateGenericKey *key,
...
int hfs_read_bad_blocks(const PedFileSystem *fs) {
  ...
  {
    uint8_t record[sizeof(HfsExtentKey) + sizeof(HfsExtDataRec)];
    HfsExtentKey search;
    HfsExtentKey *ret_key = (HfsExtentKey *)record;
    HfsExtDescriptor *ret_cache =
        (HfsExtDescriptor *)(record + sizeof(HfsExtentKey));
    ...
    while (1) {
      ...
      if (!hfs_btree_search(priv_data->extent_file,
                            (HfsPrivateGenericKey *)&search, record,
                            sizeof(record), NULL) ||
      ...
      for (i = 0; i < HFS_EXT_NB; i++) {
        if (ret_cache[i].block_count) {
          HfsPrivateLinkExtent *new_xt =
              (HfsPrivateLinkExtent *)ped_malloc(sizeof(HfsPrivateLinkExtent));
          if (!new_xt)
            goto errbb;
          new_xt->next = priv_data->bad_blocks_xtent_list;
          memcpy(&(new_xt->extent), ret_cache + i, sizeof(HfsExtDescriptor));
          priv_data->bad_blocks_xtent_list = new_xt;
          priv_data->bad_blocks_xtent_nb++;
          block += PED_BE16_TO_CPU(ret_cache[i].block_count);
        }
      }
      first_pass = 0;
    }
...
int _GL_ATTRIBUTE_PURE hfs_is_bad_block(const PedFileSystem *fs,
  ...
  for (walk = priv_data->bad_blocks_xtent_list; walk; walk = walk->next) {
    ...
    if ((fblock >= PED_BE16_TO_CPU(walk->extent.start_block)) &&
        (fblock < (unsigned int)(PED_BE16_TO_CPU(walk->extent.start_block) +
                                 PED_BE16_TO_CPU(walk->extent.block_count))))
      return 1;
  }
...
PedSector hfs_get_empty_end(const PedFileSystem *fs) {
  ...
  end_free_blocks = 0;
  for (block = last_bad; block < PED_BE16_TO_CPU(mdb->total_blocks); block++) {
