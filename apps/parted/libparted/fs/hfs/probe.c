...
PedGeometry *hfsplus_probe(PedGeometry *geom) {
  PedGeometry *geom_ret;
  ...
  if ((geom_ret = hfs_and_wrapper_probe(geom))) {
    ...
    if (!ped_geometry_read(geom, buf, 2, 1) ||
        (mdb->old_new.embedded.signature != PED_CPU_TO_BE16(HFSP_SIGNATURE))) {
      ped_geometry_destroy(geom_ret);
      return NULL;
    } else
      return geom_ret;
    ...
    if ((geom->length < 5) || !ped_geometry_read(geom, buf, 2, 1) ||
    ...
    max = ((PedSector)PED_BE32_TO_CPU(vh->total_blocks) + 1) *
          ...
          2;
    search = max -
             2 * (PED_BE32_TO_CPU(vh->block_size) / PED_SECTOR_SIZE_DEFAULT) +
             2;
    if ((search < 0) ||
        !(geom_ret = ped_geometry_new(geom->dev, geom->start, search + 2)))
      return NULL;
    ...
    search = ((PedSector)PED_BE32_TO_CPU(vh->total_blocks) - 1) *
             ...
             1;
    if ((search < 0) ||
        !ped_geometry_set(geom_ret, geom_ret->start, search + 2) ||
        !ped_geometry_read(geom_ret, buf, search, 1) ||
        vh->signature != PED_CPU_TO_BE16(HFSP_SIGNATURE)) {
      ped_geometry_destroy(geom_ret);
      return NULL;
...
PedGeometry *hfs_probe(PedGeometry *geom) {
  ...
  if ((geom_base = hfs_and_wrapper_probe(geom)) &&
    ...
    if (geom_base)
      ped_geometry_destroy(geom_base);
    if (geom_plus)
      ped_geometry_destroy(geom_plus);
    return NULL;
  }
}

PedGeometry *hfsx_probe(PedGeometry *geom) {
  PedGeometry *geom_ret;
  uint8_t buf[PED_SECTOR_SIZE_DEFAULT];
  PedSector search, max;
