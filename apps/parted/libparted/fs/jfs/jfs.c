...
static PedGeometry *jfs_probe(PedGeometry *geom) {
  struct superblock *sb = alloca(geom->dev->sector_size);

  if (geom->length * geom->dev->sector_size < JFS_SUPER_OFFSET)
    return NULL;
  if (!ped_geometry_read(geom, sb, JFS_SUPER_OFFSET / geom->dev->sector_size,
                         1))
    return NULL;
...
void ped_file_system_jfs_init() { ped_file_system_type_register(&jfs_type); }

void ped_file_system_jfs_done() { ped_file_system_type_unregister(&jfs_type); }
