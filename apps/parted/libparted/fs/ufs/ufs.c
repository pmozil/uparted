...
static PedGeometry *ufs_probe_sun(PedGeometry *geom) {
  const int sectors =
  ...
  if (PED_BE32_TO_CPU(sb->fs_magic) == UFS_MAGIC) {
    PedSector block_size =
        PED_BE32_TO_CPU(sb->fs_bsize) / geom->dev->sector_size;
  ...
  if (PED_LE32_TO_CPU(sb->fs_magic) == UFS_MAGIC) {
    PedSector block_size =
        PED_LE32_TO_CPU(sb->fs_bsize) / geom->dev->sector_size;
...
static PedGeometry *ufs_probe_hp(PedGeometry *geom) {
  struct ufs_super_block *sb;
  PedSector block_size;
