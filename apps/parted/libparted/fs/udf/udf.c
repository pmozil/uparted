...
static int read_bytes(const PedGeometry *geom, void *buffer, PedSector offset,
                      PedSector count) {
  char *sector_buffer;
  ...
  sector_count =
      (offset + count + geom->dev->sector_size - 1) / geom->dev->sector_size -
      sector_offset;
...
static int check_vrs(const PedGeometry *geom, unsigned int vsdsize) {
  ...
  /* Check only first 64 blocks, but theoretically standard does not define
   * upper limit */
  for (block = 0; block < 64; block++) {
    /* VRS starts at fixed offset 32kB, it is independent of block size or vsd
     * size */
    ...
    if (memcmp(ident, "NSR02", 5) == 0 || memcmp(ident, "NSR03", 5) == 0)
      return 1;

    /* Unknown VSD identifier means end of VRS */
    if (memcmp(ident, "BEA01", 5) != 0 && memcmp(ident, "TEA01", 5) != 0 &&
        memcmp(ident, "BOOT2", 5) != 0 && memcmp(ident, "CD001", 5) != 0 &&
...
/* Check for UDF AVDP */
static int check_anchor(const PedGeometry *geom, unsigned int blocksize,
                        int rel_block) {
  PedSector block;
  ...
  if (((unsigned long)tag[12] | ((unsigned long)tag[13] << 8) |
       ((unsigned long)tag[14] << 16) | ((unsigned long)tag[15] << 24)) !=
      block)
    return 0;
...
static int detect_udf(const PedGeometry *geom) {
  ...
  /* Check for block sizes larger then 2048, maximal theoretical block size is
   * 32kB */
...
PedGeometry *udf_probe(PedGeometry *geom) {
  if (!detect_udf(geom))
    return NULL;
...
void ped_file_system_udf_init() { ped_file_system_type_register(&udf_type); }

void ped_file_system_udf_done() { ped_file_system_type_unregister(&udf_type); }
