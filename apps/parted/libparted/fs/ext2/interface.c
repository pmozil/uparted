...
static PedGeometry *_ext2_generic_probe(PedGeometry *geom, int expect_ext_ver) {
  ...
  if (EXT2_SUPER_MAGIC(*sb) == EXT2_SUPER_MAGIC_CONST) {
    PedSector block_size =
        (EXT2_MIN_BLOCK_SIZE << (EXT2_SUPER_LOG_BLOCK_SIZE(*sb))) /
    ...
    is_ext3 =
        (EXT2_SUPER_FEATURE_COMPAT(*sb) & EXT3_FEATURE_COMPAT_HAS_JOURNAL) != 0;
    if (is_ext3) {
      is_ext4 =
          ((EXT2_SUPER_FEATURE_RO_COMPAT(*sb) &
           ...
           (EXT2_SUPER_FEATURE_INCOMPAT(*sb) & EXT4_FEATURE_INCOMPAT_EXTENTS) ||
           (EXT2_SUPER_FEATURE_INCOMPAT(*sb) & EXT4_FEATURE_INCOMPAT_64BIT) ||
           (EXT2_SUPER_FEATURE_INCOMPAT(*sb) & EXT4_FEATURE_INCOMPAT_FLEX_BG));
      if (is_ext4)
        is_ext3 = 0;
    }
    ...
    if (version > 0 && group_nr > 0) {
      ...
      start = geom->start - group_blocks * group_nr - first_data_block;

      if (start < 0)
        return NULL;
      ped_geometry_init(&probe_geom, geom->dev, start,
                        block_count * block_size);
      return _ext2_generic_probe(&probe_geom, expect_ext_ver);
    } else {
      return ped_geometry_new(geom->dev, geom->start, block_count * block_size);
    }
  }
...
static PedGeometry *_ext2_probe(PedGeometry *geom) {
  return _ext2_generic_probe(geom, 2);
}

static PedGeometry *_ext3_probe(PedGeometry *geom) {
  return _ext2_generic_probe(geom, 3);
}

static PedGeometry *_ext4_probe(PedGeometry *geom) {
