...
static struct AmigaBlock *_amiga_read_block(PedDevice *dev,
                                            struct AmigaBlock *blk,
                                            PedSector block,
                                            struct AmigaIds *ids) {
  if (!ped_device_read(dev, blk, block, 1)) {
    switch (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
  ...
  if (_amiga_checksum(blk) != 0) {
    ...
    case PED_EXCEPTION_FIX:
      ...
      if (!ped_device_write(dev, blk, block, 1)) {
        switch (ped_exception_throw(PED_EXCEPTION_FATAL, PED_EXCEPTION_CANCEL,
...
static uint32_t _amiga_find_rdb(PedDevice *dev, struct RigidDiskBlock *rdb) {
  int i;
  struct AmigaIds *ids;
...
struct PartitionBlock *amiga_find_part(PedGeometry *geom,
  ...
  for (i = 1, partblock = PED_BE32_TO_CPU(rdb->rdb_PartitionList);
    ...
    end = ((((PedSector)PED_BE32_TO_CPU(part->de_HighCyl)) + 1) * (cylblocks)) -
          1;
