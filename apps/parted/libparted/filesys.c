...
void ped_file_system_alias_register(PedFileSystemType *fs_type,
...
void ped_file_system_alias_unregister(PedFileSystemType *fs_type,
                                      const char *alias) {
...
PedFileSystemType *ped_file_system_type_get(const char *name) {
    ...
    if (alias_walk != NULL) {
        if (alias_walk->deprecated)
            PED_DEBUG(0, "File system alias %s is deprecated", name);
...
static int _geometry_error(const PedGeometry *a, const PedGeometry *b) {
...
static PedFileSystemType *_best_match(const PedGeometry *geom,
                                      PedFileSystemType *detected[],
                                      const int detected_error[],
                                      int detected_count) {
    int best_match = 0;
    int i;
    PedSector min_error;
    ...
    for (i = 0; i < detected_count; i++) {
        ...
        if (abs(detected_error[best_match] - detected_error[i]) < min_error)
            return NULL;
    }
...
PedFileSystemType *ped_file_system_probe(PedGeometry *geom) {
    ...
    while ((walk = ped_file_system_type_get_next(walk))) {
        ...
        if (probed) {
            ...
            detected_error[detected_count] = _geometry_error(geom, probed);
            detected_count++;
