...
static PedDisk *loop_alloc(const PedDevice *dev) {
    ...
    PedPartition *part = ped_partition_new(disk, PED_PARTITION_NORMAL, NULL,
                                           geom->start, geom->end);
...
static PedDisk *loop_duplicate(const PedDisk *disk) {
...
static int loop_read(PedDisk *disk) {
    ...
    part = ped_partition_new(disk, PED_PARTITION_NORMAL, fs_type, geom->start,
                             geom->end);
    ped_geometry_destroy(geom);
    if (!part)
        goto error;
...
static int loop_write(const PedDisk *disk) {
    size_t buflen = disk->dev->sector_size;
    char *buf = alloca(buflen);
    PedPartition *part = ped_disk_get_partition(disk, 1);
    /* if there is already a filesystem on the disk, we don't need to write the
     * signature */
...
static PedPartition *loop_partition_new(const PedDisk *disk,
...
static PedPartition *loop_partition_duplicate(const PedPartition *part) {
    PedPartition *result;
...
static void loop_partition_destroy(PedPartition *part) { free(part); }

static int loop_partition_set_system(PedPartition *part,
                                     const PedFileSystemType *fs_type) {
    part->fs_type = fs_type;
    return 1;
}

static int loop_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                   int state) {
    return 0;
}

static int loop_partition_get_flag(const PedPartition *part,
...
static int loop_partition_is_flag_available(const PedPartition *part,
                                            PedPartitionFlag flag) {
    return 0;
}
...
static int loop_alloc_metadata(PedDisk *disk) { return 1; }

static int loop_get_max_primary_partition_count(const PedDisk *disk) {
    return 1;
}

static bool loop_get_max_supported_partition_count(const PedDisk *disk,
                                                   int *max_n) {
    *max_n = 1;
    return true;
}
...
static PedDiskType loop_disk_type =
    {next : NULL, name : "loop", ops : &loop_disk_ops, features : 0};

void ped_disk_loop_init() { ped_disk_type_register(&loop_disk_type); }

void ped_disk_loop_done() { ped_disk_type_unregister(&loop_disk_type); }
