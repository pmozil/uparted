...
static void fdasd_error(fdasd_anchor_t *anc, enum fdasd_failure why,
    ...
    case config_syntax_error:
        sprintf(error, "fdasd: %s -- %s\n", _("Syntax error in config file"),
                str);
        break;
    case vlabel_corrupted:
        sprintf(error, "fdasd: %s -- %s\n", _("Volume label is corrupted"),
                str);
        break;
    case dsname_corrupted:
        sprintf(error, "fdasd: %s -- %s\n", _("A data set name is corrupted"),
                str);
        break;
    case malloc_failed:
...
static unsigned int recs_per_track(unsigned short dev_type, unsigned int kl,
    ...
    case DASD_3380_TYPE:
        if (kl)
            return 1499 /
                   (15 + 7 + ceil_quot(kl + 12, 32) + ceil_quot(dl + 12, 32));
        else
...
partition_info_t *fdasd_add_partition(fdasd_anchor_t *anc, unsigned int start,
    ...
    PDEBUG;
    vtoc_set_freespace(anc->f4, anc->f5, anc->f7, '-', anc->verbose, start,
                       stop, anc->formatted_cylinders, anc->geo.heads);
...
void fdasd_reuse_vtoc(fdasd_anchor_t *anc) {
    ...
    vtoc_init_format4_label(&f4, geo.cylinders, anc->formatted_cylinders,
                            geo.heads, geo.sectors, anc->blksize,
                            anc->dev_type);
