...
static void get_addr_of_highest_f1_f8_label(fdasd_anchor_t *anc,
                                            cchhb_t *addr) {
...
static void fdasd_error(fdasd_anchor_t *anc, enum fdasd_failure why,
                        char const *str) {
  PDEBUG
...
void fdasd_initialize_anchor(fdasd_anchor_t *anc) {
  ...
  if (v == NULL)
    fdasd_error(anc, malloc_failed, _("No room for volume label."));
  ...
  for (i = 1; i <= USABLE_PARTITIONS; i++) {
    ...
    if (p == NULL)
      fdasd_error(anc, malloc_failed, _("No room for partition info."));
 ...
 */
static void fdasd_write_vtoc_labels(fdasd_anchor_t *anc, int fd) {
  PDEBUG
  partition_info_t *p;
  ...
  for (p = anc->first; p != NULL; p = p->next) {
    ...
    if (p->f1->DS1FMTID == 0xf8) {
      ...
      b += anc->blksize;
      vtoc_write_label(fd, b, NULL, NULL, NULL, NULL, anc->f9);
      b += anc->blksize;
    } else {
      vtoc_write_label(fd, b, p->f1, NULL, NULL, NULL, NULL);
      b += anc->blksize;
...
void fdasd_recreate_vtoc(fdasd_anchor_t *anc) {
  ...
  vtoc_init_format4_label(anc->f4, anc->geo.cylinders, anc->formatted_cylinders,
                          anc->geo.heads, anc->geo.sectors, anc->blksize,
                          anc->dev_type);
  ...
  anc->fspace_trk =
      anc->formatted_cylinders * anc->geo.heads - FIRST_USABLE_TRK;
 ...
 */
static void fdasd_init_volume_label(fdasd_anchor_t *anc, int fd) {
...
static void fdasd_update_partition_info(fdasd_anchor_t *anc) {
  ...
  for (i = 1; i <= USABLE_PARTITIONS; i++) {
    if (p->f1->DS1FMTID != 0xf1 && p->f1->DS1FMTID != 0xf8) {
      if (i == 1)
...
int fdasd_check_volume(fdasd_anchor_t *anc, int fd) {
  ...
  if (strncmp(v->vollbl, vtoc_ebcdic_enc("VOL1", str, 4), 4) == 0) {
    ...
    vtoc_init_format4_label(anc->f4, anc->geo.cylinders,
                            anc->formatted_cylinders, anc->geo.heads,
                            anc->geo.sectors, anc->blksize, anc->dev_type);
...
void fdasd_check_api_version(fdasd_anchor_t *anc, int f) {
  ...
  if (fstat(f, &st) == 0 && S_ISREG(st.st_mode)) {
    ...
    if (ioctl(f, DASDAPIVER, &api) != 0)
      fdasd_error(anc, unable_to_ioctl, _("Could not retrieve API version."));

    if (api != DASD_MIN_API_VERSION) {
      sprintf(s,
              _("The current API version '%d' doesn't "
                "match dasd driver API version "
                "'%d'!"),
              api, DASD_MIN_API_VERSION);
...
static unsigned int recs_per_track(unsigned short dev_type, unsigned int kl,
  ...
  case DASD_3380_TYPE:
    if (kl)
      return 1499 / (15 + 7 + ceil_quot(kl + 12, 32) + ceil_quot(dl + 12, 32));
    else
  ...
  case DASD_3390_TYPE:
    ...
    if (kl) {
      ...
      return 1729 / (10 + 9 + ceil_quot(kl + 6 * kn, 34) + 9 +
  ...
  case DASD_9345_TYPE:
    ...
    if (kl) {
      kn = ceil_quot(kl + 6, 232) + 1;
      return 1420 /
             (18 + 7 + ceil_quot(kl + 6 * kn, 34) + ceil_quot(dl + 6 * dn, 34));
    } else
...
int fdasd_get_geometry(const PedDevice *dev, fdasd_anchor_t *anc, int f) {
  ...
  if (fstat(f, &st) == 0 && S_ISREG(st.st_mode)) {
    ...
    anc->hw_cylinders =
        ((st.st_size / blksize) / anc->geo.sectors) / anc->geo.heads;
    anc->is_file = 1;
  } else {
    if (ioctl(f, BLKGETSIZE64, &size_in_bytes) != 0) {
      fdasd_error(anc, unable_to_ioctl, _("Could not retrieve disk size."));
      goto error;
    }

    if (ioctl(f, HDIO_GETGEO, &anc->geo) != 0 || anc->geo.heads == 0 ||
        anc->geo.sectors == 0 || anc->geo.cylinders == 0) {
    ...
    if (ioctl(f, BIODASDINFO, &dasd_info) != 0) {
      ...
      if (!fdasd_verify_geometry(DASD_3390_TYPE, blksize, &anc->geo)) {
...
static void fdasd_enqueue_new_partition(fdasd_anchor_t *anc) {
  ...
  if (anc->first == p) {
    ...
    if (p->start_trk == FIRST_USABLE_TRK) {
      ...
      p->fspace_trk =
          anc->fspace_trk - p->len_trk - p->start_trk + FIRST_USABLE_TRK;
      anc->fspace_trk = p->start_trk - FIRST_USABLE_TRK;
    }
  } else {
    ...
    if (p->start_trk == p->prev->end_trk + 1) {
      ...
      p->fspace_trk = p->prev->fspace_trk - p->len_trk - p->start_trk +
                      p->prev->end_trk + 1;
 ...
 */
partition_info_t *fdasd_add_partition(fdasd_anchor_t *anc, unsigned int start,
                                      unsigned int stop) {
  PDEBUG
  cchhb_t hf1;
  partition_info_t *p;
  ...
  PDEBUG;
  vtoc_set_freespace(anc->f4, anc->f5, anc->f7, '-', anc->verbose, start, stop,
                     anc->formatted_cylinders, anc->geo.heads);
...
void fdasd_check_volser(char *volser, int devno) {
  ...
  for (from = 0, to = 0; volser[from] && from < VOLSER_LENGTH; from++) {
    ...
    if ((volser[from] >= 0x23 && volser[from] <= 0x25) ||
        (volser[from] >= 0x30 && volser[from] <= 0x39) ||
        (volser[from] >= 0x40 && volser[from] <= 0x5a) ||
        (volser[from] >= 0x61 && volser[from] <= 0x7a))
...
void fdasd_reuse_vtoc(fdasd_anchor_t *anc) {
  ...
  vtoc_init_format4_label(&f4, geo.cylinders, anc->formatted_cylinders,
                          geo.heads, geo.sectors, anc->blksize, anc->dev_type);
  ...
  if (anc->fspace_trk > 0)
    vtoc_set_freespace(&f4, &f5, &f7, '+', anc->verbose, FIRST_USABLE_TRK,
  ...
  while (part_info != NULL) {
    ...
    if (anc->formatted_cylinders > LV_COMPAT_CYL)
      vtoc_init_format8_label(anc->blksize, &part_info->f1->DS1EXT1, &f1);
    else
      vtoc_init_format1_label(anc->blksize, &part_info->f1->DS1EXT1, &f1);
