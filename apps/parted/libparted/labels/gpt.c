...
static void *gpt_read_PE_array(PedDisk const *disk,
  ...
  *ptes_bytes = p_ent_size * PED_LE32_TO_CPU(gpt->NumberOfPartitionEntries);
  ...
  if (!ped_device_read(disk->dev, ptes, PED_LE64_TO_CPU(gpt->PartitionEntryLBA),
                       ptes_sectors)) {
...
static int _parse_header(PedDisk *disk, const GuidPartitionTableHeader_t *gpt,
  ...
  /* Need to check whether the volume has grown, the LastUsableLBA is
     ...
     greater. If the volume has grown, offer the user the chance to use the new
  ...
  if (primary_gpt && backup_gpt) {
    ...
    if (gpt_disk_data->AlternateLBA != disk->dev->length - 1 &&
      ...
      if (ped_exception_throw(
        ...
        ptt_clear_sectors(disk->dev, PED_LE64_TO_CPU(primary_gpt->AlternateLBA),
                          1);
        gpt_disk_data->AlternateLBA = disk->dev->length - 1;
        write_back = 1;
      }
    ...
    /* The primary header is ok, but backup is corrupt.  */
    if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_OK_CANCEL,
...
static bool gpt_get_max_supported_partition_count(const PedDisk *disk,
  ...
  if (!_header_is_valid(disk, pth, 1)) {
    ...
    pth->SizeOfPartitionEntry = PED_CPU_TO_LE32(sizeof(GuidPartitionEntry_t));
  }
