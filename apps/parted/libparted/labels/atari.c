    ...
    for (i = 0; i < N_AHDI; i++) {
        ...
        if (atr_pid_eq(table.part[i].id, "XGM")) {
            ...
            if (!atr_parse_add_rawpart(disk, PED_PARTITION_EXTENDED, 0, 0,
                                       &table.part[i]) ||
                !atr_read_logicals(disk, PED_BE32_TO_CPU(table.part[i].start),
                                   &pnum))
                goto error;
    ...
    while (1) {
        ...
        if (!log_curr) {
            ...
            atr_fill_raw_entry(&table.part[0], atr_part->flag,
                               atr_part->part_id, log_curr->geom.start - parts,
...
static int atari_write(const PedDisk *disk) {
    ...
    append_ext =
        (ext_part != NULL) && (_disk_logical_partition_count(disk) == 0);
...
static int atari_alloc_metadata(PedDisk *disk) {
    ...
    if (atr_disk->bsl_start || atr_disk->bsl_count) {
        if (!atr_creat_add_metadata(
                disk, atr_disk->bsl_start,
                atr_disk->bsl_start + atr_disk->bsl_count - 1, 0))
            return 0;
    }
    ...
    if (ext) {
        ...
        for (log = ext->part_list; log; log = log->next) {
            if ((log->type &
                 (PED_PARTITION_METADATA | PED_PARTITION_FREESPACE)) ||
                log->num == i)
                continue;
