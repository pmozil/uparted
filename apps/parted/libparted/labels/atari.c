...
static const uint16_t atr_forbidden_sign[] = {0x55AA, 0};

static const char *atr_known_icd_pid[] = {"BGM", "GEM", "LNX",
                                          "SWP", "RAW", NULL};
...
static AtariPartID2BootFlag atr_pid2bf[] = {
    {"GEM", PART_FLAG_BOOT_GEM}, {"BGM", PART_FLAG_BOOT_GEM},
    {"UNX", PART_FLAG_BOOT_ASV}, {"LNX", PART_FLAG_BOOT_LNX},
    {"", PART_FLAG_BOOT_UNK},
};
...
static int atr_start_size_correct(uint32_t start, uint32_t size,
                                  uint32_t hd_size) {
  uint32_t end = start + size;

  return end >= start && 0 < start && start <= hd_size && 0 < size &&
         size <= hd_size && 0 < end && end <= hd_size;
}

static int atr_part_correct(AtariRawPartition *part, uint32_t hd_size) {
  uint32_t start, size;
...
static int _GL_ATTRIBUTE_PURE atr_pid_known(const char *pid,
                                            const char **pid_list) {
...
static int atr_is_signature_entry(AtariRawPartition *part) {
  return part->flag == 0 &&
         !memcmp(part->empty, SIGNATURE_EMPTY_TABLE, SIGNATURE_EMPTY_SIZE);
}

/* Set Parted signature in an AHDI entry */
static void atr_put_signature_entry(AtariRawPartition *part) {
  part->flag = 0;
...
#define atr_part_valid(part, sz)                                               \
  (atr_part_used(part) && atr_part_correct((part), (sz)))
#define atr_part_trash(part, sz)                                               \
  (atr_part_used(part) && !atr_part_correct((part), (sz)))

/* Check if this device can be used with an Atari label */
static int atr_can_use_dev(const PedDevice *dev) {
 ...
 */
static int atari_probe(const PedDevice *dev) {
  AtariRawTable table;
  ...
  if ((!valid_count && num_sign != N_AHDI) || xgm_part > 1 ||
  ...
  if (xgm_part) { /* ! WARNING ! reuses "table" */
    ...
    while (1) {
      ...
      if (i == N_AHDI - 1 ||
          !atr_part_correct(&table.part[i], rs_hd_size - parts) ||
      ...
      if (!atr_part_correct(&table.part[i + 1], rs_hd_size - exts) ||
...
static void atr_disk_reset(AtariDisk *atr_disk) {
 ...
 * the settings.
 */
static PedDisk *atari_alloc(const PedDevice *dev) {
  PedDisk *disk;
  AtariDisk *atr_disk;

  PED_ASSERT(dev != NULL);

  if (!atr_can_use_dev(dev) || !(disk = _ped_disk_alloc(dev, &atari_disk_type)))
    return NULL;
...
static PedDisk *atari_duplicate(const PedDisk *disk) {
  PedDisk *new_disk;
...
static void atari_free(PedDisk *disk) {
  AtariDisk *atr_disk;
  PED_ASSERT(disk != NULL);
  PED_ASSERT(disk->disk_specific != NULL);
...
static void atr_part_sysraw(PedPartition *part, const char *id, uint8_t flag) {
...
static int atr_parse_add_rawpart(PedDisk *disk, PedPartitionType type,
                                 PedSector st_off, int num,
                                 const AtariRawPartition *rawpart) {
  PedSector start, end;
  PedPartition *part;
 ...
 */
static int atr_read_logicals(PedDisk *disk, PedSector exts, int *pnum) {
  AtariRawTable table;
  ...
  while (1) {
    ...
    /* data partition must be in slot 0, 1 or 2 */
    if (i == N_AHDI - 1 || atr_pid_eq(table.part[i].id, "XGM")) {
      ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
    ...
    if (!atr_parse_add_rawpart(disk, PED_PARTITION_LOGICAL, parts, *pnum,
                               &table.part[i]))
      return 0;
...
static int atari_read(PedDisk *disk) {
  AtariRawTable table;
  AtariDisk *atr_disk;
  uint32_t rs_hd_size;
  ...
  for (i = 0; i < N_AHDI; i++) {
    ...
    if (atr_pid_eq(table.part[i].id, "XGM")) {
      ...
      xgm = 1;
      if (!atr_parse_add_rawpart(disk, PED_PARTITION_EXTENDED, 0, 0,
                                 &table.part[i]) ||
          !atr_read_logicals(disk, PED_BE32_TO_CPU(table.part[i].start), &pnum))
        goto error;

    } else {

      if (!atr_parse_add_rawpart(disk, PED_PARTITION_NORMAL, 0, pnum,
                                 &table.part[i]))
        goto error;
      pnum++;
    }
  ...
  /* ICD table. */
  if (!xgm && pcount != 0 && atr_part_valid(&table.icd_part[0], rs_hd_size) &&
      atr_part_known(&table.icd_part[0], atr_known_icd_pid))
    for (i = 0; i < N_ICD; i++) {
...
static int atr_find_first_log(const PedDisk *disk) {
  PedPartition *part;
  int first_log, last;
...
static int atari_clobber(PedDevice *dev) {
  AtariRawTable table;
  ...
  memset(table.boot_code + offsetof(AtariRawTable, hd_size), 0,
...
static uint16_t atr_calc_rs_sum(const AtariRawTable *table) {
...
static int atr_is_boot_table(const AtariRawTable *table) {
 ...
 */
static int _GL_ATTRIBUTE_PURE atr_sign_is_forbidden(uint16_t sign) {
...
static void atr_table_set_boot(AtariRawTable *table, int boot) {
  ...
  while (atr_sign_is_forbidden(noboot_cksum) || noboot_cksum == boot_cksum)
    noboot_cksum++;
...
static void atr_fill_raw_entry(AtariRawPartition *rawpart, uint8_t flag,
                               const char *id, uint32_t start, uint32_t size) {
...
static int atr_write_logicals(const PedDisk *disk) {
  AtariRawTable table;
  ...
  while (1) {
    ...
    if (part && ped_partition_is_active(part)) {
      if (log_curr)
        ...
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                            _("No room at sector %lli to store ARS."), parts);
      return 0;
    }
    ...
    if (!log_curr) {
      ...
      atr_fill_raw_entry(&table.part[0], atr_part->flag, atr_part->part_id,
                         log_curr->geom.start - parts, log_curr->geom.length);
...
_disk_logical_partition_count(const PedDisk *disk) {
  ...
  for (walk = disk->part_list; walk;
    ...
    if (ped_partition_is_active(walk) && (walk->type & PED_PARTITION_LOGICAL))
      count++;
  }
...
static int atr_load_fix_hdsize(const PedDisk *disk, uint32_t *rs_hd_size,
                               AtariRawTable *table) {
...
static int atr_empty_init_bsl(const PedDisk *disk) {
  ...
  for (sec = atr_disk->bsl_start;
       sec < atr_disk->bsl_start + atr_disk->bsl_count; sec++) {
    ...
    if (part && ped_partition_is_active(part)) {
      ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                          _("No room at sector %lli to store BSL."), sec);
      return 0;
    }
...
static int atari_write(const PedDisk *disk) {
  AtariRawTable table;
  AtariDisk *atr_disk;
  AtariPart *atr_part;
  PedPartition *log;
  ...
  PED_ASSERT(atr_disk->format != FMT_AHDI ||
  ...
  append_ext = (ext_part != NULL) && (_disk_logical_partition_count(disk) == 0);
  ...
  for (i = 0; i < N_AHDI; i++) {
    if (pnum > last_num)
      ...
      while (pnum <= last_num && !(part = ped_disk_get_partition(disk, pnum)))
        pnum++;
    ...
    atr_fill_raw_entry(&table.part[i], atr_part->flag, atr_part->part_id,
                       part->geom.start, part->geom.length);

    if (part->type & PED_PARTITION_EXTENDED) {
      while (pnum <= last_num) {
        part = ped_disk_get_partition(disk, pnum);
        if (part && !(part->type & PED_PARTITION_LOGICAL))
          break;
        pnum++;
      }
    } else
  ...
  if ((ext_part != NULL || atr_disk->format == FMT_AHDI) && pnum <= last_num) {
  ...
  if (pnum > last_num && (atr_disk->format == FMT_ICD || ext_part == NULL))
    atr_disk->format = FMT_AHDI;
  ...
  if (atr_disk->format == FMT_AHDI &&
    ...
    result = ped_exception_throw(
        PED_EXCEPTION_WARNING, PED_EXCEPTION_YES_NO_CANCEL,
    ...
    if (result == PED_EXCEPTION_YES || result == PED_EXCEPTION_UNHANDLED)
  ...
  if (atr_disk->format == FMT_ICD)
    for (i = 0; i < N_ICD; i++) {
      ...
      atr_part = ATARI_PART(part);
      atr_fill_raw_entry(&table.icd_part[i], atr_part->flag, atr_part->icd_id,
                         part->geom.start, part->geom.length);
...
/* else returns 0 */
static int atr_xgm_in_icd(const PedDisk *disk, PedPartitionType part_type) {
  AtariDisk *atrdisk;
...
static PedPartition *atari_partition_new(const PedDisk *disk,
...
static PedPartition *atari_partition_duplicate(const PedPartition *part) {
  ...
  if (ped_partition_is_active(part))
    memcpy(new_part->disk_specific, part->disk_specific, sizeof(AtariPart));

  return new_part;
}

static void atari_partition_destroy(PedPartition *part) {
  PED_ASSERT(part != NULL);
...
static int atari_partition_set_system(PedPartition *part,
                                      const PedFileSystemType *fs_type) {
  AtariPart *atrpart;
...
static int atari_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                    int state) {
  AtariPart *atr_part;
...
static int _GL_ATTRIBUTE_PURE atari_partition_get_flag(const PedPartition *part,
...
static int atari_partition_is_flag_available(const PedPartition *part,
                                             PedPartitionFlag flag) {
  if (flag == PED_PARTITION_BOOT)
    return 1;
...
static PedConstraint *atr_log_constraint(const PedPartition *part) {
  ...
  while (
      walk != NULL &&
      (walk->geom.start - (walk->num != first_log) < geom->start - not_first ||
       walk->geom.start - (walk->num != first_log) < min_start)) {
  ...
  ped_geometry_init(&safe_space, dev, min_start, max_end - min_start + 1);
  return ped_constraint_new_from_max(&safe_space);
}

/* Adapted from disk_dos */
static PedGeometry *art_min_extended_geom(const PedPartition *ext_part) {
...
/* Adapted from disk_dos */
static PedConstraint *atr_ext_constraint(const PedPartition *part) {
...
static PedConstraint *atr_prim_constraint(const PedPartition *part) {
  PedDevice *dev;
  PedGeometry max;
...
static PedGeometry *_best_solution(PedGeometry *a, PedGeometry *b) {
  if (!a)
    return b;
  if (!b)
    return a;
 ...
 */
static int atr_prim_align(PedPartition *part, const PedConstraint *constraint,
                          PedConstraint *internal) {
  PedDevice *dev;
  AtariDisk *atr_disk;
  PedConstraint *cut;
...
static int atari_partition_align(PedPartition *part,
  ...
  case PED_PARTITION_EXTENDED:
    if (atr_prim_align(part, constraint, atr_ext_constraint(part)))
      return 1;
    break;
  default:
    if (atr_prim_align(part, constraint, atr_prim_constraint(part)))
      return 1;
    break;
  }
...
static int art_room_for_logic(PedDisk *disk) {
  PedPartition *part;
...
static int atari_partition_enumerate(PedPartition *part) {
  AtariDisk *atrdisk;
  PedPartition *ext_part;
  PedPartition *log;
  ...
  xgm_begin = ((log = ped_disk_get_partition(part->disk, 1)) &&
               (log->type & PED_PARTITION_LOGICAL)) ||
              ((part->num == -1) && (part->type & PED_PARTITION_LOGICAL) &&
               !ped_disk_get_partition(part->disk, 1));
  /* </hack in the hack> */

  PED_ASSERT(atrdisk->format != FMT_ICD || ext_part == NULL);
  PED_ASSERT(atrdisk->format != FMT_XGM || prim_count + xgm_begin <= N_AHDI);
  PED_ASSERT(atrdisk->format != FMT_AHDI ||
             (ext_part == NULL && prim_count + xgm_begin <= N_AHDI));

  want_icd =
      ((prim_count + xgm_begin +
        ((part->num == -1) && !(part->type & PED_PARTITION_LOGICAL))) > N_AHDI);
  want_xgm = ((part->type & PED_PARTITION_EXTENDED) || ext_part != NULL);
...
static int atr_creat_add_metadata(PedDisk *disk, PedSector start, PedSector end,
...
static int atari_alloc_metadata(PedDisk *disk) {
  PedPartition *ext;
  PedPartition *log;
  AtariDisk *atr_disk;
  int i;
  ...
  if (atr_disk->bsl_start || atr_disk->bsl_count) {
    if (!atr_creat_add_metadata(disk, atr_disk->bsl_start,
                                atr_disk->bsl_start + atr_disk->bsl_count - 1,
                                0))
      return 0;
  }
  ...
  ext = ped_disk_extended_partition(disk);
  if (ext) {
    if (!atr_creat_add_metadata(disk, ext->geom.start, ext->geom.start,
    ...
    for (log = ext->part_list; log; log = log->next) {
      if ((log->type & (PED_PARTITION_METADATA | PED_PARTITION_FREESPACE)) ||
          log->num == i)
        continue;

      if (!atr_creat_add_metadata(disk, log->geom.start - 1,
                                  log->geom.start - 1, PED_PARTITION_LOGICAL))
        return 0;
    }
  }
...
static bool atari_get_max_supported_partition_count(const PedDisk *disk,
                                                    int *max_n) {
  AtariDisk *atr_disk;
