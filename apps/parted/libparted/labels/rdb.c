...
static const char *_amiga_block_id(uint32_t id) {
    switch (id) {
...
static struct AmigaIds *_amiga_add_id(uint32_t id, struct AmigaIds *ids) {
...
static void _amiga_free_ids(struct AmigaIds *ids) {
...
static int _GL_ATTRIBUTE_PURE _amiga_id_in_list(uint32_t id,
                                                struct AmigaIds *ids) {
...
static int _amiga_checksum(struct AmigaBlock *blk) {
    ...
    if (end > PED_SECTOR_SIZE_DEFAULT)
        end = PED_SECTOR_SIZE_DEFAULT;

    for (i = 1; i < end; i++)
        sum += PED_BE32_TO_CPU(rdb[i]);

    return sum;
}

static void _amiga_calculate_checksum(struct AmigaBlock *blk) {
    blk->amiga_ChkSum =
...
static struct AmigaBlock *_amiga_read_block(const PedDevice *dev,
    ...
    if (_amiga_checksum(blk) != 0) {
        switch (ped_exception_throw(
            ...
            _("%s : Bad checksum on block %llu of type %s."), __func__, block,
            _amiga_block_id(PED_BE32_TO_CPU(blk->amiga_ID)))) {
...
static uint32_t _amiga_find_rdb(const PedDevice *dev,
...
static int amiga_probe(const PedDevice *dev) {
    struct RigidDiskBlock *rdb;
    uint32_t found;
    PED_ASSERT(dev != NULL);
...
static PedDisk *amiga_alloc(const PedDevice *dev) {
    PedDisk *disk;
    struct RigidDiskBlock *rdb;
    PedSector cyl_size;
...
static PedDisk *amiga_duplicate(const PedDisk *disk) {
    PedDisk *new_disk;
...
static void amiga_free(PedDisk *disk) {
    PED_ASSERT(disk != NULL);
    PED_ASSERT(disk->disk_specific != NULL);
...
static int _amiga_loop_check(uint32_t block, uint32_t *blocklist,
                             uint32_t max) {
    uint32_t i;
...
static int amiga_read(PedDisk *disk) {
    struct RigidDiskBlock *rdb;
    ...
    for (i = 0; i < AMIGA_MAX_PARTITIONS; i++)
        partlist[i] = LINK_END;
    ...
    for (i = 1, partblock = PED_BE32_TO_CPU(rdb->rdb_PartitionList);
        ...
        end = (((PedSector)PED_BE32_TO_CPU(partition->de_HighCyl)) + 1) *
                  cylblocks -
              1;

        /* We can now construct a new partition */
        if (!(part = ped_partition_new(disk, PED_PARTITION_NORMAL, NULL, start,
                                       end))) {
            free(partition);
            return 0;
        }
...
static int _amiga_find_free_blocks(const PedDisk *disk, uint32_t *table,
                                   struct LinkedBlock *block, uint32_t first,
                                   uint32_t type) {
    PedSector next;
    ...
    for (next = first; next != LINK_END;
         next = PED_BE32_TO_CPU(block->lk_Next)) {
        ...
        if (PED_BE32_TO_CPU(block->lk_ID) != type) {
            switch (ped_exception_throw(
                PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                _("%s : The %s list seems bad at block %s."), __func__,
                _amiga_block_id(PED_BE32_TO_CPU(block->lk_ID)), next)) {
...
static uint32_t _GL_ATTRIBUTE_PURE _amiga_next_free_block(uint32_t *table,
                                                          uint32_t start,
                                                          uint32_t type) {
    int i;
...
static int amiga_write(const PedDisk *disk) {
    struct RigidDiskBlock *rdb;
    ...
    if ((rdb_num = _amiga_find_rdb(disk->dev, rdb)) == AMIGA_RDB_NOT_FOUND) {
        ...
        memset((char *)(RDSK(disk->disk_specific)) + pb_size, 0,
               PED_SECTOR_SIZE_DEFAULT - pb_size);
    } else {
    ...
    if (_amiga_find_free_blocks(disk, table, block,
                                ...
                                IDNAME_PARTITION) == 0) {
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                            _("%s : Failed to list partition blocks."),
                            __func__);
        goto error_free_table;
    }
    if (_amiga_find_free_blocks(disk, table, block,
                                PED_BE32_TO_CPU(rdb->rdb_FileSysHeaderList),
                                IDNAME_FILESYSHEADER) == 0) {
        ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                            _("%s : Failed to list file system blocks."),
                            __func__);
        goto error_free_table;
    }
    if (_amiga_find_free_blocks(disk, table, block,
    ...
    block_num = part_num =
        _amiga_next_free_block(table, rdb_num + 1, IDNAME_PARTITION);
    ...
    for (; part != NULL; part = next_part, block_num = next_num) {
        ...
        next_num =
            _amiga_next_free_block(table, block_num + 1, IDNAME_PARTITION);
        ...
        partition->de_HighCyl =
            PED_CPU_TO_BE32((part->geom.end + 1) / cylblocks - 1);
...
static PedPartition *amiga_partition_new(const PedDisk *disk,
    ...
    if (ped_partition_is_active(part)) {
        ...
        partition->de_BlocksPerTrack = PED_CPU_TO_BE32(dev->hw_geom.sectors);
...
static PedPartition *amiga_partition_duplicate(const PedPartition *part) {
...
static void amiga_partition_destroy(PedPartition *part) {
    PED_ASSERT(part != NULL);
...
static int amiga_partition_set_system(PedPartition *part,
                                      const PedFileSystemType *fs_type) {
    struct PartitionBlock *partition;
...
static int amiga_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                    int state) {
    struct PartitionBlock *partition;
    ...
    case PED_PARTITION_BOOT:
        if (state)
            partition->pb_Flags |= PED_CPU_TO_BE32(PBFF_BOOTABLE);
        else
            partition->pb_Flags &= ~(PED_CPU_TO_BE32(PBFF_BOOTABLE));
        return 1;
    case PED_PARTITION_HIDDEN:
        if (state)
            partition->pb_Flags |= PED_CPU_TO_BE32(PBFF_NOMOUNT);
        else
            partition->pb_Flags &= ~(PED_CPU_TO_BE32(PBFF_NOMOUNT));
        return 1;
...
static int _GL_ATTRIBUTE_PURE amiga_partition_get_flag(const PedPartition *part,
...
static int amiga_partition_is_flag_available(const PedPartition *part,
                                             PedPartitionFlag flag) {
    switch (flag) {
    case PED_PARTITION_BOOT:
    case PED_PARTITION_HIDDEN:
...
static void amiga_partition_set_name(PedPartition *part, const char *name) {
    struct PartitionBlock *partition;
...
static const char *amiga_partition_get_name(const PedPartition *part) {
    struct PartitionBlock *partition;
...
static PedAlignment *amiga_get_partition_alignment(const PedDisk *disk) {
...
static PedConstraint *_amiga_get_constraint(const PedDisk *disk) {
...
static int amiga_partition_align(PedPartition *part,
...
static int amiga_partition_enumerate(PedPartition *part) {
    int i;
    PedPartition *p;
...
static int amiga_alloc_metadata(PedDisk *disk) {
    PedPartition *new_part;
    ...
    new_part =
        ped_partition_new(disk, PED_PARTITION_METADATA, NULL, 0, MAX_RDB_BLOCK);
    if (!new_part)
        goto error;
...
static int amiga_get_max_primary_partition_count(const PedDisk *disk) {
    return AMIGA_MAX_PARTITIONS;
}

static bool amiga_get_max_supported_partition_count(const PedDisk *disk,
                                                    int *max_n) {
...
void ped_disk_amiga_done() { ped_disk_type_unregister(&amiga_disk_type); }
