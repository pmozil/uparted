...
static int dvh_probe(const PedDevice *dev) {
...
static PedDisk *dvh_alloc(const PedDevice *dev) {
  PedDisk *disk;
  ...
  disk->disk_specific = dvh_disk_data = ped_malloc(sizeof(DVHDiskData));
  if (!dvh_disk_data)
    goto error_free_disk;

  memset(&dvh_disk_data->dev_params, 0, sizeof(struct device_parameters));
  dvh_disk_data->swap = 0;
  dvh_disk_data->root = 0;
  dvh_disk_data->boot = 0;

  volume_part = ped_partition_new(disk, PED_PARTITION_EXTENDED, NULL, 0,
                                  PTYPE_VOLHDR_DFLTSZ - 1);
  if (!volume_part)
...
static PedDisk *dvh_duplicate(const PedDisk *disk) {
  PedDisk *new_disk;
  ...
  new_disk->disk_specific = new_dvh_disk_data = ped_malloc(sizeof(DVHDiskData));
...
static void dvh_free(PedDisk *disk) {
  free(disk->disk_specific);
  _ped_disk_free(disk);
}

/* two's complement 32-bit checksum */
static uint32_t _GL_ATTRIBUTE_PURE _checksum(const uint32_t *base,
                                             size_t size) {
  uint32_t sum = 0;
  size_t i;
...
static PedExceptionOption _handle_no_volume_header(PedDisk *disk) {
  PedExceptionOption ret;
  PedPartition *part;
  PedConstraint *constraint;

  switch (ped_exception_throw(
      PED_EXCEPTION_WARNING, PED_EXCEPTION_FIX + PED_EXCEPTION_CANCEL,
      _("%s has no extended partition (volume header partition)."),
      disk->dev->path)) {
  case PED_EXCEPTION_UNHANDLED:
  case PED_EXCEPTION_FIX:
  default:
    part = ped_partition_new(disk, PED_PARTITION_EXTENDED, NULL, 0,
                             PTYPE_VOLHDR_DFLTSZ - 1);
    if (!part)
      goto error;
...
static PedPartition *_parse_partition(PedDisk *disk,
  ...
  part = ped_partition_new(disk, pt->pt_type ? 0 : PED_PARTITION_EXTENDED, NULL,
...
static PedPartition *_parse_boot_file(PedDisk *disk,
  ...
  part = ped_partition_new(disk, PED_PARTITION_LOGICAL, NULL, start,
                           start + length / 512 - 1);
  if (!part)
    return NULL;
 ...
 */
static void _flush_stale_flags(const PedDisk *disk) {
  DVHDiskData *dvh_disk_data = disk->disk_specific;

  if (dvh_disk_data->root && !ped_disk_get_partition(disk, dvh_disk_data->root))
    dvh_disk_data->root = 0;
  if (dvh_disk_data->swap && !ped_disk_get_partition(disk, dvh_disk_data->swap))
    dvh_disk_data->swap = 0;
  if (dvh_disk_data->boot && !ped_disk_get_partition(disk, dvh_disk_data->boot))
    dvh_disk_data->boot = 0;
}

static int dvh_read(PedDisk *disk) {
  DVHDiskData *dvh_disk_data = disk->disk_specific;
  int i;
  struct volume_header vh;
  ...
  if (_checksum((uint32_t *)&vh, sizeof(struct volume_header))) {
    if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_IGNORE_CANCEL,
                            _("Checksum is wrong, indicating the partition "
                              "table is corrupt.")) == PED_EXCEPTION_CANCEL)
      return 0;
  }
...
#ifndef DISCOVER_ONLY
static void _generate_partition(PedPartition *part,
                                struct partition_table *pt) {
...
static void _generate_boot_file(PedPartition *part,
                                struct volume_directory *vd) {
  DVHPartData *dvh_part_data = part->disk_specific;
...
static int dvh_write(const PedDisk *disk) {
  ...
  vh.vh_dp.dp_cyls = PED_CPU_TO_BE16((short)disk->dev->bios_geom.cylinders);
  vh.vh_dp.dp_trks0 = PED_CPU_TO_BE16((short)disk->dev->bios_geom.heads);
  vh.vh_dp.dp_secs = PED_CPU_TO_BE16((short)disk->dev->bios_geom.sectors);
  ...
  for (i = 0; i < NVDIR; i++) {
    PedPartition *part = ped_disk_get_partition(disk, i + 1 + NPARTAB);
    if (part)
  ...
  vh.vh_csum = 0;
  vh.vh_csum =
      PED_CPU_TO_BE32(_checksum((uint32_t *)&vh, sizeof(struct volume_header)));

  return (ptt_write_sector(disk, &vh, sizeof vh) && ped_device_sync(disk->dev));
}
#endif /* !DISCOVER_ONLY */

static PedPartition *dvh_partition_new(const PedDisk *disk,
  ...
  dvh_part_data = part->disk_specific = ped_malloc(sizeof(DVHPartData));
  if (!dvh_part_data)
    goto error_free_part;

  dvh_part_data->type =
      (part_type == PED_PARTITION_EXTENDED) ? PTYPE_VOLHDR : PTYPE_RAW;
  strcpy(dvh_part_data->name, "");
...
static PedPartition *dvh_partition_duplicate(const PedPartition *part) {
  PedPartition *result;
  ...
  result_data = result->disk_specific = ped_malloc(sizeof(DVHPartData));
  if (!result_data)
    goto error_free_part;
...
static void dvh_partition_destroy(PedPartition *part) {
  if (ped_partition_is_active(part)) {
...
static int dvh_partition_set_system(PedPartition *part,
                                    const PedFileSystemType *fs_type) {
  DVHPartData *dvh_part_data = part->disk_specific;
...
static int dvh_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                  int state) {
...
static int _GL_ATTRIBUTE_PURE dvh_partition_get_flag(const PedPartition *part,
...
static int dvh_partition_is_flag_available(const PedPartition *part,
                                           PedPartitionFlag flag) {
  switch (flag) {
  case PED_PARTITION_ROOT:
  case PED_PARTITION_SWAP:
...
static void dvh_partition_set_name(PedPartition *part, const char *name) {
  DVHPartData *dvh_part_data = part->disk_specific;
...
static const char *dvh_partition_get_name(const PedPartition *part) {
  DVHPartData *dvh_part_data = part->disk_specific;
 ...
 */
static PedConstraint *_get_extended_constraint(PedDisk *disk) {
...
static PedConstraint *_get_primary_constraint(PedDisk *disk) {
...
static int dvh_partition_align(PedPartition *part,
...
static int dvh_partition_enumerate(PedPartition *part) {
  int i;
...
static int dvh_get_max_primary_partition_count(const PedDisk *disk) {
  return NPARTAB;
}

static bool dvh_get_max_supported_partition_count(const PedDisk *disk,
                                                  int *max_n) {
  *max_n = NPARTAB;
  return true;
}

static int dvh_alloc_metadata(PedDisk *disk) {
  PedPartition *part;
...
void ped_disk_dvh_init() {
  PED_ASSERT(sizeof(struct volume_header) == 512);

  ped_disk_type_register(&dvh_disk_type);
}

void ped_disk_dvh_done() { ped_disk_type_unregister(&dvh_disk_type); }
