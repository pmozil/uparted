...
static PedPartition *_parse_partition(PedDisk *disk,
    ...
    part = ped_partition_new(disk, pt->pt_type ? 0 : PED_PARTITION_EXTENDED,
                             NULL, start, start + length - 1);
    if (!part)
        return NULL;
...
static int dvh_write(const PedDisk *disk) {
    ...
    vh.vh_csum = 0;
    vh.vh_csum = PED_CPU_TO_BE32(
        _checksum((uint32_t *)&vh, sizeof(struct volume_header)));
