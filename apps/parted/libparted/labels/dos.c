    ...
    if (PED_LE16_TO_CPU(table->magic) != MSDOS_MAGIC) {
        if (ped_exception_throw(
                ...
                _("Invalid partition table on %s "
                  "-- wrong signature %x."),
                disk->dev->path,
                PED_LE16_TO_CPU(table->magic)) != PED_EXCEPTION_IGNORE)
            goto error;
        goto read_ok;
    }
#endif
...
static int write_ext_table(const PedDisk *disk, PedSector sector,
    ...
    if (part) {
        ...
        if (!geom)
            goto cleanup;
        partition_probe_bios_geometry(part, &bios_geom);
        fill_ext_raw_part_geom(&table->partitions[1], &bios_geom, geom,
                               lba_offset);
        ped_geometry_destroy(geom);
...
static int msdos_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
    ...
    if (p) {
        ...
        if (state) {
            ...
            // Clear the type so that fs_type will be used to return it to the
            // default
            dos_data->system = PARTITION_LINUX;
...
static int _GL_ATTRIBUTE_PURE msdos_partition_get_flag(const PedPartition *part,
    ...
    const struct flag_id_mapping_t *p = dos_find_flag_id_mapping(flag);
    if (p)
        return dos_data->system == p->type_id ||
               dos_data->system == p->alt_type_id;
...
static PedConstraint *_primary_constraint(const PedDisk *disk,
    ...
    return ped_constraint_new(&start_align, &end_align, &start_geom, &end_geom,
                              1, dev->length);
}
...
static int _logical_min_start_head(const PedPartition *part,
    ...
    if (is_start_ext_part)
        base_head =
            1 + (ext_part->geom.start % cylinder_size) / bios_geom->sectors;
    else
        base_head = 0;
...
static int _align_primary_no_geom(PedPartition *part,
    ...
    if (part->type == PED_PARTITION_EXTENDED) {
        ...
        solution = _try_constraint(
            part, constraint,
            _no_geom_constraint(disk, 1, disk->dev->length - 1));
    }
