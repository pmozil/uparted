...
static const char MBR_BOOT_CODE[] = {
    0xfa, 0xb8, 0x00, 0x10, 0x8e, 0xd0, 0xbc, 0x00, 0xb0, 0xb8, 0x00,
    0x00, 0x8e, 0xd8, 0x8e, 0xc0, 0xfb, 0xbe, 0x00, 0x7c, 0xbf, 0x00,
    0x06, 0xb9, 0x00, 0x02, 0xf3, 0xa4, 0xea, 0x21, 0x06, 0x00, 0x00,
    0xbe, 0xbe, 0x07, 0x38, 0x04, 0x75, 0x0b, 0x83, 0xc6, 0x10, 0x81,
    0xfe, 0xfe, 0x07, 0x75, 0xf3, 0xeb, 0x16, 0xb4, 0x02, 0xb0, 0x01,
    0xbb, 0x00, 0x7c, 0xb2, 0x80, 0x8a, 0x74, 0x01, 0x8b, 0x4c, 0x02,
    0xcd, 0x13, 0xea, 0x00, 0x7c, 0x00, 0x00, 0xeb, 0xfe};
...
static const unsigned char skip_set_system_types[] = {
    PARTITION_EXT_LBA,       PARTITION_DOS_EXT,   PARTITION_COMPAQ_DIAG,
    PARTITION_MSFT_RECOVERY, PARTITION_LINUX_LVM, PARTITION_LINUX_SWAP,
    PARTITION_LINUX_RAID,    PARTITION_PALO,      PARTITION_PREP,
    PARTITION_IRST,          PARTITION_ESP,       PARTITION_BLS_BOOT};
...
/**
 * Check whether the type_id supports the hidden flag. Returns true for both
 * hidden and non-hidden id.
 */
static bool dos_type_id_supports_hidden(unsigned char type_id) {
  switch (type_id) {
 ...
 */
static bool dos_type_id_is_hidden(unsigned char type_id) {
  switch (type_id) {
  case PARTITION_DOS_EXT_H:
  case PARTITION_FAT12_H:
  case PARTITION_FAT16_H:
 ...
 */
static bool dos_type_id_set_hidden(unsigned char *type_id, bool state) {
  PED_ASSERT(type_id);
...
/**
 * Check whether the type_id supports the lba flag. Returns true for both lba
 * and non-lba id.
 */
static bool dos_type_id_supports_lba(unsigned char type_id) {
  switch (type_id) {
  case PARTITION_FAT16:
  case PARTITION_FAT16_H:
  case PARTITION_FAT16_LBA:
 ...
 */
static bool dos_type_id_is_lba(unsigned char type_id) {
  switch (type_id) {
  case PARTITION_FAT16_LBA:
  case PARTITION_FAT16_LBA_H:
  case PARTITION_FAT32_LBA:
 ...
 */
static bool dos_type_id_set_lba(unsigned char *type_id, bool state) {
  PED_ASSERT(type_id);
...
static bool maybe_FAT(unsigned char const *s) {
...
static PedDisk *msdos_alloc(const PedDevice *dev) {
  PedDisk *disk;
  PED_ASSERT(dev != NULL);
...
static PedDisk *msdos_duplicate(const PedDisk *disk) {
  PedDisk *new_disk;
  ...
  memcpy(new_disk->disk_specific, disk->disk_specific, sizeof(DosDiskData));

  return new_disk;
}

static void msdos_free(PedDisk *disk) {
...
static int msdos_disk_set_flag(PedDisk *disk, PedDiskFlag flag, int state) {
  DosDiskData *disk_specific = disk->disk_specific;
  switch (flag) {
...
static int msdos_disk_get_flag(const PedDisk *disk, PedDiskFlag flag) {
  DosDiskData *disk_specific = disk->disk_specific;
  switch (flag) {
  case PED_DISK_CYLINDER_ALIGNMENT:
...
static int msdos_disk_is_flag_available(const PedDisk *disk, PedDiskFlag flag) {
  switch (flag) {
  case PED_DISK_CYLINDER_ALIGNMENT:
    return 1;
  default:
...
static int chs_get_cylinder(const RawCHS *chs) {
  return chs->cylinder + ((chs->sector >> 6) << 8);
}

static int chs_get_head(const RawCHS *chs) { return chs->head; }

/* counts from 0 */
static int chs_get_sector(const RawCHS *chs) {
  return (chs->sector & 0x3f) - 1;
}

static PedSector _GL_ATTRIBUTE_PURE chs_to_sector(
    const PedDevice *dev, const PedCHSGeometry *bios_geom, const RawCHS *chs) {
...
static void sector_to_chs(const PedDevice *dev, const PedCHSGeometry *bios_geom,
...
static PedSector _GL_ATTRIBUTE_PURE linear_start(
    const PedDisk *disk, const DosRawPartition *raw_part, PedSector offset) {
  PED_ASSERT(disk != NULL);
  PED_ASSERT(raw_part != NULL);
...
static PedSector _GL_ATTRIBUTE_PURE linear_end(const PedDisk *disk,
                                               const DosRawPartition *raw_part,
                                               PedSector offset) {
  PED_ASSERT(disk != NULL);
  PED_ASSERT(raw_part != NULL);
...
static int probe_filesystem_for_geom(const PedPartition *part,
 ...
 */
static int probe_partition_for_geom(const PedPartition *part,
                                    PedCHSGeometry *bios_geom) {
  DosPartitionData *dos_data;
  RawCHS *start_chs;
  RawCHS *end_chs;
...
static void partition_probe_bios_geometry(const PedPartition *part,
                                          PedCHSGeometry *bios_geom) {
  PED_ASSERT(part != NULL);
  PED_ASSERT(part->disk != NULL);
  PED_ASSERT(bios_geom != NULL);
...
static void disk_probe_bios_geometry(const PedDisk *disk,
                                     PedCHSGeometry *bios_geom) {
  PedPartition *part;
...
static PedPartition *raw_part_parse(const PedDisk *disk,
                                    const DosRawPartition *raw_part,
  ...
  part = ped_partition_new(disk, type, NULL,
...
static int read_table(PedDisk *disk, PedSector sector, int is_extended_table) {
  int i;
  DosRawTable *table;
  DosRawPartition *raw_part;
  PedPartition *part;
  ...
  if (PED_LE16_TO_CPU(table->magic) != MSDOS_MAGIC) {
    if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_IGNORE_CANCEL,
                            _("Invalid partition table on %s "
                              "-- wrong signature %x."),
                            disk->dev->path, PED_LE16_TO_CPU(table->magic)) !=
...
#ifndef DISCOVER_ONLY
static int fill_raw_part(DosRawPartition *raw_part, const PedPartition *part,
                         PedSector offset) {
  DosPartitionData *dos_data;
  PedCHSGeometry bios_geom;
...
static int fill_ext_raw_part_geom(DosRawPartition *raw_part,
...
static int write_ext_table(const PedDisk *disk, PedSector sector,
                           const PedPartition *logical) {
  PedPartition *part;
  PedSector lba_offset;
  ...
  if (part) {
    ...
    if (!geom)
      goto cleanup;
    partition_probe_bios_geometry(part, &bios_geom);
    fill_ext_raw_part_geom(&table->partitions[1], &bios_geom, geom, lba_offset);
    ped_geometry_destroy(geom);
...
static int write_empty_table(const PedDisk *disk, PedSector sector) {
  DosRawTable table;
  void *table_sector;
 ...
 */
static int write_extended_partitions(const PedDisk *disk) {
  PedPartition *ext_part;
  PedPartition *part;
  PedCHSGeometry bios_geom;
...
static int msdos_write(const PedDisk *disk) {
  PedPartition *part;
  int i;
...
static PedPartition *msdos_partition_new(const PedDisk *disk,
  ...
  if (ped_partition_is_active(part)) {
    part->disk_specific = dos_data = ped_calloc(sizeof(DosPartitionData));
    if (!dos_data)
...
static PedPartition *msdos_partition_duplicate(const PedPartition *part) {
...
static void msdos_partition_destroy(PedPartition *part) {
  PED_ASSERT(part != NULL);
 ...
 */
static bool is_skip_type(unsigned char type_id) {
...
static int msdos_partition_set_system(PedPartition *part,
                                      const PedFileSystemType *fs_type) {
  ...
  else if (!strcmp(fs_type->name, "ntfs") || !strcmp(fs_type->name, "hpfs"))
    dos_data->system = PARTITION_NTFS;
  else if (!strcmp(fs_type->name, "hfs") || !strcmp(fs_type->name, "hfs+"))
...
static int msdos_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                    int state) {
  PED_ASSERT(part != NULL);
  PED_ASSERT(part->disk_specific != NULL);
  PED_ASSERT(part->disk != NULL);
  ...
  if (p) {
    ...
    } else if (dos_data->system == p->type_id ||
               dos_data->system == p->alt_type_id) {
...
static int _GL_ATTRIBUTE_PURE msdos_partition_get_flag(const PedPartition *part,
...
static int msdos_partition_is_flag_available(const PedPartition *part,
                                             PedPartitionFlag flag) {
...
static PedGeometry *_try_constraint(const PedPartition *part,
...
static PedGeometry *_best_solution(const PedPartition *part,
                                   const PedCHSGeometry *bios_geom,
 ...
 */
static PedConstraint *_primary_constraint(const PedDisk *disk,
                                          const PedCHSGeometry *bios_geom,
  ...
  if (min_geom) {
    ...
    PedSector start = (cylinder_size < dev->length ? cylinder_size : 1);
    if (!ped_geometry_init(&start_geom, dev, start, dev->length - start))
      return NULL;
    if (!ped_geometry_init(&end_geom, dev, 0, dev->length))
      return NULL;
  }

  return ped_constraint_new(&start_align, &end_align, &start_geom, &end_geom, 1,
                            dev->length);
}
 ...
 */
static PedConstraint *_primary_start_constraint(const PedDisk *disk,
 ...
 */
static PedConstraint *_logical_constraint(const PedDisk *disk,
                                          const PedCHSGeometry *bios_geom,
                                          PedSector start_offset,
                                          int is_start_part) {
  ...
  if (!ped_alignment_init(&end_align, -1, cylinder_size))
    return NULL;
  if (is_start_part) {
    if (!ped_geometry_init(&max_geom, dev, ext_part->geom.start,
    ...
    min_start = ped_round_up_to(ext_part->geom.start + 1, cylinder_size);
  ...
  return ped_constraint_new(&start_align, &end_align, &max_geom, &max_geom, 1,
                            dev->length);
}
...
_get_min_extended_part_geom(const PedPartition *ext_part,
  ...
  for (walk = ext_part->part_list; walk; walk = walk->next) {
    ...
    if (walk->geom.start < min_geom->start)
      ped_geometry_set_start(min_geom, walk->geom.start - 2 * head_size);
...
static int _align_primary(PedPartition *part, const PedCHSGeometry *bios_geom,
  ...
  solution =
      _best_solution(part, bios_geom, solution,
                     _try_constraint(part, constraint,
                                     _primary_start_constraint(
                                         disk, part, bios_geom, min_geom)));

  solution = _best_solution(
      part, bios_geom, solution,
      _try_constraint(part, constraint,
                      _primary_constraint(disk, bios_geom, min_geom)));
...
static int _logical_min_start_head(const PedPartition *part,
                                   const PedCHSGeometry *bios_geom,
  ...
  if (is_start_ext_part)
    base_head = 1 + (ext_part->geom.start % cylinder_size) / bios_geom->sectors;
  else
    base_head = 0;
 ...
 * those cases.
 */
static PedConstraint *_log_meta_overlap_constraint(PedPartition *part,
  ...
  ped_geometry_init(&safe_space, part->disk->dev, min_start,
                    max_end - min_start + 1);
  return ped_constraint_new_from_max(&safe_space);
}

static int _align_logical(PedPartition *part, const PedCHSGeometry *bios_geom,
                          const PedConstraint *constraint) {
  PedDisk *disk = part->disk;
  PedPartition *ext_part = ped_disk_extended_partition(disk);
  ...
  for (head = _logical_min_start_head(part, bios_geom, ext_part, 0);
    ...
    if (head >= _logical_min_start_head(part, bios_geom, ext_part, 1))
      disk_constraint = _logical_constraint(disk, bios_geom, start, 1);
    else
      disk_constraint = _logical_constraint(disk, bios_geom, start, 0);

    solution =
        _best_solution(part, bios_geom, solution,
                       _try_constraint(part, intersect, disk_constraint));
  }
...
static int _align(PedPartition *part, const PedCHSGeometry *bios_geom,
                  const PedConstraint *constraint) {
  if (part->type == PED_PARTITION_LOGICAL)
...
static PedConstraint *_no_geom_constraint(const PedDisk *disk, PedSector start,
                                          PedSector end) {
  PedGeometry max;
...
static PedConstraint *_no_geom_extended_constraint(const PedPartition *part) {
  ...
  if (min) {
    ...
    ped_geometry_init(&end_range, dev, min->end, dev->length - min->end);
...
static int _align_primary_no_geom(PedPartition *part,
                                  const PedConstraint *constraint) {
  PedDisk *disk = part->disk;
  PedGeometry *solution;
  ...
  if (part->type == PED_PARTITION_EXTENDED) {
    solution =
        _try_constraint(part, constraint, _no_geom_extended_constraint(part));
  } else {
    solution = _try_constraint(
        part, constraint, _no_geom_constraint(disk, 1, disk->dev->length - 1));
  }
...
static int _align_logical_no_geom(PedPartition *part,
                                  const PedConstraint *constraint) {
  PedGeometry *solution;
...
static int _align_no_geom(PedPartition *part, const PedConstraint *constraint) {
  if (part->type == PED_PARTITION_LOGICAL)
...
static int msdos_partition_align(PedPartition *part,
                                 const PedConstraint *constraint) {
  PedCHSGeometry bios_geom;
  DosPartitionData *dos_data;
  ...
  if (dos_data->system == PARTITION_LDM && dos_data->orig) {
    ...
    ped_geometry_set(&part->geom, orig_geom->start, orig_geom->length);
...
static int add_metadata_part(PedDisk *disk, PedPartitionType type,
                             PedSector start, PedSector end) {
  PedPartition *new_part;
  ...
  new_part =
      ped_partition_new(disk, type | PED_PARTITION_METADATA, NULL, start, end);
  if (!new_part)
    goto error;
 ...
 * tables, etc.
 */
static int add_logical_part_metadata(PedDisk *disk,
                                     const PedPartition *log_part) {
  PedPartition *ext_part = ped_disk_extended_partition(disk);
  ...
  if (prev)
    ...
    metadata_start = PED_MAX(metadata_start, ext_part->geom.start + 1);
  ...
  return add_metadata_part(disk, PED_PARTITION_LOGICAL, metadata_start,
                           metadata_end);
}
 ...
 */
static int get_start_first_nonfree_part(const PedDisk *disk,
                                        PedSector *sector) {
  PedPartition *walk;
 ...
 * If there is no non-free partition, don't modify *SECTOR and return 0.
 */
static int get_end_last_nonfree_part(const PedDisk *disk, PedSector *sector) {
 ...
 */
static int add_startend_metadata(PedDisk *disk) {
  PedDevice *dev = disk->dev;
  ...
  if (!disk_specific->cylinder_alignment)
    ...
    final_start =
        PED_MAX(final_start + 1, ped_round_down_to(dev->length, cyl_size));
  final_end = dev->length - 1;

  // Create the metadata partitions.
  // init_end <= dev->length for devices that are _real_ small.
  if (init_start < init_end && init_end <= dev->length &&
      !add_metadata_part(disk, PED_PARTITION_NORMAL, init_start, init_end))
    return 0;

  // init_end < final_start so they dont overlap.  For very small devs.
  if (final_start < final_end && init_end < final_start &&
      final_end <= dev->length &&
      !add_metadata_part(disk, PED_PARTITION_NORMAL, final_start, final_end))
    return 0;
...
static int msdos_alloc_metadata(PedDisk *disk) {
  ...
  if (ext_part) {
    ...
    if (ext_part->part_list)
      end = PED_MIN(end, ext_part->part_list->geom.start - 1);
    if (!add_metadata_part(disk, PED_PARTITION_LOGICAL, start, end))
      return 0;
  }
...
static int next_primary(const PedDisk *disk) {
  int i;
  for (i = 1; i <= DOS_N_PRI_PARTITIONS; i++) {
...
static int _GL_ATTRIBUTE_PURE next_logical(const PedDisk *disk) {
  int i;
  ...
  ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                      _("cannot create any more partitions"), disk->dev->path);
  return -1;
}

static int msdos_partition_enumerate(PedPartition *part) {
  PED_ASSERT(part != NULL);
  PED_ASSERT(part->disk != NULL);
...
static int msdos_get_max_primary_partition_count(const PedDisk *disk) {
  return DOS_N_PRI_PARTITIONS;
}

static bool msdos_get_max_supported_partition_count(const PedDisk *disk,
                                                    int *max_n) {
...
void ped_disk_msdos_done() { ped_disk_type_unregister(&msdos_disk_type); }
