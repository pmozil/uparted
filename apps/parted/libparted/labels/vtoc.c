...
static void vtoc_error(enum failure why, char const *s1, char const *s2) {
  PDEBUG
  char error[8192];

  switch (why) {
  case unable_to_open:
    sprintf(error, "VTOC: %s -- %s\n%s\n", _("opening of device failed"), s1,
            s2);
    break;
  case unable_to_seek:
    sprintf(error, "VTOC: %s -- %s\n%s\n", _("seeking on device failed"), s1,
            s2);
    break;
  case unable_to_write:
    sprintf(error, "VTOC: %s -- %s\n%s\n", _("writing to device failed"), s1,
            s2);
    break;
  case unable_to_read:
    sprintf(error, "VTOC: %s -- %s\n%s\n", _("reading from device failed"), s1,
            s2);
    break;
  default:
...
u_int64_t cchhb2blk(cchhb_t *p, struct fdasd_hd_geometry *geo) {
  return (u_int64_t)vtoc_get_cyl_from_cchhb(p) * geo->heads * geo->sectors +
         vtoc_get_head_from_cchhb(p) * geo->sectors + p->b;
}

u_int64_t cchh2blk(cchh_t *p, struct fdasd_hd_geometry *geo) {
  return (u_int64_t)vtoc_get_cyl_from_cchh(p) * geo->heads * geo->sectors +
         vtoc_get_head_from_cchh(p) * geo->sectors;
}

u_int32_t cchh2trk(cchh_t *p, struct fdasd_hd_geometry *geo) {
  return vtoc_get_cyl_from_cchh(p) * geo->heads + vtoc_get_head_from_cchh(p);
}
 ...
 */
int vtoc_read_volume_label(int f, unsigned long vlabel_start,
 ...
 */
int vtoc_write_volume_label(int f, unsigned long vlabel_start,
 ...
 */
void vtoc_read_label(int f, unsigned long position, format1_label_t *f1,
 ...
 */
void vtoc_write_label(int f, unsigned long position, format1_label_t const *f1,
                      format4_label_t const *f4, format5_label_t const *f5,
                      format7_label_t const *f7, format9_label_t const *f9) {
  PDEBUG
  int t;
 ...
 */
void vtoc_init_format4_label(format4_label_t *f4, unsigned int compat_cylinders,
 ...
 */
void vtoc_init_format_1_8_label(unsigned int blksize, extent_t *part_extent,
                                format1_label_t *f1) {
  PDEBUG
  struct tm *creatime;
  time_t t;
...
void vtoc_init_format1_label(unsigned int blksize, extent_t *part_extent,
                             format1_label_t *f1) {
  vtoc_init_format_1_8_label(blksize, part_extent, f1);
  f1->DS1FMTID = 0xf1;
}

void vtoc_init_format8_label(unsigned int blksize, extent_t *part_extent,
                             format1_label_t *f8) {
 ...
 */
static void vtoc_reorganize_FMT5_extents(format5_label_t *f5) {
  PDEBUG
 ...
 */
void vtoc_update_format5_label_add(format5_label_t *f5, int verbose, int trk,
                                   u_int16_t a, u_int16_t b, u_int8_t c) {
  PDEBUG
 ...
 */
void vtoc_update_format5_label_del(format5_label_t *f5, int verbose, int trk,
                                   u_int16_t a, u_int16_t b, u_int8_t c) {
  PDEBUG
  ds5ext_t *ext;
  int i, counter = 0;
 ...
 */
static void vtoc_reorganize_FMT7_extents(format7_label_t *f7) {
  PDEBUG
