...
static void sun_compute_checksum(SunRawLabel *label) {
...
static int sun_verify_checksum(SunRawLabel const *label) {
...
static PedDisk *sun_alloc(const PedDevice *dev) {
  PedDisk *disk;
  SunRawLabel *label;
  ...
  snprintf(label->info, sizeof(label->info) - 1,
           ...
           PED_BE16_TO_CPU(label->ncyl), PED_BE16_TO_CPU(label->nacyl),
           PED_BE16_TO_CPU(label->ntrks), PED_BE16_TO_CPU(label->nsect));
...
static PedDisk *sun_duplicate(const PedDisk *disk) {
  PedDisk *new_disk;
...
static void sun_free(PedDisk *disk) {
  free(disk->disk_specific);
  _ped_disk_free(disk);
}

static int _check_geometry_sanity(PedDisk *disk, SunRawLabel *label) {
  ...
  if (!!PED_BE16_TO_CPU(label->pcylcount) * !!PED_BE16_TO_CPU(label->ntrks) *
  ...
  if (PED_BE16_TO_CPU(label->nsect) != dev->bios_geom.sectors ||
    ...
    if (ped_exception_throw(
            ...
            _("The disk CHS geometry (%d,%d,%d) reported "
              ...
              "(%d,%d,%d)."),
            dev->bios_geom.cylinders, dev->bios_geom.heads,
            dev->bios_geom.sectors, PED_BE16_TO_CPU(label->pcylcount),
            PED_BE16_TO_CPU(label->ntrks),
            PED_BE16_TO_CPU(label->nsect)) == PED_EXCEPTION_CANCEL)
      return 0;
#endif
...
static int sun_read(PedDisk *disk) {
  ...
  for (i = 0; i < SUN_DISK_MAXPARTITIONS; i++) {
    ...
    start = PED_BE32_TO_CPU(label->partitions[i].start_cylinder) * block;
...
#ifndef DISCOVER_ONLY
static int _use_old_info(const PedDisk *disk, const void *sector_0) {
  ...
  if (old_label->info[0] &&
    ...
    memcpy(&sun_specific->raw_label, sector_0, sizeof(sun_specific->raw_label));
...
static int sun_write(const PedDisk *disk) {
  SunRawLabel *label;
  SunPartitionData *sun_data;
  SunDiskData *disk_data;
  PedPartition *part;
  ...
  memset(label->infos, 0, sizeof(SunPartitionInfo) * SUN_DISK_MAXPARTITIONS);
  ...
  for (i = 0; i < SUN_DISK_MAXPARTITIONS; i++) {
    ...
    if (!part && i == WHOLE_DISK_PART) {
      ...
      label->partitions[i].num_sectors = PED_CPU_TO_BE32(disk_data->length);
      continue;
    }
    if (!part)
      continue;
    ...
    label->partitions[i].start_cylinder =
        PED_CPU_TO_BE32(part->geom.start / (disk->dev->bios_geom.sectors *
                                            disk->dev->bios_geom.heads));
    label->partitions[i].num_sectors =
        PED_CPU_TO_BE32(part->geom.end - part->geom.start + 1);
  }
...
static PedPartition *sun_partition_new(const PedDisk *disk,
  ...
  if (ped_partition_is_active(part)) {
    part->disk_specific = sun_data = ped_malloc(sizeof(SunPartitionData));
    if (!sun_data)
...
static PedPartition *sun_partition_duplicate(const PedPartition *part) {
...
static int sun_partition_set_system(PedPartition *part,
                                    const PedFileSystemType *fs_type) {
...
static int sun_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                  int state) {
  SunPartitionData *sun_data;
...
static int _GL_ATTRIBUTE_PURE sun_partition_get_flag(const PedPartition *part,
...
static int sun_partition_is_flag_available(const PedPartition *part,
                                           PedPartitionFlag flag) {
  switch (flag) {
  case PED_PARTITION_BOOT:
  case PED_PARTITION_ROOT:
...
static bool sun_get_max_supported_partition_count(const PedDisk *disk,
                                                  int *max_n) {
...
static PedAlignment *sun_get_partition_alignment(const PedDisk *disk) {
  PedSector block = disk->dev->hw_geom.sectors * disk->dev->hw_geom.heads;

  return ped_alignment_new(0, block);
}

static PedConstraint *_get_strict_constraint(PedDisk *disk) {
  PedDevice *dev = disk->dev;
...
static PedConstraint *_get_lax_constraint(PedDisk *disk) {
  PedDevice *dev = disk->dev;
  PedAlignment start_align;
  PedGeometry max_geom;
  SunDiskData *disk_data = disk->disk_specific;
...
static int sun_alloc_metadata(PedDisk *disk) {
  ...
  if (disk->dev->length <= 0 || disk_data->length <= 0 ||
...
static PedDiskType sun_disk_type =
    {next : NULL, name : "sun", ops : &sun_disk_ops, features : 0};
...
void ped_disk_sun_done() { ped_disk_type_unregister(&sun_disk_type); }
