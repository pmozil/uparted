...
static void sector_to_chs(const PedDevice *dev, PedSector sector, int *c,
                          int *h, int *s);

/* magic(?) check */
static int pc98_check_magic(const PC98RawTable *part_table) {
...
static int pc98_check_ipl_signature(const PC98RawTable *part_table) {
...
static PedDisk *pc98_alloc(const PedDevice *dev) {
    PED_ASSERT(dev != NULL);

    return _ped_disk_alloc(dev, &pc98_disk_type);
}

static PedDisk *pc98_duplicate(const PedDisk *disk) {
    return ped_disk_new_fresh(disk->dev, &pc98_disk_type);
}

static void pc98_free(PedDisk *disk) {
    PED_ASSERT(disk != NULL);

    _ped_disk_free(disk);
}

static PedSector _GL_ATTRIBUTE_PURE chs_to_sector(const PedDevice *dev, int c,
                                                  int h, int s) {
    PED_ASSERT(dev != NULL);
...
legacy_end(const PedDisk *disk, const PC98RawPartition *raw_part) {
    ...
    if (raw_part->end_head == 0 && raw_part->end_sector == 0) {
        return chs_to_sector(disk->dev, PED_LE16_TO_CPU(raw_part->end_cyl),
...
static int is_unused_partition(const PC98RawPartition *raw_part) {
    if (raw_part->mid || raw_part->sid || raw_part->ipl_sect ||
        raw_part->ipl_head || PED_LE16_TO_CPU(raw_part->ipl_cyl) ||
        raw_part->sector || raw_part->head || PED_LE16_TO_CPU(raw_part->cyl) ||
        raw_part->end_sector || raw_part->end_head ||
        PED_LE16_TO_CPU(raw_part->end_cyl))
        return 0;
    return 1;
}

static int read_table(PedDisk *disk) {
    int i;
    PC98RawTable table;
    ...
    if (!pc98_check_magic(&table)) {
        if (ped_exception_throw(
                ...
                _("Invalid partition table on %s."), disk->dev->path))
            goto error;
    }
    ...
    for (i = 0; i < MAX_PART_COUNT; i++) {
        ...
        part = ped_partition_new(disk, PED_PARTITION_NORMAL, NULL, part_start,
                                 part_end);
        if (!part)
            goto error;
        ...
        pc98_data->ipl_sector =
            chs_to_sector(disk->dev, PED_LE16_TO_CPU(raw_part->ipl_cyl),
        ...
        if (part->geom.start != part_start || part->geom.end != part_end) {
...
static int fill_raw_part(PC98RawPartition *raw_part, const PedPartition *part) {
    PC98PartitionData *pc98_data;
    int c, h, s;
    const char *name;
    ...
    if (pc98_data->ipl_sector) {
        sector_to_chs(part->disk->dev, pc98_data->ipl_sector, &c, &h, &s);
...
static int pc98_write(const PedDisk *disk) {
    PedPartition *part;
    int i;
...
static PedPartition *pc98_partition_new(const PedDisk *disk,
    ...
    if (ped_partition_is_active(part)) {
        part->disk_specific = pc98_data = ped_malloc(sizeof(PC98PartitionData));
        if (!pc98_data)
...
static PedPartition *pc98_partition_duplicate(const PedPartition *part) {
...
static int pc98_partition_set_system(PedPartition *part,
                                     const PedFileSystemType *fs_type) {
...
static int pc98_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                   int state) {
    PC98PartitionData *pc98_data;
...
static int _GL_ATTRIBUTE_PURE pc98_partition_get_flag(const PedPartition *part,
...
static int pc98_partition_is_flag_available(const PedPartition *part,
                                            PedPartitionFlag flag) {
    switch (flag) {
    case PED_PARTITION_HIDDEN:
    case PED_PARTITION_BOOT:
...
static PedAlignment *pc98_get_partition_alignment(const PedDisk *disk) {
...
static int next_primary(PedDisk *disk) {
    int i;
    for (i = 1; i <= MAX_PART_COUNT; i++) {
...
static int pc98_alloc_metadata(PedDisk *disk) {
    ...
    new_part =
        ped_partition_new(disk, PED_PARTITION_METADATA, NULL, 0, cyl_size - 1);
    if (!new_part)
        goto error;
...
static bool pc98_get_max_supported_partition_count(const PedDisk *disk,
                                                   int *max_n) {
...
void ped_disk_pc98_done() { ped_disk_type_unregister(&pc98_disk_type); }
