...
static int _disk_add_part_map_entry(PedDisk *disk, int warn) {
    ...
    new_part = ped_partition_new(disk, PED_PARTITION_NORMAL, NULL, 1,
                                 part_map_size - 1);
    if (!new_part)
        goto error;
...
static PedPartition *_rawpart_analyse(MacRawPartition *raw_part, PedDisk *disk,
    ...
    if (mac_part_data->is_driver)
        mac_part_data->has_driver =
            _rawpart_has_driver(raw_part, mac_disk_data);
        ...
        if (mac_part_data->boot_region_length < part->geom.length) {
            if (ped_exception_throw(
                    ...
                    _("The partition's boot region doesn't occupy "
                      "the entire partition.")) != PED_EXCEPTION_IGNORE)
                goto error_destroy_part;
        }
    } else {
...
static int add_metadata_part(PedDisk *disk, PedSector start, PedSector end) {
    ...
    new_part =
        ped_partition_new(disk, PED_PARTITION_METADATA, NULL, start, end);
    if (!new_part)
        goto error;
    if (!ped_disk_add_partition(disk, new_part, constraint_any))
        goto error_destroy_new_part;
