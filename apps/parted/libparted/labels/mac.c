...
static int _check_signature(MacRawDisk const *raw_disk) {
  if (PED_BE16_TO_CPU(raw_disk->signature) != MAC_DISK_MAGIC) {
#ifdef DISCOVER_ONLY
    return 0;
#else
    return ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_IGNORE_CANCEL,
...
static int _disk_add_part_map_entry(PedDisk *disk, int warn) {
  ...
  if (warn &&
      ped_exception_throw(
          PED_EXCEPTION_ERROR, PED_EXCEPTION_FIX | PED_EXCEPTION_CANCEL,
          _("Partition map has no partition map entry!")) != PED_EXCEPTION_FIX)
    goto error;
#endif /* !DISCOVER_ONLY */

  part_map_size = ped_round_up_to(mac_disk_data->last_part_entry_num, 64);
  if (part_map_size == 0)
    part_map_size = 64;

  new_part =
      ped_partition_new(disk, PED_PARTITION_NORMAL, NULL, 1, part_map_size - 1);
  if (!new_part)
    goto error;
...
static PedDisk *mac_alloc(const PedDevice *dev) {
  PedDisk *disk;
  ...
  if (dev->length < 256) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                        _("%s is too small for a Mac disk label!"), dev->path);
    goto error;
  }
#endif
...
static PedDisk *mac_duplicate(const PedDisk *disk) {
  PedDisk *new_disk;
...
static void mac_free(PedDisk *disk) {
  MacDiskData *mac_disk_data = disk->disk_specific;
...
static int _rawpart_cmp_type(const MacRawPartition *raw_part,
                             const char *type) {
  return strncasecmp(raw_part->type, type, 32) == 0;
}

static int _rawpart_cmp_name(const MacRawPartition *raw_part,
                             const char *name) {
  return strncasecmp(raw_part->name, name, 32) == 0;
}

static int _rawpart_is_partition_map(const MacRawPartition *raw_part) {
  return _rawpart_cmp_type(raw_part, "Apple_partition_map");
}

static int strncasestr(const char *haystack, const char *needle, int n) {
...
static int _rawpart_is_boot(const MacRawPartition *raw_part) {
...
static int _rawpart_is_driver(const MacRawPartition *raw_part) {
...
static int _GL_ATTRIBUTE_PURE _rawpart_has_driver(
    const MacRawPartition *raw_part, MacDiskData *mac_disk_data) {
...
static int _rawpart_is_root(MacRawPartition *raw_part) {
...
static int _rawpart_is_swap(MacRawPartition *raw_part) {
  if (!_rawpart_cmp_type(raw_part, "Apple_UNIX_SVR2"))
    return 0;
...
static int _rawpart_is_lvm(MacRawPartition *raw_part) {
  if (strcmp(raw_part->type, "Linux_LVM") != 0)
    return 0;
  return 1;
}

static int _rawpart_is_raid(MacRawPartition *raw_part) {
  if (strcmp(raw_part->type, "Linux_RAID") != 0)
    return 0;
  return 1;
}

static int _rawpart_is_void(MacRawPartition *raw_part) {
 ...
 * partition.
 */
static int _rawpart_is_active(MacRawPartition *raw_part) {
  if (_rawpart_cmp_type(raw_part, "Apple_Free") &&
      (_rawpart_cmp_name(raw_part, "Extra") || _rawpart_cmp_name(raw_part, "")))
    return 0;
...
static PedPartition *_rawpart_analyse(MacRawPartition *raw_part, PedDisk *disk,
                                      int num) {
  MacDiskData *mac_disk_data;
  PedPartition *part;
  MacPartitionData *mac_part_data;
...
#ifndef DISCOVER_ONLY
    if (ped_exception_throw(PED_EXCEPTION_WARNING, PED_EXCEPTION_IGNORE_CANCEL,
                            _("Partition %d has an invalid signature %x."), num,
  ...
  part = ped_partition_new(disk, PED_PARTITION_NORMAL, NULL, start,
                           start + length - 1);
  if (!part)
    goto error;
...
#endif /* !DISCOVER_ONLY */
  mac_part_data->data_region_length = PED_BE32_TO_CPU(raw_part->data_count);

  mac_part_data->boot_region_length = PED_BE32_TO_CPU(raw_part->boot_count);
  ...
  mac_part_data->boot_base_address = PED_BE32_TO_CPU(raw_part->boot_load);
  mac_part_data->boot_entry_address = PED_BE32_TO_CPU(raw_part->boot_entry);
  mac_part_data->boot_checksum = PED_BE32_TO_CPU(raw_part->boot_cksum);
 ...
 */
static int _rawpart_get_partmap_size(MacRawPartition *raw_part, PedDisk *disk) {
  MacDiskData *mac_disk_data = disk->disk_specific;
...
static int _disk_analyse_block_size(PedDisk *disk, MacRawDisk *raw_disk) {
  ...
  if (block_size != disk->dev->sector_size / 512) {
    ...
    if (ped_exception_throw(
            ...
            (int)block_size * 512,
            (int)disk->dev->sector_size) != PED_EXCEPTION_IGNORE)
      goto error;
#endif
 ...
 */
static int _disk_analyse_ghost_size(PedDisk *disk) {
  MacDiskData *mac_disk_data = disk->disk_specific;
  ...
  for (i = 1; i < 64; i *= 2) {
    if (!ped_device_read(disk->dev, buf, i, 1))
      break;
    if (_rawpart_check_signature(buf) && !_rawpart_is_void(buf)) {
...
static int mac_read(PedDisk *disk) {
  MacDiskData *mac_disk_data;
  PedPartition *part;
  int num;
  ...
  for (num = 1; num == 1 || num <= last_part_entry_num; num++) {
    void *raw_part = buf;
    if (!ped_device_read(disk->dev, raw_part, num * ghost_size, 1))
    ...
    if (num == 1)
      last_part_entry_num = _rawpart_get_partmap_size(raw_part, disk);
    if (_rawpart_get_partmap_size(raw_part, disk) != last_part_entry_num) {
      if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_IGNORE_CANCEL,
      ...
      if (mac_disk_data->part_map_entry_num &&
          ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_IGNORE_CANCEL,
                              _("Weird!  There are 2 partitions "
                                "map entries!")) != PED_EXCEPTION_IGNORE)
        goto error_delete_all;

      mac_disk_data->part_map_entry_num = num;
      mac_disk_data->part_map_entry_count = part->geom.end - ghost_size + 1;
    }
  }
 ...
 */
static int _pad_raw_part(PedDisk *disk, int num, MacRawPartition *part_map) {
  MacDiskData *mac_disk_data = disk->disk_specific;
  int i;
  ...
  ghost_entry->map_count = PED_CPU_TO_BE32(mac_disk_data->last_part_entry_num);
...
static void _update_driver_count(MacRawPartition *part_map_entry,
                                 MacDiskData *mac_driverdata,
                                 const MacDiskData *mac_disk_data) {
  uint16_t i;
  uint32_t start_block, block_count;
...
static MacRawPartition *get_pme(MacRawPartition const *part_map, PedSector i,
                                PedDisk const *disk) {
  MacDiskData const *mac_disk_data = disk->disk_specific;
  PedSector idx = i * mac_disk_data->ghost_size - 1;
  return (MacRawPartition *)((char *)part_map + idx * disk->dev->sector_size);
}

/* Initialize the disk->dev->sector_size bytes of part_map[part->num]. */
static int _generate_raw_part(PedDisk *disk, PedPartition *part,
  ...
  if (mac_part_data->is_driver) {
    if (mac_part_data->has_driver)
      _update_driver_count(part_map_entry, mac_driverdata, mac_disk_data);
  } else
    mac_part_data->data_region_length = part->geom.length;
  part_map_entry->data_count =
      PED_CPU_TO_BE32(mac_part_data->data_region_length);
  part_map_entry->boot_count =
      PED_CPU_TO_BE32(mac_part_data->boot_region_length);
  part_map_entry->status = PED_CPU_TO_BE32(mac_part_data->status);
  part_map_entry->driver_sig = PED_CPU_TO_BE32(mac_part_data->driver_sig);

  part_map_entry->boot_load = PED_CPU_TO_BE32(mac_part_data->boot_base_address);
  part_map_entry->boot_entry =
      PED_CPU_TO_BE32(mac_part_data->boot_entry_address);
  part_map_entry->boot_cksum = PED_CPU_TO_BE32(mac_part_data->boot_checksum);
...
static int _generate_raw_freespace_part(PedDisk *disk, PedGeometry *geom,
                                        int num, MacRawPartition *part_map) {
  MacDiskData *mac_disk_data = disk->disk_specific;
...
static int _generate_empty_part(PedDisk *disk, int num,
                                MacRawPartition *part_map) {
  MacDiskData *mac_disk_data = disk->disk_specific;
...
static int write_block_zero(PedDisk *disk, MacDiskData *mac_driverdata) {
...
static int mac_write(PedDisk *disk) {
  MacRawPartition *part_map;
  MacDiskData *mac_disk_data;
  ...
  size_t pmap_bytes = (mac_disk_data->part_map_entry_count *
                       mac_disk_data->ghost_size * disk->dev->sector_size);
  ...
  for (part = ped_disk_next_partition(disk, NULL); part;
    ...
    if (!_generate_raw_freespace_part(disk, &part->geom, num, part_map))
      goto error_free_part_map;
  }
...
static PedPartition *mac_partition_new(const PedDisk *disk,
...
static PedPartition *mac_partition_duplicate(const PedPartition *part) {
  PedPartition *new_part;
...
static void mac_partition_destroy(PedPartition *part) {
...
static int mac_partition_set_system(PedPartition *part,
                                    const PedFileSystemType *fs_type) {
  ...
  if (fs_type &&
      (!strcmp(fs_type->name, "hfs") || !strcmp(fs_type->name, "hfs+"))) {
...
static int mac_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                  int state) {
  MacPartitionData *mac_data;
...
static int _GL_ATTRIBUTE_PURE mac_partition_get_flag(const PedPartition *part,
...
static int mac_partition_is_flag_available(const PedPartition *part,
                                           PedPartitionFlag flag) {
  switch (flag) {
  case PED_PARTITION_BOOT:
  case PED_PARTITION_ROOT:
...
static void mac_partition_set_name(PedPartition *part, const char *name) {
  MacPartitionData *mac_data;
  int i;
  ...
  for (i = strlen(mac_data->volume_name) - 1; mac_data->volume_name[i] == ' ';
       i--)
...
static PedAlignment *mac_get_partition_alignment(const PedDisk *disk) {
  return ped_alignment_new(0, 1);
}

static PedConstraint *_primary_constraint(PedDisk *disk) {
...
static int mac_partition_align(PedPartition *part,
...
static int mac_partition_enumerate(PedPartition *part) {
  PedDisk *disk;
  MacDiskData *mac_disk_data;
  int i;
  int max_part_count;
...
static int _disk_count_partitions(PedDisk *disk) {
  MacDiskData *mac_disk_data = disk->disk_specific;
  ...
  for (part = ped_disk_next_partition(disk, part); part;
    ...
    mac_disk_data->last_part_entry_num =
        PED_MAX(mac_disk_data->last_part_entry_num, part->num);
...
static int add_metadata_part(PedDisk *disk, PedSector start, PedSector end) {
  PedPartition *new_part;
  PedConstraint *constraint_any = ped_constraint_any(disk->dev);

  PED_ASSERT(disk != NULL);

  new_part = ped_partition_new(disk, PED_PARTITION_METADATA, NULL, start, end);
  if (!new_part)
    goto error;
  if (!ped_disk_add_partition(disk, new_part, constraint_any))
    goto error_destroy_new_part;
...
static int mac_alloc_metadata(PedDisk *disk) {
  PED_ASSERT(disk != NULL);
  PED_ASSERT(disk->disk_specific != NULL);
  PED_ASSERT(disk->dev != NULL);
...
static int mac_get_max_primary_partition_count(const PedDisk *disk) {
  MacDiskData *mac_disk_data = disk->disk_specific;
  PedPartition *part_map_partition;

  part_map_partition =
      ped_disk_get_partition(disk, mac_disk_data->part_map_entry_num);
...
static bool mac_get_max_supported_partition_count(const PedDisk *disk,
                                                  int *max_n) {
  *max_n = 65536;
  return true;
}
