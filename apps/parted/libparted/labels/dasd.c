...
static int dasd_probe(const PedDevice *dev) {
  ...
  if (fdasd_check_volume(&anchor, arch_specific->fd) && anchor.FBA_layout == 0)
    goto error_cleanup;
...
static int dasd_read(PedDisk *disk) {
  ...
  if (is_ldl || is_cms) {
    ...
    if (is_ldl)
      ...
      end = (long long)cms_ptr->block_size / (long long)disk->dev->sector_size *
  ...
  if (disk_specific->format_type == 1 && part) {
    ...
    */
    ...
    if (trailing_meta_end >= trailing_meta_start) {
      new_part2 = ped_partition_new(disk, PED_PARTITION_METADATA, NULL,
