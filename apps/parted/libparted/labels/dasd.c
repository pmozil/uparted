...
static int dasd_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                   int state);
...
static bool dasd_get_max_supported_partition_count(const PedDisk *disk,
                                                   int *max_n);
...
static PedDiskType dasd_disk_type =
    {next : NULL, name : "dasd", ops : &dasd_disk_ops, features : 0};
...
static PedDisk *dasd_alloc(const PedDevice *dev) {
    ...
    vtoc_set_cchhb(&disk_specific->vlabel.vtoc, VTOC_START_CC, VTOC_START_HH,
                   0x01);
...
void ped_disk_dasd_init() { ped_disk_type_register(&dasd_disk_type); }

void ped_disk_dasd_done() { ped_disk_type_unregister(&dasd_disk_type); }
...
static int dasd_read(PedDisk *disk) {
    ...
    bool is_ldl =
        strncmp(anchor.vlabel->volkey, vtoc_ebcdic_enc("LNX1", str, 4), 4) == 0;
    bool is_cms =
        strncmp(anchor.vlabel->volkey, vtoc_ebcdic_enc("CMS1", str, 4), 4) == 0;
    if (is_ldl || is_cms) {
        DasdPartitionData *dasd_data;
        ...
        if (is_ldl)
            if (ldl_ptr->ldl_version >= 0xf2)
                          ...
                          (long long)disk->dev->sector_size *
                          (long long)ldl_ptr->formatted_blocks -
                      1;
            else
                end = disk->dev->length - 1;
        else if (cms_ptr->disk_offset == 0)
    ...
    for (i = 1; i <= USABLE_PARTITIONS; i++) {
        ...
        if (p->fspace_trk > 0) {
            start = (long long)((long long)p->end_trk + 1) *
                  ...
                  1;
            part =
                ped_partition_new(disk, PED_PARTITION_NORMAL, NULL, start, end);
...
static int dasd_update_type(const PedDisk *disk, struct fdasd_anchor *anchor,
...
static int dasd_write(const PedDisk *disk) {
    ...
    for (i = 1; i <= USABLE_PARTITIONS; i++) {
        ...
        stop = (part->geom.end + 1) * disk->dev->sector_size /
...
static int dasd_partition_set_system(PedPartition *part,
    ...
    cyl_size =
        part->disk->dev->hw_geom.sectors * part->disk->dev->hw_geom.heads;
    PDEBUG;
...
static int dasd_alloc_metadata(PedDisk *disk) {
    ...
    new_part =
        ped_partition_new(disk, PED_PARTITION_METADATA, NULL, 0, vtoc_end);
    if (!new_part)
        goto error;
    ...
    if (disk_specific->format_type == 1 && part) {
        ...
        */
        ...
        if (trailing_meta_end >= trailing_meta_start) {
            new_part2 =
                ped_partition_new(disk, PED_PARTITION_METADATA, NULL,
                                  trailing_meta_start, trailing_meta_end);
            if (!new_part2) {
                ped_partition_destroy(new_part);
                goto error;
            }
            if (!ped_disk_add_partition(disk, new_part2, constraint_any)) {
