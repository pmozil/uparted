...
/* XXX fixme: endian? */
static unsigned short xbsd_dkcksum(BSDRawLabel *lp) {
  const u_short *word = (u_short *)(lp);
  const u_short *end =
      word + D_PARTITIONS_WORDS + PED_LE16_TO_CPU(lp->d_npartitions);
  u_short sum;
...
/* XXX fixme: endian? */
static void alpha_bootblock_checksum(void *boot) {
...
static PedDisk *bsd_alloc(const PedDevice *dev) {
  PedDisk *disk;
  ...
  label->d_secpercyl =
      PED_CPU_TO_LE32(dev->bios_geom.sectors * dev->bios_geom.heads);
  label->d_secperunit = PED_CPU_TO_LE32(
      dev->bios_geom.sectors * dev->bios_geom.heads * dev->bios_geom.cylinders);
...
static PedDisk *bsd_duplicate(const PedDisk *disk) {
  PedDisk *new_disk;
...
static void bsd_free(PedDisk *disk) {
...
static void _probe_and_add_boot_code(const PedDisk *disk) {
...
static int bsd_write(const PedDisk *disk) {
  BSDDiskData *bsd_specific;
  BSDRawLabel *label;
  ...
  memset(label->d_partitions, 0, sizeof(BSDRawPartition) * BSD_MAXPARTITIONS);
  ...
  for (i = 1; i <= BSD_MAXPARTITIONS; i++) {
    ...
    label->d_partitions[i - 1].p_offset = PED_CPU_TO_LE32(part->geom.start);
    label->d_partitions[i - 1].p_size = PED_CPU_TO_LE32(part->geom.length);
    max_part = i;
  }
  ...
  if (!ptt_write_sector(disk, bsd_specific, sizeof(BSDDiskData)))
    goto error;
...
static PedPartition *bsd_partition_new(const PedDisk *disk,
...
static PedPartition *bsd_partition_duplicate(const PedPartition *part) {
...
static int bsd_partition_set_system(PedPartition *part,
                                    const PedFileSystemType *fs_type) {
...
static int bsd_partition_set_flag(PedPartition *part, PedPartitionFlag flag,
                                  int state) {
  BSDPartitionData *bsd_data;
...
static int _GL_ATTRIBUTE_PURE bsd_partition_get_flag(const PedPartition *part,
...
static int bsd_partition_is_flag_available(const PedPartition *part,
                                           PedPartitionFlag flag) {
  switch (flag) {
  case PED_PARTITION_BOOT:
  case PED_PARTITION_RAID:
...
static bool bsd_get_max_supported_partition_count(const PedDisk *disk,
                                                  int *max_n) {
  *max_n = BSD_MAXPARTITIONS;
  return true;
}

static PedConstraint *_get_constraint(const PedDevice *dev) {
  PedGeometry max;
...
static PedDiskType bsd_disk_type =
    {next : NULL, name : "bsd", ops : &bsd_disk_ops, features : 0};
...
void ped_disk_bsd_done() { ped_disk_type_unregister(&bsd_disk_type); }
