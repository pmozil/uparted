...
ptrdiff_t argmatch_exact(char const *arg,
                         char const *const *arglist) _GL_ATTRIBUTE_PURE;

#define ARGMATCH(Arg, Arglist, Vallist)                                        \
    argmatch(Arg, Arglist, (void const *)(Vallist), sizeof *(Vallist))

#define ARGMATCH_EXACT(Arg, Arglist) argmatch_exact(Arg, Arglist)
...
#define XARGMATCH(Context, Arg, Arglist, Vallist)                              \
    ((Vallist)[__xargmatch_internal(Context, Arg, Arglist,                     \
                                    (void const *)(Vallist),                   \
                                    sizeof *(Vallist), argmatch_die, true)])

#define XARGMATCH_EXACT(Context, Arg, Arglist, Vallist)                        \
    ((Vallist)[__xargmatch_internal(Context, Arg, Arglist,                     \
                                    (void const *)(Vallist),                   \
                                    sizeof *(Vallist), argmatch_die, false)])

/* Convert a value into a corresponding argument. */

char const *argmatch_to_argument(void const *value, char const *const *arglist,
                                 void const *vallist,
                                 size_t valsize) _GL_ATTRIBUTE_PURE;

#define ARGMATCH_TO_ARGUMENT(Value, Arglist, Vallist)                          \
    argmatch_to_argument(Value, Arglist, (void const *)(Vallist),              \
                         sizeof *(Vallist))
...
#define ARGMATCH_DEFINE_GROUP(Name, Type)                                      \
    ...
    enum argmatch_##Name##_size_enum{argmatch_##Name##_size =                  \
                                         sizeof(argmatch_##Name##_type)};      \
                                                                               \
    ...
    /* A pointer to the corresponding value if it exists, or                   \
       ...
       not recognized. */                                                      \
    const argmatch_##Name##_type *argmatch_##Name##_value(const char *context, \
                                                          const char *arg);    \
                                                                               \
    ...
    ptrdiff_t argmatch_##Name##_choice(const char *arg) {                      \
        ...
        return ambiguous ? -2 : res;                                           \
                                                                               ...
                                                                               \
    const char *argmatch_##Name##_argument(                                    \
        const argmatch_##Name##_type *val) {                                   \
        const argmatch_##Name##_group_type *g = &argmatch_##Name##_group;      \
        size_t size = argmatch_##Name##_size;                                  \
        for (size_t i = 0; g->args[i].arg; i++)                                \
