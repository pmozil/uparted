...
static void __argmatch_die(void) { ARGMATCH_DIE; }
...
void argmatch_invalid(const char *context, const char *value,
                      ptrdiff_t problem) {
    char const *format = (problem == -1 ? _("invalid argument %s for %s")
...
void argmatch_valid(const char *const *arglist, const void *vallist,
                    size_t valsize) {
    size_t i;
...
ptrdiff_t __xargmatch_internal(const char *context, const char *arg,
                               const char *const *arglist, const void *vallist,
...
const char *argmatch_to_argument(const void *value, const char *const *arglist,
                                 const void *vallist, size_t valsize) {
    size_t i;
...
static const char *const backup_args[] = {
    "no",       "none", "off",      "simple", "never",
    "existing", "nil",  "numbered", "t",      0};

static const enum backup_type backup_vals[] = {no_backups,
                                               no_backups,
                                               no_backups,
                                               simple_backups,
                                               simple_backups,
...
int main(int argc, const char *const *argv) {
    ...
    if ((cp = getenv("VERSION_CONTROL")))
        backup_type =
            XARGMATCH("$VERSION_CONTROL", cp, backup_args, backup_vals);

    if (argc == 2)
        backup_type =
            XARGMATCH(getprogname(), argv[1], backup_args, backup_vals);
