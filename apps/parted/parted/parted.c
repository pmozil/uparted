...
static int _disk_warn_busy(PedDisk *disk) {
    if (ped_device_is_busy(disk->dev))
        return ped_exception_throw((opt_script_mode ? PED_EXCEPTION_ERROR
...
static EMoves prefer_snap(PedSector s, int what, PedGeometry *range,
    ...
    if ((*allow & MOVE_UP) && (*allow & MOVE_DOWN)) {
        if (down_dist < up_dist || (down_dist == up_dist && what == SECT_START))
            move = MOVE_DOWN;
        else if (up_dist < down_dist ||
                 (down_dist == up_dist && what == SECT_END))
            move = MOVE_UP;
        else
            PED_ASSERT(0);
...
static void snap_to_boundaries(PedGeometry *new_geom, PedGeometry *old_geom,
    ...
    if (adjacent && start_want == MOVE_UP && end_want == MOVE_DOWN) {
        if (end_dist < start_dist) {
            start_allow &= ~MOVE_UP;
            start_want = prefer_snap(start, SECT_START, start_range,
                                     &start_allow, start_part, &start_dist);
...
void print_options_help() {
    ...
    for (i = 0; options_help[i][0]; i++) {
        printf("  -%c, --%-25.25s %s\n", options_help[i][0][0],
               options_help[i][0], _(options_help[i][1]));
    }
}
...
static int do_mkpart(PedDevice **dev, PedDisk **diskp) {
    ...
    final_constraint =
        ped_constraint_intersect(user_constraint, dev_constraint);
    ...
    if (!added_ok) {
        ...
        if (!ped_geometry_test_sector_inside(range_start, part->geom.start) ||
            ...
            switch (ped_exception_throw(
                ...
                start_usr, end_usr, start, end, start_sol, end_sol,
...
static void _print_disk_geometry(const PedDevice *dev) {
    ...
    char *cyl_size = ped_unit_format_custom(dev, chs->heads * chs->sectors,
                                            PED_UNIT_KILOBYTE);
    ...
    if (opt_output_mode == MACHINE) {
        ...
        snprintf(tmp, 128, "%d,%d,%d %s", chs->cylinders, chs->heads,
                 chs->sectors, cyl_size);
...
static int do_print(PedDevice **dev, PedDisk **diskp) {
    ...
    bool has_type_uuid = ped_disk_type_check_feature(
        (*diskp)->type, PED_DISK_TYPE_PARTITION_TYPE_UUID);
    bool has_part_uuid = ped_disk_type_check_feature(
        (*diskp)->type, PED_DISK_TYPE_PARTITION_UUID);
    ...
    if (opt_output_mode == HUMAN) {
        ...
        if (ped_unit_get_default() == PED_UNIT_CHS) {
            ...
            row1 = str_list_create(_("Number"), _("Start"), _("End"), _("Size"),
                                   NULL);
        }
        ...
        for (part = ped_disk_next_partition(*diskp, NULL); part;
            ...
            ul_jsonwrt_value_u64(&json, "number",
            ...
            if (!(part->type & PED_PARTITION_FREESPACE)) {
                ...
                if (part->fs_type)
                    ul_jsonwrt_value_s(&json, "filesystem",
                                       part->fs_type->name);
...
static void _init_commands() {
    ...
    command_register(
        commands,
        command_create(str_list_create_unique("mklabel", _("mklabel"),
                                              "mktable", _("mktable"), NULL),
                       do_mklabel,
    ...
    command_register(
        commands,
        command_create(
            str_list_create_unique("resize", _("resize"), NULL), do_resize,
            NULL,
            str_list_create(
...
static PedDevice *_choose_device(int *argc_ptr, char ***argv_ptr) {
    ...
    retry:
        ...
        if (!dev) {
            if (ped_exception_throw(
                    PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_CANCEL,
