...
static char const *const align_args[] = {"none", "cylinder", "minimal",
                                         "optimal", NULL};
...
static const char *number_msg = N_(
    ...
    "primary partitions number from 1 to 4, logical partitions from 5 "
    "onwards.\n");
...
static const char *copyright_msg =
    N_("Copyright (C) 1998 - 2006 Free Software Foundation, Inc.\n"
...
static bool partition_align_check(PedDisk const *disk, PedPartition const *part,
                                  enum AlignmentType a_type, char **align_err);

static void _timer_handler(PedTimer *timer, void *context) {
  ...
  if (draw_this_time) {
    ...
    printf(_("%0.f%%\t(time left %.2d:%.2d)"), (double)(100.0f * timer->frac),
...
static int _partition_warn_busy(PedPartition *part) {
  char *path;
...
static int _disk_warn_busy(PedDisk *disk) {
  if (ped_device_is_busy(disk->dev))
    return ped_exception_throw(
               (opt_script_mode ? PED_EXCEPTION_ERROR : PED_EXCEPTION_WARNING),
...
static int _disk_warn_loss(PedDisk *disk) {
  return ped_exception_throw(
             PED_EXCEPTION_WARNING, PED_EXCEPTION_YES_NO,
...
typedef enum { MOVE_NO = 0, MOVE_STILL = 1, MOVE_UP = 2, MOVE_DOWN = 4 } EMoves;
 ...
 */
static EMoves prefer_snap(PedSector s, int what, PedGeometry *range,
                          EMoves *allow, PedPartition *part, PedSector *dist) {
...
static void snap_to_boundaries(PedGeometry *new_geom, PedGeometry *old_geom,
  ...
  if (adjacent && start_want == MOVE_UP && end_want == MOVE_DOWN) {
    if (end_dist < start_dist) {
      ...
      end_allow &= ~MOVE_DOWN;
      end_want = prefer_snap(end, SECT_END, end_range, &end_allow, end_part,
                             &end_dist);
  ...
  /* New positions */
  start = (start_want == MOVE_DOWN
               ? start_part->geom.start
               : (start_want == MOVE_UP ? start_part->geom.end + 1 : start));
  end = (end_want == MOVE_DOWN
             ? end_part->geom.start - 1
...
int do_help(PedDevice **dev, PedDisk **diskp) {
...
static int do_mklabel(PedDevice **dev, PedDisk **diskp) {
  PedDisk *disk;
...
static bool _is_unit_IEC(const PedUnit unit) {
...
static bool _string_ends_with_iec_unit(const char *str) {
 ...
 */
static bool _string_has_unit_suffix(const char *str) {
 ...
 */
static void _adjust_end_if_iec(PedSector *start, PedSector *end,
...
static int do_mkpart(PedDevice **dev, PedDisk **diskp) {
  PedDisk *disk;
  PedPartition *part;
  ...
  if (ped_disk_type_check_feature(disk->type, PED_DISK_TYPE_PARTITION_NAME) &&
    ...
    part_name = command_line_get_word(_("Partition name?"), "", NULL, 1);
  ...
  if (alignment == ALIGNMENT_OPTIMAL)
    ...
    dev_constraint = ped_device_get_minimal_aligned_constraint(*dev);
  else
  ...
  if (!added_ok) {
    ...
    added_ok = ped_disk_add_partition(disk, part, constraint_any);
    ...
    if (!ped_geometry_test_sector_inside(range_start, part->geom.start) ||
      ...
      switch (ped_exception_throw(
          (opt_script_mode ? PED_EXCEPTION_ERROR : PED_EXCEPTION_WARNING),
          (opt_script_mode ? PED_EXCEPTION_CANCEL : PED_EXCEPTION_YES_NO),
          _("You requested a partition from %s to %s "
            "(sectors %llu..%llu).\n"
            "The closest location we can manage is "
            "%s to %s (sectors %llu..%llu).%s"),
          start_usr, end_usr, start, end, start_sol, end_sol, part->geom.start,
          part->geom.end,
...
static int do_name(PedDevice **dev, PedDisk **diskp) {
...
static int do_type(PedDevice **dev, PedDisk **diskp) {
  if (!*diskp)
    *diskp = ped_disk_new(*dev);
  if (!*diskp)
    goto error;

  bool has_type_id = ped_disk_type_check_feature(
      (*diskp)->type, PED_DISK_TYPE_PARTITION_TYPE_ID);
  bool has_type_uuid = ped_disk_type_check_feature(
      (*diskp)->type, PED_DISK_TYPE_PARTITION_TYPE_UUID);
...
static void partition_print_flags_json(PedPartition const *part) {
  if (!part)
    return;
...
static int partition_print(PedPartition *part) { return 1; }
...
static char *disk_print_flags(PedDisk const *disk) {
  ...
  size_t res_buf_len = 1;
  char const *sep = "";
  for (flag = ped_disk_flag_next(0); flag; flag = ped_disk_flag_next(flag)) {
...
static void disk_print_flags_json(PedDisk const *disk) {
  if (!disk)
    return;
...
static void _print_disk_geometry(const PedDevice *dev) {
  PED_ASSERT(dev != NULL);
  const PedCHSGeometry *chs = &dev->bios_geom;
  char *cyl_size =
      ped_unit_format_custom(dev, chs->heads * chs->sectors, PED_UNIT_KILOBYTE);
  ...
  if (opt_output_mode == MACHINE) {
    ...
    snprintf(tmp, 128, "%d,%d,%d %s", chs->cylinders, chs->heads, chs->sectors,
             cyl_size);
...
static char *_escape_machine_string(const char *str) {
  size_t i, j;
  char *dest;
...
static void _print_disk_info(const PedDevice *dev, const PedDisk *diskp) {
  ...
  if (opt_output_mode == MACHINE) {
    ...
    if (diskp) {
      bool has_disk_uuid =
          ped_disk_type_check_feature(diskp->type, PED_DISK_TYPE_DISK_UUID);
      if (has_disk_uuid) {
...
static int do_print(PedDevice **dev, PedDisk **diskp) {
  ...
  if (*diskp && ped_disk_is_flag_available(*diskp, PED_DISK_CYLINDER_ALIGNMENT))
  ...
  if (has_devices_arg) {
    ...
    while ((current_dev = ped_device_get_next(current_dev))) {
      end = ped_unit_format_byte(current_dev, current_dev->length *
  ...
  has_extended =
      ped_disk_type_check_feature((*diskp)->type, PED_DISK_TYPE_EXTENDED);
  has_name =
      ped_disk_type_check_feature((*diskp)->type, PED_DISK_TYPE_PARTITION_NAME);
  bool has_type_id = ped_disk_type_check_feature(
      (*diskp)->type, PED_DISK_TYPE_PARTITION_TYPE_ID);
  bool has_type_uuid = ped_disk_type_check_feature(
      (*diskp)->type, PED_DISK_TYPE_PARTITION_TYPE_UUID);
  bool has_part_uuid =
      ped_disk_type_check_feature((*diskp)->type, PED_DISK_TYPE_PARTITION_UUID);
  ...
  if (opt_output_mode == HUMAN) {
    ...
    if (ped_unit_get_default() == PED_UNIT_CHS) {
      row1 = str_list_create(_("Number"), _("Start"), _("End"), NULL);
    } else {
      row1 =
          str_list_create(_("Number"), _("Start"), _("End"), _("Size"), NULL);
    }
    ...
    for (part = ped_disk_next_partition(*diskp, NULL); part;
      ...
      if (!(part->type & PED_PARTITION_FREESPACE)) {
        ...
        str_list_append(row, part->fs_type ? part->fs_type->name : "");
    ...
    for (part = ped_disk_next_partition(*diskp, NULL); part;
      ...
      if (!(part->type & PED_PARTITION_FREESPACE)) {
        ...
        if (has_name) {
          char *escaped_name =
              _escape_machine_string(ped_partition_get_name(part));
...
static int do_quit(PedDevice **dev, PedDisk **diskp) {
  _done(*dev, *diskp);
  exit(EXIT_SUCCESS);
}

static PedPartitionType _disk_get_part_type_for_sector(PedDisk *disk,
                                                       PedSector sector) {
  PedPartition *extended;

  extended = ped_disk_extended_partition(disk);
  if (!extended || !ped_geometry_test_sector_inside(&extended->geom, sector))
    return 0;
 ...
 */
static int _rescue_add_partition(PedPartition *part) {
  ...
  if (!ped_disk_set_partition_geom(part->disk, part, constraint, probed->start,
                                   probed->end)) {
  ...
  ex_opt = ped_exception_throw(
      PED_EXCEPTION_INFORMATION, PED_EXCEPTION_YES_NO_CANCEL,
      _("A %s %s partition was found at %s -> %s.  "
        "Do you want to add it to the partition table?"),
      fs_type->name, ped_partition_type_get_name(part->type), found_start,
      found_end);
  ped_geometry_destroy(probed);
  free(found_start);
  free(found_end);
 ...
 */
static int _rescue_pass(PedDisk *disk, PedGeometry *start_range,
                        PedGeometry *end_range) {
  PedSector start;
  ...
  for (start = start_range->start; start <= start_range->end; start++) {
    ped_timer_update(g_timer,
                     1.0 * (start - start_range->start) / start_range->length);

    ped_geometry_init(&start_geom_exact, disk->dev, start, 1);
    ped_constraint_init(&constraint, ped_alignment_any, ped_alignment_any,
                        &start_geom_exact, &entire_dev, 1, disk->dev->length);
    part = ped_partition_new(disk, part_type, NULL, start, end_range->end);
    if (!part) {
...
static int do_rescue(PedDevice **dev, PedDisk **diskp) {
  PedDisk *disk;
  PedSector start = 0, end = 0;
  PedSector fuzz;
  ...
  if (!disk)
    goto error;
  if (ped_disk_is_flag_available(disk, PED_DISK_CYLINDER_ALIGNMENT))
    if (!ped_disk_set_flag(disk, PED_DISK_CYLINDER_ALIGNMENT, 0))
      goto error;
  ...
  ped_geometry_init(&probe_start_region, *dev, PED_MAX(start - fuzz, 0),
                    PED_MIN(2 * fuzz, (*dev)->length - (start - fuzz)));
  ped_geometry_init(&probe_end_region, *dev, PED_MAX(end - fuzz, 0),
...
static int do_resize(PedDevice **dev, PedDisk **diskp) {
  ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                      _("The resize command has been removed in parted 3.0"));
  return 0;
}

static int do_resizepart(PedDevice **dev, PedDisk **diskp) {
  PedDisk *disk = *diskp;
  PedPartition *part = NULL;
  ...
  /* If the partition is busy this may clear the command_line and prompt the
   * user */
...
static int do_rm(PedDevice **dev, PedDisk **diskp) {
  PedPartition *part = NULL;
...
static int do_select(PedDevice **dev, PedDisk **diskp) {
...
static bool partition_align_check(PedDisk const *disk, PedPartition const *part,
  ...
  PedAlignment *pa =
      (a_type == PA_MINIMUM ? ped_device_get_minimum_alignment(dev)
...
static int do_align_check(PedDevice **dev, PedDisk **diskp) {
  if (!*diskp)
    *diskp = ped_disk_new(*dev);
  if (!*diskp)
    goto error;
...
static int do_disk_set(PedDevice **dev, PedDisk **diskp) {
  PedDiskFlag flag;
  int state;
...
static int do_set(PedDevice **dev, PedDisk **diskp) {
  PedPartition *part = NULL;
  PedPartitionFlag flag;
  int state;
...
static int do_disk_toggle(PedDevice **dev, PedDisk **diskp) {
  int result;
...
static int do_toggle(PedDevice **dev, PedDisk **diskp) {
  int result;
...
static int do_unit(PedDevice **dev, PedDisk **diskp) {
  PedUnit unit = ped_unit_get_default();
...
static int do_version() {
  printf("\n%s\n%s", prog_name, _(copyright_msg));
  return 1;
}

static void _init_messages() {
  StrList *list;
  int first;
  ...
  for (disk_flag = ped_disk_flag_next(0); disk_flag;
    ...
    if (first)
      ...
      str_list_append(list, ", ");
    str_list_append(list, _(ped_disk_flag_get_name(disk_flag)));
  }
  str_list_append(list, "\n");
  ...
  first = 1;
  for (disk_type = ped_disk_type_get_next(NULL); disk_type;
       disk_type = ped_disk_type_get_next(disk_type)) {
  ...
  for (fs_alias = ped_file_system_alias_get_next(NULL); fs_alias;
       fs_alias = ped_file_system_alias_get_next(fs_alias)) {
    if (first)
      first = 0;
    else
      str_list_append(list, ", ");
...
static void _init_commands() {
  command_register(
      commands,
      command_create(
          str_list_create_unique("align-check", _("align-check"), NULL),
          do_align_check,
          str_list_create(_("align-check TYPE N"
                            "                       "
  ...
  command_register(
      ...
      command_create(str_list_create_unique("help", _("help"), NULL), do_help,
                     str_list_create(_("help [COMMAND]                       "
                                       "    print general help, or help "
                                       "on COMMAND"),
                                     NULL),
                     NULL, 1));
  ...
  command_register(
      commands,
      command_create(str_list_create_unique("mklabel", _("mklabel"), "mktable",
                                            _("mktable"), NULL),
                     do_mklabel,
                     str_list_create(_("mklabel,mktable LABEL-TYPE           "
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("mkpart", _("mkpart"), NULL), do_mkpart,
          str_list_create(
              _("mkpart PART-TYPE [FS-TYPE] START END     make a partition"),
              NULL),
          str_list_create(_(part_type_msg), _(mkpart_fs_type_msg),
                          _(start_end_msg), "\n",
                          _("'mkpart' makes a partition without creating a "
                            "new file system on the "
                            "partition.  FS-TYPE may be specified to set an "
  ...
  command_register(
      commands,
      command_create(str_list_create_unique("name", _("name"), NULL), do_name,
                     str_list_create(_("name NUMBER NAME                     "
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("print", _("print"), NULL), do_print,
          str_list_create(_("print [devices|free|list,all]            "
                            "display the partition table, "
                            "or available devices, or free space, or all "
                            "found partitions"),
                          NULL),
          str_list_create(
              _("Without arguments, 'print' displays the entire partition "
                "table. However "
                "with the following arguments it performs various other "
                "actions.\n"),
              _("  devices   : display all active block devices\n"),
              _("  free      : display information about free unpartitioned "
                "space on the "
                "current block device\n"),
              _("  list, all : display the partition tables of all active "
                "block devices\n"),
              NULL),
          1));
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("quit", _("quit"), NULL), do_quit,
          str_list_create(
              _("quit                                     exit program"), NULL),
          NULL, 1));

  command_register(
      commands,
      command_create(str_list_create_unique("rescue", _("rescue"), NULL),
                     do_rescue,
                     str_list_create(_("rescue START END                     "
                                       "    rescue a lost partition near "
                                       "START and END"),
                                     NULL),
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("resize", _("resize"), NULL), do_resize, NULL,
          str_list_create(
              _(N_("The resize command was removed in parted 3.0\n")), NULL),
          1));
  ...
  command_register(
      ...
      command_create(
          ...
          do_resizepart,
          str_list_create(_("resizepart NUMBER END                    resize "
                            "partition NUMBER"),
                          NULL),
  ...
  command_register(
      commands,
      command_create(str_list_create_unique("rm", _("rm"), NULL), do_rm,
                     str_list_create(_("rm NUMBER                            "
  ...
  command_register(
      commands,
      command_create(str_list_create_unique("select", _("select"), NULL),
                     do_select,
                     str_list_create(_("select DEVICE                        "
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("disk_set", _("disk_set"), NULL), do_disk_set,
          str_list_create(_("disk_set FLAG STATE                      change "
  ...
  command_register(
      ...
      command_create(
          ...
          do_disk_toggle,
          str_list_create(_("disk_toggle [FLAG]                       toggle "
                            "the state of FLAG on "
                            "selected device"),
                          NULL),
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("set", _("set"), NULL), do_set,
          str_list_create(_("set NUMBER FLAG STATE                    change "
                            "the FLAG on partition "
                            "NUMBER"),
                          NULL),
  ...
  command_register(
      commands,
      command_create(str_list_create_unique("toggle", _("toggle"), NULL),
                     do_toggle,
                     str_list_create(_("toggle [NUMBER [FLAG]]               "
                                       "    toggle the state of FLAG on "
                                       "partition NUMBER"),
                                     NULL),
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("type", _("type"), NULL), do_type,
          str_list_create(_("type NUMBER TYPE-ID or TYPE-UUID         type "
                            "set TYPE-ID or TYPE-UUID of partition NUMBER"),
                          NULL),
          str_list_create(_(number_msg), _(type_msg), NULL), 1));

  command_register(
      commands,
      command_create(str_list_create_unique("unit", _("unit"), NULL), do_unit,
                     str_list_create(_("unit UNIT                            "
  ...
  command_register(
      commands,
      command_create(
          str_list_create_unique("version", _("version"), NULL), do_version,
          str_list_create(_("version                                  "
                            "display the version number "
                            "and copyright information of GNU Parted"),
                          NULL),
          str_list_create(_("'version' displays copyright and version "
...
static int _parse_options(int *argc_ptr, char ***argv_ptr) {
  int opt, help = 0, list = 0, version = 0, wrong = 0;

  while (1) {
    opt = getopt_long(*argc_ptr, *argv_ptr, "hlmjsfva:", options, NULL);
    if (opt == -1)
      break;
    ...
    case 'l':
      ...
      break;
    case 'm':
      opt_output_mode = MACHINE;
      break;
    case 'j':
      opt_output_mode = JSON;
      break;
    case 's':
      opt_script_mode = 1;
      break;
    ...
    case 'v':
      ...
      break;
    case 'a':
      alignment = XARGMATCH("--align", optarg, align_args, align_types);
      break;
  ...
  if (wrong == 1) {
    fprintf(stderr,
            _("Usage: %s [-hlmsfv] [-a<align>] [DEVICE [COMMAND "
              "[PARAMETERS]]...]\n"),
            program_name);
    return 0;
  }
...
static PedDevice *_choose_device(int *argc_ptr, char ***argv_ptr) {
  PedDevice *dev;
  ...
  retry:
    ...
    if (!dev) {
      if (ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_RETRY_CANCEL,
                              _("No device found")) == PED_EXCEPTION_RETRY)
        goto retry;
      else
        return NULL;
    }
