...
static int _GL_ATTRIBUTE_PURE wchar_strlen(const wchar_t *str) {
#ifdef ENABLE_NLS
    return wcslen(str);
#else
    return strlen(str);
...
static wchar_t *_GL_ATTRIBUTE_PURE wchar_strchr(const wchar_t *str, char ch) {
#ifdef ENABLE_NLS
...
static int _GL_ATTRIBUTE_PURE wchar_strcasecmp(const wchar_t *a,
                                               const wchar_t *b) {
#ifdef ENABLE_NLS
...
static int _GL_ATTRIBUTE_PURE wchar_strncasecmp(const wchar_t *a,
...
static wchar_t *_GL_ATTRIBUTE_PURE wchar_strdup(const wchar_t *str) {
#ifdef ENABLE_NLS
    return wcsdup(str);
#else
    return xstrdup(str);
 ...
 */
#ifdef ENABLE_NLS
static wchar_t *gettext_to_wchar(const char *str) {
    int count;
    wchar_t *result;
    size_t status;
    mbstate_t ps;
...
static wchar_t *gettext_to_wchar(const char *str) { return xstrdup(str); }

#endif /* !ENABLE_NLS */

#ifdef ENABLE_NLS
static char *wchar_to_str(const wchar_t *str, size_t count) {
    char *result;
    char *out_buf;
    size_t status;
    mbstate_t ps;
...
static void print_wchar(const wchar_t *str, size_t count, FILE *fp) {
    char *tmp = wchar_to_str(str, count);
    fprintf(fp, "%s", tmp);
    free(tmp);
}

static StrList *str_list_alloc() {
    StrList *list;
...
void str_list_destroy_node(StrList *list) {
    void *p = (char *)(list->str); /* discard const */
    free(p);
    free(list);
}

StrList *str_list_duplicate_node(const StrList *node) {
    StrList *result = str_list_alloc();
    result->str = wchar_strdup(node->str);
    return result;
}

StrList *str_list_duplicate(const StrList *list) {
    if (list)
...
StrList *str_list_join(StrList *a, StrList *b) {
    StrList *walk;
...
static StrList *_str_list_append(StrList *list, const wchar_t *str) {
    StrList *walk;
...
StrList *str_list_append(StrList *list, const char *str) {
    return _str_list_append(list, gettext_to_wchar(str));
}

StrList *str_list_append_unique(StrList *list, const char *str) {
    StrList *walk;
...
StrList *str_list_insert(StrList *list, const char *str) {
...
StrList *str_list_create_unique(const char *first, ...) {
    va_list args;
    char *str;
    StrList *list;
...
char *str_list_convert_node(const StrList *list) {
    return wchar_to_str(list->str, 0);
}

char *str_list_convert(const StrList *list) {
    const StrList *walk;
    int pos = 0;
    int length = 1;
    char *str = xstrdup("");
...
void str_list_print(const StrList *list, FILE *fp) {
    const StrList *walk;
...
static int str_search(const wchar_t *str, int n, wchar_t c) {
    int i;
...
static int is_break_point(wchar_t c) {
#ifdef ENABLE_NLS
...
void str_list_print_wrap(const StrList *list, int line_length, int offset,
                         int indent, FILE *fp) {
    const StrList *walk;
    const wchar_t *str;
    int str_len;
    ...
    for (walk = list; walk; walk = walk->next) {
        ...
        while (line_left < str_len || wchar_strchr(str, '\n')) {
            ...
            if (cut_left < str_len && is_space(str[cut_left + 1]))
                cut_left++;
            ...
            for (cut_right = cut_left + (line_break ? 2 : 1);
                 cut_right < str_len && is_space(str[cut_right]); cut_right++)
                ;
...
static int _str_list_match_node(const StrList *list, const wchar_t *str) {
...
int str_list_match_node(const StrList *list, const char *str) {
...
/* returns:  2 for full match
             ...
             0 for no match
 */
int str_list_match_any(const StrList *list, const char *str) {
    const StrList *walk;
    int best_status = 0;
...
StrList *str_list_match(const StrList *list, const char *str) {
    const StrList *walk;
...
int str_list_length(const StrList *list) {
    int length = 0;
    const StrList *walk;
