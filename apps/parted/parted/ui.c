...
static inline int sigaction(int signum, const struct *sigaction,
                            struct *sigaction) {}
...
static const char *banner_msg =
    N_("Welcome to GNU Parted! Type 'help' to view a list of commands.\n");

static const char *usage_msg =
    N_("Usage: parted [OPTION]... [DEVICE [COMMAND [PARAMETERS]...]...]\n"
...
char **complete_function(char *text, int start, int end) {
  return rl_completion_matches(text, (rl_compentry_func_t *)command_generator);
}

static void _add_history_unique(const char *line) {
 ...
 */
static void reset_env(int quit) {
...
// static void sa_sigint_handler(int signum, siginfo_t *info, void *ucontext) {
//     if (info)
...
// static void sa_sigsegv_handler(int signum, siginfo_t *info, void *ucontext) {
...
// static void sa_sigfpe_handler(int signum, siginfo_t *info, void *ucontext) {
//     fprintf(stderr, bug_msg, VERSION);
//     _dump_history();
//
//     if (!info)
...
// static void sa_sigill_handler(int signum, siginfo_t *info, void *ucontext) {
//     fprintf(stderr, bug_msg, VERSION);
//     _dump_history();
//
//     if (!info)
...
// static void s_sigill_handler(int signum) {
...
// static void s_sigsegv_handler(int signum) {
...
// static void s_sigfpe_handler(int signum) {
...
static char *_readline(const char *prompt, const StrList *possibilities) {
  char *line;
...
static void _print_exception_text(PedException *ex) {
  StrList *text;
...
void command_line_push_word(const char *word) {
  command_line = str_list_append(command_line, word);
}

char *command_line_pop_word() {
  char *result;
  StrList *next;
...
void command_line_flush() {
  str_list_destroy(command_line);
  command_line = NULL;
}

char *command_line_peek_word() {
  if (command_line)
...
int command_line_get_word_count() { return str_list_length(command_line); }

static int _GL_ATTRIBUTE_PURE _str_is_spaces(const char *str) {
  while (isspace(*str))
    str++;
 ...
 */
void command_line_push_line(const char *line, int multi_word) {
  int quoted = 0;
  int quotes_empty = 0;
  char quote_char = 0;
  char this_word[256];
...
static char *realloc_and_cat(char *str, const char *append) {
...
static char *_construct_prompt(const char *head, const char *def,
                               const StrList *possibilities) {
...
void command_line_prompt_words(const char *prompt, const char *def,
 ...
 */
char *command_line_get_word(const char *prompt, const char *def,
                            const StrList *possibilities, int multi_word) {
  do {
    ...
    command_line_prompt_words(prompt, def, possibilities, multi_word);
    if (got_ctrl_c) {
      got_ctrl_c = 0;
      return NULL;
    }
...
int command_line_get_integer(const char *prompt, int *value) {
  char def_str[20];
  char *input;
  long ret;
...
int command_line_get_sector(const char *prompt, PedDevice *dev,
...
int command_line_get_state(const char *prompt, int *value) {
  char *def_word;
  char *input;
...
int command_line_get_device(const char *prompt, PedDevice **value) {
...
int command_line_get_disk(const char *prompt, PedDisk **value) {
...
int command_line_get_partition(const char *prompt, PedDisk *disk,
                               PedPartition **value) {
  PedPartition *part;
  int ret;
...
int command_line_get_fs_type(const char *prompt,
                             const PedFileSystemType *(*value)) {
  char *fs_type_name;
  PedFileSystemType *fs_type;

  fs_type_name = command_line_get_word(prompt, *value ? (*value)->name : NULL,
                                       fs_type_list, 1);
  if (!fs_type_name) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
  ...
  if (!fs_type) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
                        _("Unknown file system type \"%s\"."), fs_type_name);
    free(fs_type_name);
    return 0;
  }
...
int command_line_get_disk_type(const char *prompt,
                               const PedDiskType *(*value)) {
  char *disk_type_name;

  disk_type_name = command_line_get_word(prompt, *value ? (*value)->name : NULL,
                                         disk_type_list, 1);
  if (!disk_type_name) {
    ped_exception_throw(PED_EXCEPTION_ERROR, PED_EXCEPTION_CANCEL,
...
int command_line_get_disk_flag(const char *prompt, const PedDisk *disk,
                               PedDiskFlag *flag) {
  StrList *opts = NULL;
  PedDiskFlag walk = 0;
  char *flag_name;
...
int command_line_get_part_flag(const char *prompt, const PedPartition *part,
...
static int _can_create_primary(const PedDisk *disk) {
  int i;
...
static int _can_create_extended(const PedDisk *disk) {
...
static int _can_create_logical(const PedDisk *disk) {
  if (!ped_disk_type_check_feature(disk->type, PED_DISK_TYPE_EXTENDED))
    return 0;
...
int command_line_get_part_type(const char *prompt, const PedDisk *disk,
  ...
  if (!strcmp(type_name, "primary") || !strcmp(type_name, _("primary"))) {
    *type = 0;
  }
  if (!strcmp(type_name, "extended") || !strcmp(type_name, _("extended"))) {
    *type = PED_PARTITION_EXTENDED;
  }
  if (!strcmp(type_name, "logical") || !strcmp(type_name, _("logical"))) {
...
PedExceptionOption command_line_get_ex_opt(const char *prompt,
  ...
  for (opt = option_get_next(options, 0); opt;
    ...
    options_strlist = str_list_append_unique(
        options_strlist, _(ped_exception_get_option_string(opt)));
    options_strlist = str_list_append_unique(
        options_strlist, ped_exception_get_option_string(opt));
  }
  ...
  while (1) {
    if (strcmp(opt_name, ped_exception_get_option_string(opt)) == 0)
      break;
    if (strcmp(opt_name, _(ped_exception_get_option_string(opt))) == 0)
      break;
    opt = option_get_next(options, opt);
  }
  free(opt_name);
...
int command_line_get_align_type(const char *prompt,
  ...
  if (!input)
    return 0;
  *align_type =
      (str_list_match_any(align_opt_list, input) ? PA_OPTIMUM : PA_MINIMUM);
  free(input);
  return 1;
}

int command_line_get_unit(const char *prompt, PedUnit *unit) {
  StrList *opts = NULL;
  PedUnit walk;
  char *unit_name;
...
static int init_state_str() {
  on_list = str_list_create_unique(_("on"), "on", NULL);
  off_list = str_list_create_unique(_("off"), "off", NULL);
  on_off_list =
      str_list_join(str_list_duplicate(on_list), str_list_duplicate(off_list));
  return 1;
}
...
static int init_alignment_type_str() {
...
static int init_fs_type_str() {
  ...
  for (alias_walk = ped_file_system_alias_get_next(NULL); alias_walk;
    ...
    fs_type_list = str_list_insert(fs_type_list, alias_walk->alias);
    if (!fs_type_list)
      return 0;
  }
...
int init_readline(void) {
  ...
  if (!opt_script_mode) {
    ...
    rl_attempted_completion_function =
        (rl_completion_func_t *)complete_function;
    readline_state.in_readline = 0;
  }
#endif
  return 0;
...
int init_ui() {
  if (!init_ex_opt_str() || !init_state_str() || !init_alignment_type_str() ||
      !init_fs_type_str() || !init_disk_type_str())
    return 0;
  ...
  //
  // sig_segv.sa_mask = sig_int.sa_mask = sig_fpe.sa_mask = sig_ill.sa_mask =
  //     curr;
  //
  // sig_segv.sa_flags = sig_int.sa_flags = sig_fpe.sa_flags =
  // sig_ill.sa_flags =
  //     SA_SIGINFO;
  //
  // sigaction(SIGSEGV, &sig_segv, NULL);
...
int interactive_mode(PedDevice **dev, PedDisk **disk, Command *cmd_list[]) {
  ...
  while (1) {
    ...
    while (!command_line_get_word_count()) {
      ...
      command_line_prompt_words("(parted)", NULL, command_names, 1);
    }
...
int non_interactive_mode(PedDevice **dev, PedDisk **disk, Command *cmd_list[],
                         int argc, char *argv[]) {
  int i;
  Command *cmd;
