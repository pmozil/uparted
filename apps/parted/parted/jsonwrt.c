...
static void fputs_quoted_case_json(const char *data, FILE *out, int dir) {
  ...
  for (p = data; p && *p; p++) {
    ...
    if (c >= 0x20) {
       ...
       */
      if (c <= 127)
        fputc(dir == 1 ? c_toupper(c) : dir == -1 ? c_tolower(c) : *p, out);
      else
...
void ul_jsonwrt_value_raw(struct ul_jsonwrt *fmt, const char *name,
                          const char *data) {
...
void ul_jsonwrt_value_s(struct ul_jsonwrt *fmt, const char *name,
                        const char *data) {
  ul_jsonwrt_value_open(fmt, name);
  if (data)
...
void ul_jsonwrt_value_u64(struct ul_jsonwrt *fmt, const char *name,
                          uint64_t data) {
  ul_jsonwrt_value_open(fmt, name);
  fprintf(fmt->out,
          "%"
...
void ul_jsonwrt_value_boolean(struct ul_jsonwrt *fmt, const char *name,
                              int data) {
  ul_jsonwrt_value_open(fmt, name);
...
void ul_jsonwrt_value_null(struct ul_jsonwrt *fmt, const char *name) {
  ul_jsonwrt_value_open(fmt, name);
  fputs("null", fmt->out);
  ul_jsonwrt_value_close(fmt);
}
